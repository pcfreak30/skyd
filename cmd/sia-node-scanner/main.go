...
type nodeScanner struct {
	...
	// scanner so far.
	seen map[skymodules.NetAddress]struct{}
	// The queue holds nodes to be added to workCh.
	queue []skymodules.NetAddress
...
type persistData struct {
	...
	NodeStats map[skymodules.NetAddress]nodeStats
}
...
type nodeScanResult struct {
	...
	Timestamp time.Time
	Err       error
	nodes     map[skymodules.NetAddress]struct{}
}
...
func (ns *nodeScanner) initialize() {
	...
	if len(ns.data.NodeStats) == 0 {
		...
		ns.queue = make([]skymodules.NetAddress, len(skymodules.BootstrapPeers))
		copy(ns.queue, skymodules.BootstrapPeers)
	} else {
		ns.queue = make([]skymodules.NetAddress, 0, len(ns.data.NodeStats))
	...
	ns.seen = make(map[skymodules.NetAddress]struct{})
	...
	var i int
	var node skymodules.NetAddress
...
func sendShareNodesRequests(g *gateway.Gateway, work workAssignment) nodeScanResult {
	result := nodeScanResult{
		...
		Err:       nil,
		Timestamp: time.Now(),
		nodes:     make(map[skymodules.NetAddress]struct{}),
	}
	...
	for i := 0; i < maxRPCAttempts; i++ {
		var newNodes []skymodules.NetAddress
		result.Err = g.RPC(work.node, "ShareNodes", func(conn skymodules.PeerConn) error {
			return encoding.ReadObject(conn, &newNodes, maxSharedNodes*skymodules.MaxEncodedNetAddressLength)
		})
		if result.Err != nil {
			return result
		}
