...
func main() {
	...
	/* TODO: Parse a port number from the flags.
		// Parse flags.
		var port string
		portPtr := flag.String("port", ":9980", "which port to use for the siad api")
		if port == nil {
			port = ":9980"
		}
	*/

	c = client.New("localhost:9970")
	...
	threadss := []uint64{16, 64} // threadss is the plural of threads, deal with it
