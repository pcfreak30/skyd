...
func TestLimitStreamer(t *testing.T) {
	...
	streamer := renter.SkylinkStreamerFromSlice(data, skymodules.SkyfileMetadata{}, []byte{})
	...
	// length of the data
	streamer = renter.SkylinkStreamerFromSlice(data, skymodules.SkyfileMetadata{}, []byte{})
	streamer, err = NewLimitStreamer(streamer, skymodules.SkyfileMetadata{}, 0, uint64(len(data)))
	if err != nil {
		t.Fatal(err)
	}
	allData, err = ioutil.ReadAll(streamer)
	...
	streamer = renter.SkylinkStreamerFromSlice(data, skymodules.SkyfileMetadata{}, []byte{})
	streamer, err = NewLimitStreamer(streamer, skymodules.SkyfileMetadata{}, 20, uint64(len(data)))
	if err != nil {
		t.Fatal(err)
	}
	allData, err = ioutil.ReadAll(streamer)
	...
	streamer = renter.SkylinkStreamerFromSlice(data, skymodules.SkyfileMetadata{}, []byte{})
	streamer, err = NewLimitStreamer(streamer, skymodules.SkyfileMetadata{}, 20, 13)
	if err != nil {
		t.Fatal(err)
	}
	allData, err = ioutil.ReadAll(streamer)
