...
func TestInstructionAppend(t *testing.T) {
	...
	finalize, outputs, err := mdm.ExecuteProgram(context.Background(), pt, instructions, modules.InitCost(pt, dataLen).Add(modules.WriteCost(pt, modules.SectorSize)), so, dataLen, bytes.NewReader(programData))
	if err != nil {
		t.Fatal(err)
	}
