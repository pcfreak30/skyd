...
func (h *Host) managedRPCLoopWrite(s *rpcSession) error {
	...
	if !req.MerkleProof {
		if err := s.readResponse(&sigResponse, modules.RPCMaxLen); err != nil {
			return err
		}
	}
	...
	if req.MerkleProof {
		if err := s.writeResponse(merkleResp); err != nil {
			return err
		} else if err := s.readResponse(&sigResponse, modules.RPCMaxLen); err != nil {
			return err
		}
	}
...
func (h *Host) managedRPCLoopRead(s *rpcSession) error {
	...
	go func() {
		...
		err := s.readResponse(&id, modules.RPCMaxLen)
		if err != nil {
			stopSignal <- err
...
func (h *Host) managedRPCLoopFormContract(s *rpcSession) error {
	...
	if err := s.readResponse(&renterSigs, modules.RPCMaxLen); err != nil {
		s.writeError(err)
		return err
	}
