...
func (r *Renter) FileList() ([]modules.FileInfo, error) {
	offlineMap, _, contractsMap := r.managedContractUtilityMaps()
	...
	err := filepath.Walk(r.staticFilesDir, func(path string, info os.FileInfo, err error) error {
		...
		file, err := r.fileInfo(siaPath, offlineMap, contractsMap)
		if err != nil {
			return err
		}
...
func (r *Renter) File(siaPath string) (modules.FileInfo, error) {
	offline, _, contracts := r.managedContractUtilityMaps()
	return r.fileInfo(siaPath, offline, contracts)
}
...
// many files at once.
func (r *Renter) fileInfo(siaPath string, offline map[string]bool, contracts map[string]modules.RenterContract) (modules.FileInfo, error) {
