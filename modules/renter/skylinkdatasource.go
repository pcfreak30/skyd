...
func (sds *skylinkDataSource) ReadChannel(off, fetchSize int64) (chan byte, chan error) {
	...
	firstChunkLength := int64(len(sds.staticFirstChunk))

	// Prepare an array of download chans on which we'll receive the data.
	numChunks := fetchSize / int64(chunkSize)
	if fetchSize%int64(chunkSize) != 0 {
		numChunks += 1
	}
	...
	for n < fetchSize && off < int64(sds.staticLayout.Filesize) {
		...
		off += int64(downloadSize)
		n += int64(downloadSize)
	}
