...
func TestBubbleHealth(t *testing.T) {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		if err = equalBubbledMetadata(metadata, expectedHealth, time.Since(beforeBubble)); err != nil {
			return err
		}
		return nil
	})
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		expectedHealth.AggregateHealth = 1
		expectedHealth.AggregateStuckHealth = 2
		expectedHealth.AggregateNumFiles = 1
		expectedHealth.AggregateNumSubDirs = 8
		expectedHealth.AggregateNumStuckChunks = f.NumStuckChunks()
		expectedHealth.AggregateSize = f.Size()
		expectedHealth.AggregateModTime = beforeBubble
		if err = equalBubbledMetadata(metadata, expectedHealth, time.Since(beforeBubble)); err != nil {
			return err
		}
		return nil
...
func TestOldestHealthCheckTime(t *testing.T) {
	...
	// Now update the root directory to have an older
	// AggregateLastHealthCheckTime than the sub directory's but still more
	// recent than the sub directory's LastHealthCheckTime. This will simulate a
	// shutdown before all the pending bubbles could finish.
	entry, err := rt.renter.staticFileSystem.OpenSiaDir(modules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	rootTime := oldestCheckTime.AddDate(0, 0, 1)
	...
	// Try again but now the AggregateLastHealthCheckTime is older than the sub
	// directory's and also older than the sub directory's LastHealthCheckTime.
	entry, err = rt.renter.staticFileSystem.OpenSiaDir(modules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	rootTime = oldestCheckTime.AddDate(0, 0, -3)
	err = entry.UpdateLastHealthCheckTime(rootTime, now)
	if err != nil {
		t.Fatal(err)
	}
