...
func (w *worker) threadedWorkLoop() {
	...
	if build.VersionCmp(w.staticCache().staticHostVersion, minAsyncVersion) >= 0 {
		...
		// We do a quick balance check on the host right here. Even if we had a
		// clean shutdown and know the exact balance, we still want to ask the
		// host what our balance is, because we don't want the host to be able
