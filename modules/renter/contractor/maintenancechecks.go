...
func (c *Contractor) criticalUtilityChecks(contract modules.RenterContract, host modules.HostDBEntry) (modules.ContractUtility, bool) {
	...
	u, needsUpdate = c.sufficientFundsCheck(contract, host)
	if needsUpdate {
		return u, needsUpdate
	}
...
// Returns true if a check fails and the utility returned must be used to update
// the contract state.
func (c *Contractor) sufficientFundsCheck(contract modules.RenterContract, host modules.HostDBEntry) (modules.ContractUtility, bool) {
	u := contract.Utility
	...
	blockBytes := types.NewCurrency64(modules.SectorSize * uint64(contractLength))
