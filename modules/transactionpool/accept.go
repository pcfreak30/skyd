...
func (tp *TransactionPool) acceptTransactionSet(ts []types.Transaction, txnFn func([]types.Transaction) (modules.ConsensusChange, error)) (superset []types.Transaction, err error) {
	...
	defer func() {
		tp.log.Printf(" . acceptTransactionSet end (%v): %v", time.Since(start).Milliseconds(), len(ts))
	}()
...
func (tp *TransactionPool) managedSubmitTransactionSet(ts []types.Transaction) ([]types.Transaction, error) {
	...
	err := cs.LockedTryTransactionSet(func(txnFn func(txns []types.Transaction) (modules.ConsensusChange, error)) error {
		...
		defer func() {
			tp.log.Printf("lockedTryTransactionSet end (%v): %v", time.Since(start).Milliseconds(), len(ts))
		}()
