...
func dbReset(tx *bolt.Tx) error {
	...
	dbPutConsensusChangeID(tx, skymodules.ConsensusChangeBeginning)
...
// bucketProcessedTransactions.
func dbAddProcessedTransactionAddrs(tx *bolt.Tx, pt skymodules.ProcessedTransaction, txn uint64) error {
...
func decodeProcessedTransaction(ptBytes []byte, pt *skymodules.ProcessedTransaction) error {
...
func dbAppendProcessedTransaction(tx *bolt.Tx, pt skymodules.ProcessedTransaction) error {
...
func dbGetLastProcessedTransaction(tx *bolt.Tx) (pt skymodules.ProcessedTransaction, err error) {
...
func dbGetProcessedTransaction(tx *bolt.Tx, index uint64) (pt skymodules.ProcessedTransaction, err error) {
	// big-endian is used so that the keys are properly sorted
	indexBytes := make([]byte, 8)
	binary.BigEndian.PutUint64(indexBytes, index)
...
func (it *processedTransactionsIter) value() skymodules.ProcessedTransaction {
	return it.pt
}
...
func dbGetConsensusChangeID(tx *bolt.Tx) (cc skymodules.ConsensusChangeID) {
...
func dbPutConsensusChangeID(tx *bolt.Tx, cc skymodules.ConsensusChangeID) error {
...
func convertProcessedTransaction(oldpt v121ProcessedTransaction) (pt skymodules.ProcessedTransaction) {
	...
	pt.Inputs = make([]skymodules.ProcessedInput, len(oldpt.Inputs))
	for i, in := range oldpt.Inputs {
		pt.Inputs[i] = skymodules.ProcessedInput{
	...
	pt.Outputs = make([]skymodules.ProcessedOutput, len(oldpt.Outputs))
	for i, out := range oldpt.Outputs {
		pt.Outputs[i] = skymodules.ProcessedOutput{
