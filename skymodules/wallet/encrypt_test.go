...
// errors.
func postEncryptionTesting(m skymodules.TestMiner, w *Wallet, masterKey crypto.CipherKey) {
	...
	err = w.Lock()
	if !errors.Contains(err, skymodules.ErrLockedWallet) {
		panic(err)
	}
	err = w.Unlock(nil)
...
func TestIntegrationPreEncryption(t *testing.T) {
	...
	w1, err := New(wt.cs, wt.tpool, filepath.Join(wt.persistDir, skymodules.WalletDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLock(t *testing.T) {
	...
	wt, err := createWalletTester(t.Name(), skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestInitFromSeedConcurrentUnlock(t *testing.T) {
	...
	dir := filepath.Join(build.TempDir(skymodules.WalletDir, t.Name()+"-new"), skymodules.WalletDir)
...
func TestInitFromSeed(t *testing.T) {
	...
	// create a wallet with some money
	wt, err := createWalletTester("TestInitFromSeed0", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	// create a blank wallet
	dir := filepath.Join(build.TempDir(skymodules.WalletDir, "TestInitFromSeed1"), skymodules.WalletDir)
	w, err := New(wt.cs, wt.tpool, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReset(t *testing.T) {
	...
	minerData := filepath.Join(wt.persistDir, skymodules.MinerDir)
	err = os.RemoveAll(minerData)
	if err != nil {
		t.Fatal(err)
	}
	newminer, err := miner.New(wt.cs, wt.tpool, wt.wallet, filepath.Join(wt.persistDir, skymodules.MinerDir))
	if err != nil {
		t.Fatal(err)
	}
	wt.miner = newminer
...
func TestChangeKeyWithSeedCompatV141(t *testing.T) {
	...
	wallet, err := New(wt.cs, wt.tpool, filepath.Join(wt.persistDir, skymodules.WalletDir))
	if err != nil {
