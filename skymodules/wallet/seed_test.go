...
func TestLoadSeed(t *testing.T) {
	...
	dir := filepath.Join(build.TempDir(skymodules.WalletDir, t.Name()+"1"), skymodules.WalletDir)
...
func TestSweepSeedCoins(t *testing.T) {
	...
	wt, err := createWalletTester("TestSweepSeedCoins0", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	dir := filepath.Join(build.TempDir(skymodules.WalletDir, "TestSweepSeedCoins1"), skymodules.WalletDir)
...
func TestSweepSeedFunds(t *testing.T) {
	...
	t.Parallel()
	wt, err := createWalletTester("TestSweepSeedFunds", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestSweepSeedSentFunds(t *testing.T) {
	...
	t.Parallel()
	wt, err := createWalletTester("TestSweepSeedSentFunds", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestSweepSeedCoinsAndFunds(t *testing.T) {
	...
	t.Parallel()
	wt, err := createWalletTester("TestSweepSeedCoinsAndFunds", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestGenerateKeys(t *testing.T) {
	for i, k := range generateKeys(skymodules.Seed{}, 1000, 4000) {
