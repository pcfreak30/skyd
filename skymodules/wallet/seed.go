...
// given index.
func generateSpendableKey(seed skymodules.Seed, index uint64) spendableKey {
...
func generateKeys(seed skymodules.Seed, start, n uint64) []spendableKey {
...
func createSeedFile(masterKey crypto.CipherKey, seed skymodules.Seed) seedFile {
	var sf seedFile
...
func decryptSeedFile(masterKey crypto.CipherKey, sf seedFile) (seed skymodules.Seed, err error) {
	...
	if err != nil {
		return skymodules.Seed{}, err
	}

	// Decrypt and return the seed.
	plainSeed, err := decryptionKey.DecryptBytes(sf.Seed)
	if err != nil {
		return skymodules.Seed{}, errors.AddContext(err, "failed to decrypt seed")
	}
	copy(seed[:], plainSeed)
	return seed, nil
}
...
// the wallet.
func (w *Wallet) integrateSeed(seed skymodules.Seed, n uint64) {
...
func (w *Wallet) nextPrimarySeedAddresses(tx *bolt.Tx, n uint64) ([]types.UnlockConditions, error) {
	...
	if !w.unlocked {
		return []types.UnlockConditions{}, skymodules.ErrLockedWallet
	}
...
func (w *Wallet) AllSeeds() ([]skymodules.Seed, error) {
	if err := w.tg.Add(); err != nil {
		return nil, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
	...
	defer w.mu.Unlock()
	if !w.unlocked {
		return nil, skymodules.ErrLockedWallet
	}
	return append([]skymodules.Seed{w.primarySeed}, w.seeds...), nil
}
...
func (w *Wallet) PrimarySeed() (skymodules.Seed, uint64, error) {
	if err := w.tg.Add(); err != nil {
		return skymodules.Seed{}, 0, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
	...
	defer w.mu.Unlock()
	if !w.unlocked {
		return skymodules.Seed{}, 0, skymodules.ErrLockedWallet
	}
	progress, err := dbGetPrimarySeedProgress(w.dbTx)
	if err != nil {
		return skymodules.Seed{}, 0, err
	}
...
func (w *Wallet) MarkAddressUnused(addrs ...types.UnlockConditions) error {
	if err := w.tg.Add(); err != nil {
		return skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
...
func (w *Wallet) NextAddress() (types.UnlockConditions, error) {
	if err := w.tg.Add(); err != nil {
		return types.UnlockConditions{}, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
...
// the wallet.
func (w *Wallet) LoadSeed(masterKey crypto.CipherKey, seed skymodules.Seed) error {
	if err := w.tg.Add(); err != nil {
		return skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
	...
	if !w.unlocked {
		w.mu.RUnlock()
		return skymodules.ErrLockedWallet
	}
	for _, wSeed := range append([]skymodules.Seed{w.primarySeed}, w.seeds...) {
		if seed == wSeed {
			w.mu.RUnlock()
			return errKnownSeed
		}
	...
	err := func() error {
		...
		err = dbPutConsensusChangeID(w.dbTx, skymodules.ConsensusChangeBeginning)
		if err != nil {
			return err
		}
	...
	err = w.cs.ConsensusSetSubscribe(w, skymodules.ConsensusChangeBeginning, w.tg.StopChan())
	if err != nil {
		return err
	}
...
func (w *Wallet) SweepSeed(seed skymodules.Seed) (coins, funds types.Currency, err error) {
	if err = w.tg.Add(); err != nil {
		return types.Currency{}, types.Currency{}, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
