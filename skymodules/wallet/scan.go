...
// seedScanner.
func (s *seedScanner) ProcessConsensusChange(cc skymodules.ConsensusChange) {
	// update outputs
	for _, diff := range cc.SiacoinOutputDiffs {
		...
		} else if diff.Direction == skymodules.DiffRevert {
...
func (s *seedScanner) scan(cs skymodules.ConsensusSet, cancel <-chan struct{}) error {
	...
	for s.numKeys() < maxScanKeys {
		...
		s.scannedHeight = 0
		if err := cs.ConsensusSetSubscribe(s, skymodules.ConsensusChangeBeginning, cancel); err != nil {
			return err
		}
		cs.Unsubscribe(s)
...
func newSeedScanner(seed skymodules.Seed, log *persist.Logger) *seedScanner {
	return &seedScanner{
		seed:           seed,
