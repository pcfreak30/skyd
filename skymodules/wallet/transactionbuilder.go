...
// transaction).
func (tb *transactionBuilder) Copy() skymodules.TransactionBuilder {
...
func (tb *transactionBuilder) FundSiacoins(amount types.Currency) (err error) {
	...
	if potentialFund.Cmp(amount) >= 0 && fund.Cmp(amount) < 0 {
		return skymodules.ErrIncompleteTransactions
	}
	if fund.Cmp(amount) < 0 {
		return skymodules.ErrLowBalance
	}
...
// RegisterTransaction takes a transaction and its parents and returns a
// skymodules.TransactionBuilder which can be used to expand the transaction. The
// most typical call is 'RegisterTransaction(types.Transaction{}, nil)', which
// registers a new transaction without parents.
func (w *Wallet) RegisterTransaction(t types.Transaction, parents []types.Transaction) (skymodules.TransactionBuilder, error) {
...
func (w *Wallet) StartTransaction() (skymodules.TransactionBuilder, error) {
	if err := w.tg.Add(); err != nil {
		return nil, err
	}
	defer w.tg.Done()
