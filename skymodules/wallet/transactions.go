...
// single unlock hash.
func (w *Wallet) AddressTransactions(uh types.UnlockHash) (pts []skymodules.ProcessedTransaction, err error) {
	if err := w.tg.Add(); err != nil {
		return []skymodules.ProcessedTransaction{}, err
	}
	defer w.tg.Done()
...
func (w *Wallet) AddressUnconfirmedTransactions(uh types.UnlockHash) (pts []skymodules.ProcessedTransaction, err error) {
	if err := w.tg.Add(); err != nil {
		return []skymodules.ProcessedTransaction{}, err
	}
	defer w.tg.Done()
...
func (w *Wallet) Transaction(txid types.TransactionID) (pt skymodules.ProcessedTransaction, found bool, err error) {
	if err := w.tg.Add(); err != nil {
		return skymodules.ProcessedTransaction{}, false, err
	}
	defer w.tg.Done()
	...
	if err != nil {
		...
		return skymodules.ProcessedTransaction{}, false, nil
	}
...
func (w *Wallet) Transactions(startHeight, endHeight types.BlockHeight) (pts []skymodules.ProcessedTransaction, err error) {
	if err := w.tg.Add(); err != nil {
		return nil, err
	}
	defer w.tg.Done()
	...
	var pt skymodules.ProcessedTransaction
...
func ComputeValuedTransactions(pts []skymodules.ProcessedTransaction, blockHeight types.BlockHeight) ([]skymodules.ValuedTransaction, error) {
	...
	sts := make([]skymodules.ValuedTransaction, 0, len(pts))
	for _, pt := range pts {
		...
		// revisions.
		st := skymodules.ValuedTransaction{
...
func (w *Wallet) UnconfirmedTransactions() ([]skymodules.ProcessedTransaction, error) {
	if err := w.tg.Add(); err != nil {
		return nil, err
	}
	defer w.tg.Done()
