...
func (w *Wallet) updateLookahead(tx *bolt.Tx, cc skymodules.ConsensusChange) (bool, error) {
...
func (w *Wallet) updateConfirmedSet(tx *bolt.Tx, cc skymodules.ConsensusChange) error {
	for _, diff := range cc.SiacoinOutputDiffs {
		...
		var err error
		if diff.Direction == skymodules.DiffApply {
...
func computeSpentSiacoinOutputSet(diffs []skymodules.SiacoinOutputDiff) spentSiacoinOutputSet {
	outputs := make(spentSiacoinOutputSet)
	for _, diff := range diffs {
		if diff.Direction == skymodules.DiffRevert {
...
func computeSpentSiafundOutputSet(diffs []skymodules.SiafundOutputDiff) spentSiafundOutputSet {
...
func (w *Wallet) computeProcessedTransactionsFromBlock(tx *bolt.Tx, block types.Block, spentSiacoinOutputs spentSiacoinOutputSet, spentSiafundOutputs spentSiafundOutputSet, consensusHeight types.BlockHeight) []skymodules.ProcessedTransaction {
	var pts []skymodules.ProcessedTransaction
	...
	if relevant {
		...
		minerPT := skymodules.ProcessedTransaction{
		...
		for i, mp := range block.MinerPayouts {
			...
			minerPT.Outputs = append(minerPT.Outputs, skymodules.ProcessedOutput{
	...
	for _, txn := range block.Transactions {
		...
		pt := skymodules.ProcessedTransaction{
			Transaction:           txn,
		...
		for _, fee := range txn.MinerFees {
			pt.Outputs = append(pt.Outputs, skymodules.ProcessedOutput{
...
// introduced.
func (w *Wallet) applyHistory(tx *bolt.Tx, cc skymodules.ConsensusChange) error {
...
func (w *Wallet) ProcessConsensusChange(cc skymodules.ConsensusChange) {
...
// transaction set.
func (w *Wallet) ReceiveUpdatedUnconfirmedTransactions(diff *skymodules.TransactionPoolDiff) {
	if err := w.tg.Add(); err != nil {
		return
	}
	defer w.tg.Done()
	...
	if len(droppedTransactions) != 0 {
		...
		newUPT := make([]skymodules.ProcessedTransaction, 0, len(w.unconfirmedProcessedTransactions))
	...
	for _, unconfirmedTxnSet := range diff.AppliedTransactions {
		...
		for _, scod := range unconfirmedTxnSet.Change.SiacoinOutputDiffs {
			...
			if scod.Direction == skymodules.DiffRevert {
		...
		for i, txn := range unconfirmedTxnSet.Transactions {
			...
			for _, sci := range txn.SiacoinInputs {
				pt.Inputs = append(pt.Inputs, skymodules.ProcessedInput{
					ParentID:       types.OutputID(sci.ParentID),
					FundType:       types.SpecifierSiacoinInput,
					WalletAddress:  w.isWalletAddress(sci.UnlockConditions.UnlockHash()),
					RelatedAddress: sci.UnlockConditions.UnlockHash(),
