...
type Wallet struct {
	...
	unconfirmedSets                  map[skymodules.TransactionSetID][]types.TransactionID
	unconfirmedProcessedTransactions []skymodules.ProcessedTransaction
...
func (w *Wallet) Height() (types.BlockHeight, error) {
	if err := w.tg.Add(); err != nil {
		return types.BlockHeight(0), skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
...
// call to 'Unlock'.
func New(cs skymodules.ConsensusSet, tpool skymodules.TransactionPool, persistDir string) (*Wallet, error) {
	return NewCustomWallet(cs, tpool, persistDir, skymodules.ProdDependencies)
}

// NewCustomWallet creates a new wallet using custom dependencies.
func NewCustomWallet(cs skymodules.ConsensusSet, tpool skymodules.TransactionPool, persistDir string, deps skymodules.Dependencies) (*Wallet, error) {
	...
	w := &Wallet{
		...
		unconfirmedSets: make(map[skymodules.TransactionSetID][]types.TransactionID),
...
func (w *Wallet) AllAddresses() ([]types.UnlockHash, error) {
	if err := w.tg.Add(); err != nil {
		return []types.UnlockHash{}, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
...
func (w *Wallet) Rescanning() (bool, error) {
	if err := w.tg.Add(); err != nil {
		return false, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
...
func (w *Wallet) Settings() (skymodules.WalletSettings, error) {
	if err := w.tg.Add(); err != nil {
		return skymodules.WalletSettings{}, skymodules.ErrWalletShutdown
	}
	defer w.tg.Done()
	return skymodules.WalletSettings{
		NoDefrag: w.defragDisabled,
	}, nil
}

// SetSettings will update the settings for the wallet.
func (w *Wallet) SetSettings(s skymodules.WalletSettings) error {
	if err := w.tg.Add(); err != nil {
