...
func createWalletTester(name string, deps skymodules.Dependencies) (*walletTester, error) {
	// Create the modules
	testdir := build.TempDir(skymodules.WalletDir, name)
	...
	w, err := NewCustomWallet(cs, tp, filepath.Join(testdir, skymodules.WalletDir), deps)
	if err != nil {
		return nil, err
	}
	...
	m, err := miner.New(cs, tp, w, filepath.Join(testdir, skymodules.WalletDir))
	if err != nil {
		return nil, err
	}
...
func createBlankWalletTester(name string) (*walletTester, error) {
	...
	w, err := New(cs, tp, filepath.Join(testdir, skymodules.WalletDir))
	if err != nil {
		return nil, err
	}
...
func TestNilInputs(t *testing.T) {
	...
	testdir := build.TempDir(skymodules.WalletDir, t.Name())
	g, err := gateway.New("localhost:0", false, filepath.Join(testdir, skymodules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	wdir := filepath.Join(testdir, skymodules.WalletDir)
...
func TestDistantWallets(t *testing.T) {
	...
	w2, err := New(wt.cs, wt.tpool, build.TempDir(skymodules.WalletDir, t.Name()+"2", skymodules.WalletDir))
	if err != nil {
		t.Fatal(err)
	}
