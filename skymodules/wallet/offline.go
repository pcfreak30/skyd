...
func (w *Wallet) UnspentOutputs() ([]skymodules.UnspentOutput, error) {
...
func SignTransaction(txn *types.Transaction, seed skymodules.Seed, toSign []crypto.Hash, height types.BlockHeight) error {
	if len(toSign) == 0 {
...
func (w *Wallet) AddWatchAddresses(addrs []types.UnlockHash, unused bool) error {
	...
	err := func() error {
		...
		if !unused {
			...
			if err := dbPutConsensusChangeID(w.dbTx, skymodules.ConsensusChangeBeginning); err != nil {
				return err
			}
	...
	if !unused {
		...
		defer close(done)
		if err := w.cs.ConsensusSetSubscribe(w, skymodules.ConsensusChangeBeginning, w.tg.StopChan()); err != nil {
			return err
		}
