...
func BenchmarkAddressTransactions(b *testing.B) {
	wt, err := createWalletTester(b.Name(), skymodules.ProdDependencies)
	if err != nil {
		b.Fatal(err)
	}
	...
	wt.wallet.mu.Lock()
	for i := 0; i < 10000; i++ {
		err := dbAppendProcessedTransaction(wt.wallet.dbTx, skymodules.ProcessedTransaction{
	...
	err = dbAppendProcessedTransaction(wt.wallet.dbTx, skymodules.ProcessedTransaction{
		TransactionID: types.TransactionID{1},
		Inputs: []skymodules.ProcessedInput{{
