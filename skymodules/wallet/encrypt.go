...
func walletPasswordEncryptionKey(seed skymodules.Seed, salt walletSalt) (key crypto.CipherKey) {
	var h crypto.Hash
...
func verifyEncryption(key crypto.CipherKey, encrypted crypto.Ciphertext) error {
	...
	if err != nil {
		contextErr := errors.AddContext(skymodules.ErrBadEncryptionKey, "failed to decrypt key")
...
func (w *Wallet) initEncryption(masterKey crypto.CipherKey, seed skymodules.Seed, progress uint64) (skymodules.Seed, error) {
	...
	if wb.Get(keyEncryptionVerification) != nil {
		return skymodules.Seed{}, errReencrypt
	}
...
// wallet's database.
func (w *Wallet) managedMasterKey(seed skymodules.Seed) (crypto.CipherKey, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *Wallet) managedBlockingUnlock(masterKey crypto.CipherKey) (skymodules.ConsensusChangeID, error) {
	w.mu.RLock()
	unlocked := w.unlocked
	encrypted := w.encrypted
	w.mu.RUnlock()
	if unlocked {
		return skymodules.ConsensusChangeID{}, errAlreadyUnlocked
	} else if !encrypted {
		return skymodules.ConsensusChangeID{}, errUnencryptedWallet
	}

	// Load db objects into memory.
	var lastChange skymodules.ConsensusChangeID
	...
	err = func() error {
		...
		for _, sf := range auxiliarySeedFiles {
			...
			w.integrateSeed(auxSeed, skymodules.PublicKeysPerSeed)
...
func (w *Wallet) managedAsyncUnlock(lastChange skymodules.ConsensusChangeID) error {
...
// or deleted.
func (w *Wallet) Encrypt(masterKey crypto.CipherKey) (skymodules.Seed, error) {
	if err := w.tg.Add(); err != nil {
		return skymodules.Seed{}, err
	}
	defer w.tg.Done()
...
func (w *Wallet) Reset() error {
	...
	w.seeds = []skymodules.Seed{}
	w.unconfirmedProcessedTransactions = []skymodules.ProcessedTransaction{}
	w.unlocked = false
	w.encrypted = false
...
// synced.
func (w *Wallet) InitFromSeed(masterKey crypto.CipherKey, seed skymodules.Seed) error {
	if err := w.tg.Add(); err != nil {
		return err
	}
	defer w.tg.Done()
...
func (w *Wallet) ChangeKeyWithSeed(seed skymodules.Seed, newKey crypto.CipherKey) error {
	if err := w.tg.Add(); err != nil {
		return err
	}
	defer w.tg.Done()
...
func (w *Wallet) managedChangeKey(masterKey crypto.CipherKey, newKey crypto.CipherKey) error {
	...
	// decrypt key files
	var primarySeed skymodules.Seed
	var auxiliarySeeds []skymodules.Seed
