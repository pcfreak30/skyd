...
type watchdog struct {
	...
	// archival purposes.
	archivedContracts map[types.FileContractID]skymodules.ContractWatchStatus
...
func newWatchdog(contractor *Contractor) *watchdog {
	...
	return &watchdog{
		...
		archivedContracts:  make(map[types.FileContractID]skymodules.ContractWatchStatus),
...
func (w *watchdog) callAllowanceUpdated(a skymodules.Allowance) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *watchdog) archiveContract(fcID types.FileContractID, doubleSpendHeight types.BlockHeight) {
	...
	w.archivedContracts[fcID] = skymodules.ContractWatchStatus{
		Archived:                  true,
...
func (w *watchdog) findDependencySpends(txn types.Transaction) {
	...
	for fcID := range inputsSpent {
		...
		if err != nil {
			...
			go w.staticContractor.callNotifyDoubleSpend(fcID, w.blockHeight)
			continue
		}
...
// threadedCheckMonitorRevision checks if the given FileContract has it latest
// revision posted on-chain. If not, the watchdog broadcasts the latest revision
// transaction itself.
func (w *watchdog) threadedCheckMonitorRevision(fcID types.FileContractID, height types.BlockHeight) {
	...
	contract, ok := w.staticContractor.staticContracts.Acquire(fcID)
	if ok {
...
func (w *watchdog) sweepContractInputs(fcID types.FileContractID, contractData *fileContractStatus) {
	...
	_, maxFee := w.staticTPool.FeeEstimation()
...
// exists.
func (w *watchdog) managedContractStatus(fcID types.FileContractID) (skymodules.ContractWatchStatus, bool) {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	return skymodules.ContractWatchStatus{
		Archived:                  false,
		FormationSweepHeight:      contractData.formationSweepHeight,
		ContractFound:             contractData.contractFound,
		LatestRevisionFound:       contractData.revisionFound,
