...
func (c *Contractor) findRecoverableContracts(renterSeed skymodules.RenterSeed, b types.Block) {
	for _, txn := range b.Transactions {
		...
		for i, fc := range txn.FileContracts {
			...
			ourSK, ourPK := skymodules.GenerateContractKeyPair(rs, txn)
			...
			c.recoverableContracts[fcid] = skymodules.RecoverableContract{
				FileContract:  fc,
				ID:            fcid,
...
func (c *Contractor) callRecoverContracts() {
	...
	renterSeed := skymodules.DeriveRenterSeed(ws)
	defer fastrand.Read(renterSeed[:])
	// Copy necessary fields to avoid having to hold the lock for too long.
	c.mu.RLock()
	blockHeight := c.blockHeight
	recoverableContracts := make([]skymodules.RecoverableContract, 0, len(c.recoverableContracts))
	...
	for i, recoverableContract := range recoverableContracts {
		wg.Add(1)
		go func(j int, rc skymodules.RecoverableContract) {
			defer wg.Done()
