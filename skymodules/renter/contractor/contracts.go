...
func (c *Contractor) managedCancelContract(cid types.FileContractID) error {
	return c.managedAcquireAndUpdateContractUtility(cid, skymodules.ContractUtility{
		GoodForRenew:  false,
		GoodForUpload: false,
		Locked:        true,
	})
...
// child contract.
func (c *Contractor) managedContractByPublicKey(pk types.SiaPublicKey) (skymodules.RenterContract, bool) {
	c.mu.RLock()
...
func (c *Contractor) managedContractUtility(id types.FileContractID) (skymodules.ContractUtility, bool) {
...
func (c *Contractor) updatePubKeyToContractIDMap(contracts []skymodules.RenterContract) {
...
func (c *Contractor) tryAddContractToPubKeyMap(newContract skymodules.RenterContract) {
...
// contract.
func (c *Contractor) ContractByPublicKey(pk types.SiaPublicKey) (skymodules.RenterContract, bool) {
...
// returned.
func (c *Contractor) Contracts() []skymodules.RenterContract {
	return c.staticContracts.ViewAll()
}

// ContractUtility returns the utility fields for the given contract.
func (c *Contractor) ContractUtility(pk types.SiaPublicKey) (skymodules.ContractUtility, bool) {
	c.mu.RLock()
	id, ok := c.pubKeysToContractID[pk.String()]
	c.mu.RUnlock()
	if !ok {
...
// expired
func (c *Contractor) OldContracts() []skymodules.RenterContract {
	c.mu.Lock()
	defer c.mu.Unlock()
	contracts := make([]skymodules.RenterContract, 0, len(c.oldContracts))
...
func (c *Contractor) RecoverableContracts() []skymodules.RecoverableContract {
	c.mu.Lock()
	defer c.mu.Unlock()
	contracts := make([]skymodules.RecoverableContract, 0, len(c.recoverableContracts))
