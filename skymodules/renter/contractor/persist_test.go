...
func TestSaveLoad(t *testing.T) {
	...
	expectedArchivedContract := skymodules.ContractWatchStatus{
		Archived:                  true,
	...
	c.staticWatchdog.archivedContracts = map[types.FileContractID]skymodules.ContractWatchStatus{
		{2}: expectedArchivedContract,
	}

	c.oldContracts = map[types.FileContractID]skymodules.RenterContract{
	...
	c.oldContracts = make(map[types.FileContractID]skymodules.RenterContract)
	...
	// values.
	c.allowance = skymodules.DefaultAllowance
	...
	if c.allowance.ExpectedStorage != skymodules.DefaultAllowance.ExpectedStorage {
		t.Errorf("ExpectedStorage was %v but should be %v",
			c.allowance.ExpectedStorage, skymodules.DefaultAllowance.ExpectedStorage)
	}
	if c.allowance.ExpectedUpload != skymodules.DefaultAllowance.ExpectedUpload {
		t.Errorf("ExpectedUpload was %v but should be %v",
			c.allowance.ExpectedUpload, skymodules.DefaultAllowance.ExpectedUpload)
	}
	if c.allowance.ExpectedDownload != skymodules.DefaultAllowance.ExpectedDownload {
		t.Errorf("ExpectedDownload was %v but should be %v",
			c.allowance.ExpectedDownload, skymodules.DefaultAllowance.ExpectedDownload)
	}
	if c.allowance.ExpectedRedundancy != skymodules.DefaultAllowance.ExpectedRedundancy {
		t.Errorf("ExpectedRedundancy was %v but should be %v",
			c.allowance.ExpectedRedundancy, skymodules.DefaultAllowance.ExpectedRedundancy)
	}
