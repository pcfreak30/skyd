...
func TestHostsForPortalFormation(t *testing.T) {
	...
	var activeHosts []skymodules.HostDBEntry
	for i := 0; i < 5; i++ {
		activeHosts = append(activeHosts, skymodules.HostDBEntry{
			PublicKey: randomPK(),
		})
	}
	// The existing contract contain a contract with host 1.
	allContracts := []skymodules.RenterContract{
		{
			ID:            randomID(),
	...
	scoreBreakdown := func(host skymodules.HostDBEntry) (skymodules.HostScoreBreakdown, error) {
		var sb skymodules.HostScoreBreakdown
...
func TestHostsForRegularFormation(t *testing.T) {
	...
	var returnedHosts []skymodules.HostDBEntry
	randomHosts := func(n int, blacklist, addressBlacklist []types.SiaPublicKey) ([]skymodules.HostDBEntry, error) {
		...
		for i := 0; i < n; i++ {
			hosts = append(hosts, skymodules.HostDBEntry{
				PublicKey: randomPK(),
			})
		}
		returnedHosts = hosts
