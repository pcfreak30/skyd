...
func (c *Contractor) managedMarkContractUtility(contract skymodules.RenterContract, minScoreGFR, minScoreGFU types.Currency) (skymodules.HostScoreBreakdown, skymodules.ContractUtility, bool, error) {
	// Acquire contract.
	sc, ok := c.staticContracts.Acquire(contract.ID)
	if !ok {
		return skymodules.HostScoreBreakdown{}, skymodules.ContractUtility{}, false, errors.New("managedMarkContractUtility: Unable to acquire contract")
	}
	...
	u, utilityUpdateStatus := c.managedBasicUtilityChecks(sc.Metadata(), host, sb, minScoreGFR, minScoreGFU)
