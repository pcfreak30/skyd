...
func (w *worker) newPriceTable() {
	if w.staticPriceTable() != nil {
		w.renter.staticLog.Critical("creating a new price table when a new price table already exists")
	}
...
func (w *worker) staticTryForcePriceTableUpdate() {
	...
	if time.Now().Before(current.staticLastForcedUpdate.Add(minElapsedTimeSinceLastScheduledUpdate)) {
		w.renter.staticLog.Debugf("worker for host %v tried scheduling a price table update before the minimum elapsed time", w.staticHostPubKeyStr)
		return
	}
...
func (w *worker) staticUpdatePriceTable() {
	...
	if time.Now().Before(updateTime) {
		w.renter.staticLog.Critical("price table is being updated prematurely")
	}
	...
	if !atomic.CompareAndSwapUint64(&w.atomicPriceTableUpdateRunning, 0, 1) {
		w.renter.staticLog.Critical("price table is being updated in two threads concurrently")
	}
	...
	defer func() {
		...
		if streamCloseErr != nil {
			w.renter.staticLog.Println("ERROR: failed to close stream", streamCloseErr)
		}
	}()
	...
	if err != nil {
		...
		w.renter.staticLog.Println("ERROR: ", err)
		return
	}
