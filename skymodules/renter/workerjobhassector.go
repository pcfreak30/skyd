...
// NOTE: We use the p999 for the HasSector estimate because we want to avoid as
...
// that are faster than their p999 estimate even though we don't anticipate it -
// using the p999 estimate here really improves our long tail numbers without
...
// in on the p999 of the ReadSector jobs results in us avoiding workers that
...
func (jq *jobHasSectorQueue) expectedJobTime() time.Duration {
	return jq.staticDT.Percentiles()[0][2] // p999
}
