...
type (
	...
	jobRenewResponse struct {
		staticNewContract skymodules.RenterContract
...
func (w *worker) initJobRenewQueue() {
	...
	if w.staticJobRenewQueue != nil {
		w.staticRenter.staticLog.Critical("incorret call on initJobRenewQueue")
		return
	}
...
func (w *worker) RenewContract(ctx context.Context, fcid types.FileContractID, params skymodules.ContractParams, txnBuilder modules.TransactionBuilder) (skymodules.RenterContract, []types.Transaction, error) {
	...
	if !w.staticJobRenewQueue.callAdd(jro) {
		return skymodules.RenterContract{}, nil, errors.New("worker unavailable")
	}
	...
	select {
	case <-ctx.Done():
		return skymodules.RenterContract{}, nil, errors.New("Renew interrupted")
