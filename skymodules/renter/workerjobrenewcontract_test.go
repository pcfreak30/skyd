...
func TestRenewContract(t *testing.T) {
	...
	err = wt.UploadSnapshot(context.Background(), skymodules.UploadedBackup{UID: [16]byte{1, 2, 3}}, fastrand.Bytes(100))
	if err != nil {
		t.Fatal(err)
	}
	...
	host, _, err := wt.staticRenter.hostDB.Host(wt.staticHostPubKey)
	if err != nil {
		t.Fatal(err)
	}
	...
	oldContractPreRenew, ok := wt.staticRenter.hostContractor.ContractByPublicKey(params.Host.PublicKey)
	if !ok {
	...
	_, ourPK := skymodules.GenerateContractKeyPair(params.RenterSeed, fcTxn)
	...
	c, found := wt.staticRenter.hostContractor.ContractByPublicKey(wt.staticHostPubKey)
	if !found {
	...
	err = wt.UploadSnapshot(context.Background(), skymodules.UploadedBackup{UID: [16]byte{3, 2, 1}}, fastrand.Bytes(100))
	if err != nil {
		t.Fatal(err)
	}
