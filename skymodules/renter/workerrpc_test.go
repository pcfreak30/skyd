...
func TestUseHostBlockHeight(t *testing.T) {
	...
	// execute the program
	_, _, err = w.managedExecuteProgram(nil, p, data, types.FileContractID{}, categoryDownload, cost)
...
func testExecuteProgramUsedBandwidthHasSector(t *testing.T, wt *workerTester) {
	...
	// execute it
	_, limit, err := w.managedExecuteProgram(nil, p, data, types.FileContractID{}, categoryDownload, cost)
	if err != nil {
		t.Fatal(err)
	}
