...
// health of a siadir
func (r *Renter) updateSiaDirHealth(siaPath skymodules.SiaPath, health, aggregateHealth float64) (err error) {
...
func addDirectoriesToHeap(r *Renter, numDirs int, explored, remote bool) {
	...
	for i := 0; i < numDirs; i++ {
		...
		if explored {
			aggregateHealth = health + skymodules.RepairThreshold/2
			aggregateRemoteHealth = remoteHealth + skymodules.RepairThreshold/2
		}
...
func TestDirectoryHeap(t *testing.T) {
	...
	if rt.renter.staticDirectoryHeap.managedLen() != 0 {
		t.Fatal("directory heap should have length of 0 but has length of", rt.renter.staticDirectoryHeap.managedLen())
	}
	...
	if rt.renter.staticDirectoryHeap.managedLen() != 4 {
		t.Fatalf("heap should have length of %v but was %v", 4, rt.renter.staticDirectoryHeap.managedLen())
	}

	// Check that the heapHealth is remote
	_, remote := rt.renter.staticDirectoryHeap.managedPeekHealth()
	if !remote {
		t.Error("Heap should have a remote health at the top")
	}

	// Pop the directories and validate their position
	d1 := rt.renter.staticDirectoryHeap.managedPop()
	d2 := rt.renter.staticDirectoryHeap.managedPop()
	...
	d3 := rt.renter.staticDirectoryHeap.managedPop()
	d4 := rt.renter.staticDirectoryHeap.managedPop()
	...
	rt.renter.staticDirectoryHeap.managedPush(d1)
	rt.renter.staticDirectoryHeap.managedPush(d2)
	rt.renter.staticDirectoryHeap.managedPush(d3)
	rt.renter.staticDirectoryHeap.managedPush(d4)
	...
	rt.renter.staticDirectoryHeap.managedPush(d)
	...
	rt.renter.staticDirectoryHeap.managedReset()

	// Confirm that the heap is empty
	if rt.renter.staticDirectoryHeap.managedLen() != 0 {
		t.Fatal("heap should empty but has length of", rt.renter.staticDirectoryHeap.managedLen())
	}
	...
	if rt.renter.staticDirectoryHeap.managedLen() != 1 {
		t.Fatal("directory heap should have length of 1 but has length of", rt.renter.staticDirectoryHeap.managedLen())
	}
	d = rt.renter.staticDirectoryHeap.managedPop()
	if d.explored {
	...
	randomSP, err := skymodules.RootSiaPath().Join(skymodules.RandomSiaPath().String())
	if err != nil {
		t.Fatal(err)
	}
...
func TestPushSubDirectories(t *testing.T) {
	...
	siaPath1, err := skymodules.NewSiaPath("SubDir1")
	if err != nil {
		t.Fatal(err)
	}
	siaPath2, err := skymodules.NewSiaPath("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(siaPath1, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(siaPath2, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	...
	d := &directory{
		staticSiaPath: skymodules.RootSiaPath(),
	}
	...
	if rt.renter.staticDirectoryHeap.managedLen() != 5 {
		t.Fatal("Heap should have length of 5 but was", rt.renter.staticDirectoryHeap.managedLen())
	}
...
func TestNextExploredDirectory(t *testing.T) {
	...
	// Create Directories
	if err := rt.renter.CreateDir(siaPath1_1, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(siaPath1_2, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(siaPath2_1, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(siaPath2_2, skymodules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}

	// Update metadata
	err = rt.renter.updateSiaDirHealth(skymodules.RootSiaPath(), float64(0), float64(3))
	if err != nil {
		t.Fatal(err)
	}
