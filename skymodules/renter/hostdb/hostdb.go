...
type HostDB struct {
	...
	scanList                []skymodules.HostDBEntry
...
var _ skymodules.HostDB = (*HostDB)(nil)

// insert inserts the HostDBEntry into both hosttrees
func (hdb *HostDB) insert(host skymodules.HostDBEntry) error {
	err := hdb.staticHostTree.Insert(host)
	_, ok := hdb.filteredHosts[host.PublicKey.String()]
	isWhitelist := hdb.filterMode == skymodules.HostDBActiveWhitelist
	if isWhitelist == ok {
...
func (hdb *HostDB) modify(host skymodules.HostDBEntry) error {
...
// contracts.
func (hdb *HostDB) updateContracts(contracts []skymodules.RenterContract) {
...
func hostdbBlockingStartup(g skymodules.Gateway, cs skymodules.ConsensusSet, tpool skymodules.TransactionPool, siamux *siamux.SiaMux, persistDir string, deps skymodules.Dependencies) (*HostDB, error) {
	...
	hdb.allowance = skymodules.DefaultAllowance
...
func (hdb *HostDB) ActiveHosts() (activeHosts []skymodules.HostDBEntry, err error) {
	if err = hdb.tg.Add(); err != nil {
...
func (hdb *HostDB) AllHosts() (allHosts []skymodules.HostDBEntry, err error) {
	if err := hdb.tg.Add(); err != nil {
		return allHosts, err
	}
	defer hdb.tg.Done()
...
func (hdb *HostDB) CheckForIPViolations(hosts []types.SiaPublicKey) ([]types.SiaPublicKey, error) {
	...
	var entries []skymodules.HostDBEntry
...
func (hdb *HostDB) Host(spk types.SiaPublicKey) (skymodules.HostDBEntry, bool, error) {
	if err := hdb.tg.Add(); err != nil {
		return skymodules.HostDBEntry{}, false, errors.AddContext(err, "error adding hostdb threadgroup:")
	}
	defer hdb.tg.Done()

	hdb.mu.Lock()
	whitelist := hdb.filterMode == skymodules.HostDBActiveWhitelist
...
func (hdb *HostDB) Filter() (skymodules.FilterMode, map[string]types.SiaPublicKey, error) {
	if err := hdb.tg.Add(); err != nil {
		return skymodules.HostDBFilterError, nil, errors.AddContext(err, "error adding hostdb threadgroup:")
	}
	defer hdb.tg.Done()
...
func (hdb *HostDB) SetFilterMode(fm skymodules.FilterMode, hosts []types.SiaPublicKey) error {
	if err := hdb.tg.Add(); err != nil {
...
func (hdb *HostDB) SetAllowance(allowance skymodules.Allowance) error {
	if err := hdb.tg.Add(); err != nil {
		return errors.AddContext(err, "error adding hostdb threadgroup:")
	}
	defer hdb.tg.Done()
...
// contracts.
func (hdb *HostDB) UpdateContracts(contracts []skymodules.RenterContract) error {
	if err := hdb.tg.Add(); err != nil {
		return errors.AddContext(err, "error adding hostdb threadgroup:")
	}
	defer hdb.tg.Done()
