...
type (
	...
	WeightFunc func(skymodules.HostDBEntry) ScoreBreakdown
...
func (ht *HostTree) All() []skymodules.HostDBEntry {
	ht.mu.Lock()
	defer ht.mu.Unlock()
	return ht.all()
}
...
func (ht *HostTree) Insert(hdbe skymodules.HostDBEntry) error {
	ht.mu.Lock()
	defer ht.mu.Unlock()
...
func (ht *HostTree) Modify(hdbe skymodules.HostDBEntry) error {
	ht.mu.Lock()
	defer ht.mu.Unlock()
...
func (ht *HostTree) Select(spk types.SiaPublicKey) (skymodules.HostDBEntry, bool) {
	ht.mu.Lock()
	defer ht.mu.Unlock()

	node, exists := ht.hosts[spk.String()]
	if !exists {
		return skymodules.HostDBEntry{}, false
	}
...
func (ht *HostTree) all() []skymodules.HostDBEntry {
	...
	entries := make([]skymodules.HostDBEntry, 0, len(he))
