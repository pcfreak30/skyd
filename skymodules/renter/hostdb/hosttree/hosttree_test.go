...
func (sb customScoreBreakdown) HostScoreBreakdown(_ types.Currency, _, _, _ bool) skymodules.HostScoreBreakdown {
	return skymodules.HostScoreBreakdown{}
}
...
func TestHostTreeParallel(t *testing.T) {
	...
	for i := 0; i < nthreads; i++ {
		go func() {
			...
			inserted := make(map[string]skymodules.HostDBEntry)
			randEntry := func() *skymodules.HostDBEntry {
...
func TestHostTreeModify(t *testing.T) {
	...
	err := tree.Modify(skymodules.HostDBEntry{})
