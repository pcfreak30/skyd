...
func (hdb *HostDB) basePriceAdjustments(entry skymodules.HostDBEntry) float64 {
...
func (hdb *HostDB) collateralAdjustments(entry skymodules.HostDBEntry, allowance skymodules.Allowance) float64 {
...
// again.
func (hdb *HostDB) acceptContractAdjustments(entry skymodules.HostDBEntry) float64 {
...
// not.
func (hdb *HostDB) durationAdjustments(entry skymodules.HostDBEntry, allowance skymodules.Allowance) float64 {
...
func (hdb *HostDB) interactionAdjustments(entry skymodules.HostDBEntry) float64 {
...
func (hdb *HostDB) priceAdjustments(entry skymodules.HostDBEntry, allowance skymodules.Allowance, txnFees types.Currency) float64 {
	...
	contractExpectedDownloadRPCs := contractExpectedDownload.Div64(skymodules.StreamDownloadSize)
	extraDownloadRPCCost := contractExpectedDownloadRPCs.Mul(extraCostsPerRPC)

	contractExpectedUploadRPCs := contractExpectedUpload.Div64(skymodules.StreamUploadSize)
	...
	// into a contract.
	//
	contractTxnFees := txnFees.Mul64(skymodules.EstimatedFileContractTransactionSetSize)
	_, _, hostCollateral, err := skymodules.RenterPayoutsPreTax(entry, contractExpectedFunds, contractTxnFees, types.ZeroCurrency, types.ZeroCurrency, allowance.Period, contractExpectedStorage)
	if err != nil {
...
func (hdb *HostDB) storageRemainingAdjustments(entry skymodules.HostDBEntry, allowance skymodules.Allowance) float64 {
...
func versionAdjustments(entry skymodules.HostDBEntry) float64 {
	base := float64(1)
	...
	if build.VersionCmp(entry.Version, "1.5.6") < 0 {
		...
		// When no longer the current version give 10% penalty for <v1.5.6 hosts
		// not having the read registry by subscription id instruction.
	}
...
func (hdb *HostDB) lifetimeAdjustments(entry skymodules.HostDBEntry) float64 {
	base := float64(1)
...
func (hdb *HostDB) uptimeAdjustments(entry skymodules.HostDBEntry) float64 {
...
func (hdb *HostDB) managedCalculateHostWeightFn(allowance skymodules.Allowance) hosttree.WeightFunc {
	// Get the txnFees.
	hdb.mu.RLock()
	txnFees := hdb.txnFees
	hdb.mu.RUnlock()
	// Create the weight function.
	return func(entry skymodules.HostDBEntry) hosttree.ScoreBreakdown {
...
func (hdb *HostDB) EstimateHostScore(entry skymodules.HostDBEntry, allowance skymodules.Allowance) (skymodules.HostScoreBreakdown, error) {
	if err := hdb.tg.Add(); err != nil {
		return skymodules.HostScoreBreakdown{}, err
	}
	defer hdb.tg.Done()
...
func (hdb *HostDB) ScoreBreakdown(entry skymodules.HostDBEntry) (skymodules.HostScoreBreakdown, error) {
	if err := hdb.tg.Add(); err != nil {
		return skymodules.HostScoreBreakdown{}, err
	}
	defer hdb.tg.Done()
...
func (hdb *HostDB) managedEstimatedScoreBreakdown(entry skymodules.HostDBEntry, allowance skymodules.Allowance, ignoreAge, ignoreDuration, ignoreUptime bool) (skymodules.HostScoreBreakdown, error) {
