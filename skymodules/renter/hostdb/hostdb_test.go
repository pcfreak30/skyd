...
func makeHostDBEntry() skymodules.HostDBEntry {
	dbe := DefaultHostDBEntry
	_, pk := crypto.GenerateKeyPair()

	dbe.PublicKey = types.Ed25519PublicKey(pk)
	dbe.ScanHistory = skymodules.HostDBScans{{
		Timestamp: time.Now(),
		Success:   true,
	}}
	return dbe
...
func newHDBTesterDeps(name string, deps modules.Dependencies) (*hdbTester, error) {
	...
	hdb, errChan := NewCustomHostDB(g, cs, tp, mux, filepath.Join(testDir, skymodules.RenterDir), deps)
	if err := <-errChan; err != nil {
		return nil, err
	}
...
func TestNew(t *testing.T) {
	...
	hdbName := filepath.Join(testDir, skymodules.RenterDir)
...
func TestRandomHosts(t *testing.T) {
	...
	entries := make(map[string]skymodules.HostDBEntry)
	nEntries := int(1e3)
	...
	for i := 0; i < 25; i++ {
		...
		dupCheck := make(map[string]skymodules.HostDBEntry)
	...
	dupCheck1 := make(map[string]skymodules.HostDBEntry)
	...
	for i := 0; i < 10; i++ {
		dupCheck2 := make(map[string]skymodules.HostDBEntry)
