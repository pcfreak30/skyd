...
func TestUpdateEntry(t *testing.T) {
	...
	entry1 := skymodules.HostDBEntry{
		PublicKey: types.SiaPublicKey{
			Key: []byte{1},
		},
	}
	entry2 := skymodules.HostDBEntry{
		PublicKey: types.SiaPublicKey{
			Key: []byte{2},
		},
	}
	...
	updatedEntry.ScanHistory = append([]skymodules.HostDBScan{{}}, updatedEntry.ScanHistory...)
	...
	updatedEntry.ScanHistory = append([]skymodules.HostDBScan{{Timestamp: time.Now().Add(maxHostDowntime * -1).Add(time.Hour * -1)}}, updatedEntry.ScanHistory...)
	err = hdbt.hdb.staticHostTree.Modify(updatedEntry)
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedEntry.ScanHistory = append([]skymodules.HostDBScan{{Success: true, Timestamp: time.Now().Add(time.Hour * 24 * (maxHostDownTimeInDays + 1) * -1)}}, updatedEntry.ScanHistory...)
	err = hdbt.hdb.staticHostTree.Modify(updatedEntry)
	if err != nil {
		t.Fatal(err)
	}
