...
func testAccountCompatV150Basic(t *testing.T, rt *renterTester) {
	...
	testdir := build.TempDir("renter", t.Name())
	renterDir := filepath.Join(testdir, skymodules.RenterDir)
...
func testAccountCompatV150_TmpFileExistsWithClean(t *testing.T, rt *renterTester, clean bool) {
	...
	// open the tmp file
	tmpFile, err := os.OpenFile(dst, os.O_RDWR, skymodules.DefaultFilePerm)
	if err != nil {
		t.Fatal(err)
	}
...
func corruptAccountsFile(path string) (err error) {
	// open the file
	var file *os.File
	file, err = os.OpenFile(path, os.O_RDWR, skymodules.DefaultFilePerm)
	if err != nil {
		return err
	}
