...
func nodeSiaPath(rootPath string, n *node) (sp skymodules.SiaPath) {
...
// remains the same.
func (fs *FileSystem) AddSiaFileFromReader(rs io.ReadSeeker, siaPath skymodules.SiaPath) (err error) {
	// Load the file.
...
func (fs *FileSystem) CachedFileInfo(siaPath skymodules.SiaPath) (skymodules.FileInfo, error) {
	return fs.managedFileInfo(siaPath, true, nil, nil, nil)
}

// CachedList lists the files and directories within a SiaDir.
func (fs *FileSystem) CachedList(siaPath skymodules.SiaPath, recursive bool, flf skymodules.FileListFunc, dlf skymodules.DirListFunc) error {
...
func (fs *FileSystem) CachedListOnNode(d *DirNode) (fis []skymodules.FileInfo, dis []skymodules.DirectoryInfo, err error) {
...
func (fs *FileSystem) DeleteDir(siaPath skymodules.SiaPath) error {
...
// file of the same path can be created and the existing file can't be opened
// until all instances of it are closed.
func (fs *FileSystem) DeleteFile(siaPath skymodules.SiaPath) error {
	return fs.managedDeleteFile(siaPath.String())
}

// DirInfo returns the Directory Information of the siadir
func (fs *FileSystem) DirInfo(siaPath skymodules.SiaPath) (_ skymodules.DirectoryInfo, err error) {
	dir, err := fs.managedOpenDir(siaPath.String())
	if err != nil {
		return skymodules.DirectoryInfo{}, nil
	}
	defer func() {
		err = errors.Compose(err, dir.Close())
	}()
...
func (fs *FileSystem) DirNodeInfo(n *DirNode) (skymodules.DirectoryInfo, error) {
	sp := fs.DirSiaPath(n)
	return n.managedInfo(sp)
}

// FileInfo returns the File Information of the siafile
func (fs *FileSystem) FileInfo(siaPath skymodules.SiaPath, offline map[string]bool, goodForRenew map[string]bool, contracts map[string]skymodules.RenterContract) (skymodules.FileInfo, error) {
...
func (fs *FileSystem) FileNodeInfo(n *FileNode) (skymodules.FileInfo, error) {
	sp := fs.FileSiaPath(n)
	return n.staticCachedInfo(sp)
}

// List lists the files and directories within a SiaDir.
func (fs *FileSystem) List(siaPath skymodules.SiaPath, recursive bool, offlineMap, goodForRenewMap map[string]bool, contractsMap map[string]skymodules.RenterContract, flf skymodules.FileListFunc, dlf skymodules.DirListFunc) error {
...
// in the renter.
func (fs *FileSystem) FileExists(siaPath skymodules.SiaPath) (bool, error) {
	path := fs.FilePath(siaPath)
	_, err := os.Stat(path)
...
func (fs *FileSystem) FilePath(siaPath skymodules.SiaPath) string {
	return siaPath.SiaFileSysPath(fs.managedAbsPath())
}

// NewSiaDir creates the folder for the specified siaPath.
func (fs *FileSystem) NewSiaDir(siaPath skymodules.SiaPath, mode os.FileMode) error {
...
func (fs *FileSystem) ReadDir(siaPath skymodules.SiaPath) ([]os.FileInfo, error) {
	// Open dir.
...
// the renter.
func (fs *FileSystem) DirExists(siaPath skymodules.SiaPath) (bool, error) {
...
func (fs *FileSystem) DirPath(siaPath skymodules.SiaPath) string {
	return siaPath.SiaDirSysPath(fs.managedAbsPath())
}

// Root returns the root system path of the FileSystem.
func (fs *FileSystem) Root() string {
	return fs.DirPath(skymodules.RootSiaPath())
}

// FileSiaPath returns the SiaPath of a file node.
func (fs *FileSystem) FileSiaPath(n *FileNode) (sp skymodules.SiaPath) {
	return fs.managedSiaPath(&n.node)
}

// DirSiaPath returns the SiaPath of a dir node.
func (fs *FileSystem) DirSiaPath(n *DirNode) (sp skymodules.SiaPath) {
	return fs.managedSiaPath(&n.node)
}

// UpdateDirMetadata updates the metadata of a SiaDir.
func (fs *FileSystem) UpdateDirMetadata(siaPath skymodules.SiaPath, metadata siadir.Metadata) (err error) {
...
func (fs *FileSystem) managedSiaPath(n *node) skymodules.SiaPath {
	return nodeSiaPath(fs.managedAbsPath(), n)
}

// Stat is a wrapper for os.Stat which takes a SiaPath as an argument instead of
// a system path.
func (fs *FileSystem) Stat(siaPath skymodules.SiaPath) (os.FileInfo, error) {
...
func (fs *FileSystem) Walk(siaPath skymodules.SiaPath, walkFn filepath.WalkFunc) error {
	dirPath := siaPath.SiaDirSysPath(fs.managedAbsPath())
...
func (fs *FileSystem) WriteFile(siaPath skymodules.SiaPath, data []byte, perm os.FileMode) error {
...
func (fs *FileSystem) NewSiaFileFromLegacyData(fd siafile.FileData) (_ *FileNode, err error) {
	// Get file's SiaPath.
	sp, err := skymodules.UserFolder.Join(fd.Name)
	if err != nil {
		return nil, err
	}
	// Get siapath of dir.
...
// filesystem tree.
func (fs *FileSystem) OpenSiaDir(siaPath skymodules.SiaPath) (*DirNode, error) {
...
// exist.
func (fs *FileSystem) OpenSiaDirCustom(siaPath skymodules.SiaPath, create bool) (*DirNode, error) {
	dn, err := fs.managedOpenSiaDir(siaPath)
	if create && errors.Contains(err, ErrNotExist) {
		// If siadir doesn't exist create one
		err = fs.NewSiaDir(siaPath, skymodules.DefaultDirPerm)
		if err != nil && !errors.Contains(err, ErrExists) {
			return nil, err
		}
...
// filesystem tree.
func (fs *FileSystem) OpenSiaFile(siaPath skymodules.SiaPath) (*FileNode, error) {
...
func (fs *FileSystem) RenameFile(oldSiaPath, newSiaPath skymodules.SiaPath) (err error) {
...
func (fs *FileSystem) RenameDir(oldSiaPath, newSiaPath skymodules.SiaPath) error {
...
func (fs *FileSystem) managedFileInfo(siaPath skymodules.SiaPath, cached bool, offline map[string]bool, goodForRenew map[string]bool, contracts map[string]skymodules.RenterContract) (_ skymodules.FileInfo, err error) {
	// Open the file.
...
func (fs *FileSystem) managedList(siaPath skymodules.SiaPath, recursive, cached bool, offlineMap map[string]bool, goodForRenewMap map[string]bool, contractsMap map[string]skymodules.RenterContract, flf skymodules.FileListFunc, dlf skymodules.DirListFunc) (err error) {
	// Open the folder.
	dir, err := fs.managedOpenDir(siaPath.String())
	if err != nil {
...
func (fs *FileSystem) managedNewSiaDir(siaPath skymodules.SiaPath, mode os.FileMode) (err error) {
...
// filesystem tree.
func (fs *FileSystem) managedOpenSiaDir(siaPath skymodules.SiaPath) (*DirNode, error) {
	if siaPath.IsRoot() {
		// Make sure the metadata exists.
		_, err := os.Stat(filepath.Join(fs.absPath(), skymodules.SiaDirExtension))
		if os.IsNotExist(err) {
