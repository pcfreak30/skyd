...
func (fs *FileSystem) CachedListCollect(siaPath skymodules.SiaPath, recursive bool) (fis []skymodules.FileInfo, dis []skymodules.DirectoryInfo, err error) {
...
func newTestFileSystemWithDir(name string) (*DirNode, *FileSystem, error) {
	...
	fs := newTestFileSystem(dir)
	sp := skymodules.RandomSiaPath()
	if err := fs.NewSiaDir(sp, skymodules.DefaultDirPerm); err != nil {
...
func newSiaPath(path string) skymodules.SiaPath {
...
// FileSystem.
func (fs *FileSystem) addTestSiaFile(siaPath skymodules.SiaPath) {
...
// a FileSystem.
func (fs *FileSystem) addTestSiaFileWithErr(siaPath skymodules.SiaPath) error {
	ec, err := skymodules.NewRSSubCode(10, 20, crypto.SegmentSize)
	if err != nil {
		return err
	}
...
func TestNewSiaFile(t *testing.T) {
	...
	fs.addTestSiaFile(sp)
	if err := fs.NewSiaDir(sp, skymodules.DefaultDirPerm); !errors.Contains(err, ErrExists) {
	...
	if _, err := os.Stat(filepath.Join(root, sp.String()+skymodules.SiaFileExtension)); err != nil {
		t.Fatal(err)
	}
...
func TestOpenSiaDir(t *testing.T) {
	...
	rootSD, err := fs.OpenSiaDir(skymodules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenSiaFile(t *testing.T) {
	...
	if *sf.path != filepath.Join(root, (*sf.name)+skymodules.SiaFileExtension) {
...
func TestSiaDirRename(t *testing.T) {
	...
	for _, dir := range dirStructure {
		sp, err := skymodules.NewSiaPath(dir)
		if err != nil {
			t.Fatal(err)
		}
	...
	// Rename dir1 to dir2.
	oldPath, err1 := skymodules.NewSiaPath(dirStructure[0])
	newPath, err2 := skymodules.NewSiaPath("dir2")
	...
	// new ones.
	for _, dir := range dirStructure {
		oldDir, err1 := skymodules.NewSiaPath(dir)
...
func TestAddSiaFileFromReader(t *testing.T) {
	...
	numSiaFiles := 0
	err = sfs.Walk(skymodules.RootSiaPath(), func(path string, info os.FileInfo, err error) error {
		if filepath.Ext(path) == skymodules.SiaFileExtension {
			numSiaFiles++
		}
		return nil
	})
	...
	reader = bytes.NewReader(b)
	var newSFSiaPath skymodules.SiaPath
	...
	newSFPath := filepath.Join(filepath.Dir(sf.SiaFilePath()), newSFSiaPath.String()+"_1"+skymodules.SiaFileExtension)
	...
	if !strings.HasSuffix(newSF.SiaFilePath(), "_1"+skymodules.SiaFileExtension) {
...
func TestSiaFileSetDeleteOpen(t *testing.T) {
	...
	rc, _ := skymodules.NewRSSubCode(10, 20, crypto.SegmentSize)
	fileSize := uint64(100)
	source := ""
...
func TestDeleteCorruptSiaFile(t *testing.T) {
	...
	siaPath, err := skymodules.NewSiaPath("badFile")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSiaDirDelete(t *testing.T) {
	...
	time.Sleep(time.Second)
	// Delete dir1.
	sp, err := skymodules.NewSiaPath("dir1")
	if err != nil {
		t.Fatal(err)
	}
	...
	files, err := fs.ReadDir(skymodules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	if len(files) != 1 {
...
func TestDeleteDirInMemory(t *testing.T) {
	...
	// Create a new dir at the same path.
	err = fs.NewSiaDir(dirPath, skymodules.DefaultDirPerm)
	if err != nil {
		t.Fatal(err)
	}
	// Get the entry.
...
func TestOpenCloseRoot(t *testing.T) {
	...
	rootNode, err := fs.OpenSiaDir(skymodules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	err = rootNode.Close()
...
func testFileDirConflict(t *testing.T, open bool) {
	...
	err = fs.NewSiaDir(filepath, skymodules.DefaultDirPerm)
	if !errors.Contains(err, ErrExists) {
		t.Fatalf("Expected err %v, got %v", ErrExists, err)
	}
	...
	err = fs.NewSiaDir(dirpath2, skymodules.DefaultDirPerm)
	if err != nil {
		t.Fatal(err)
	}
