...
func newTestDir(rootDir string) (*SiaDir, error) {
	...
	wal, _ := newTestWAL()
	return New(skymodules.RandomSiaPath().SiaDirSysPath(rootPath), rootPath, skymodules.DefaultDirPerm, wal)
}
...
func TestCreateReadMetadataUpdate(t *testing.T) {
	...
	path := filepath.Join(sd.path, skymodules.SiaDirExtension)
	...
	// Check path
	path2 := filepath.Join(sd.path, skymodules.SiaDirExtension)
	if path != path2 {
...
func testApply(t *testing.T, siadir *SiaDir, apply func(...writeaheadlog.Update) error) {
	...
	metadata.Health = 1.0
	path := filepath.Join(siadir.path, skymodules.SiaDirExtension)
