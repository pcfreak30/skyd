...
func TestNewSiaDir(t *testing.T) {
	...
	_, err = os.Stat(filepath.Join(path, skymodules.SiaDirExtension))
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = os.Stat(filepath.Join(topDir, skymodules.SiaDirExtension))
	if err != nil {
		t.Fatal(err)
	}
	// Get SiaDir
	...
	_, err = os.Stat(filepath.Join(testDir, skymodules.SiaDirExtension))
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestUpdateMetadata(t *testing.T) {
	...
	siaPath, err := skymodules.NewSiaPath("TestDir")
	if err != nil {
		t.Fatal(err)
	}
	siaDirSysPath := siaPath.SiaDirSysPath(rootDir)
	wal, _ := newTestWAL()
	siaDir, err := New(siaDirSysPath, rootDir, skymodules.DefaultDirPerm, wal)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSiaDirDelete(t *testing.T) {
	...
	siaPath, err := skymodules.NewSiaPath("deleteddir")
	if err != nil {
		t.Fatal(err)
	}
	siaDirSysPath := siaPath.SiaDirSysPath(rootDir)
