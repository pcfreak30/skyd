...
func (n *FileNode) managedFileInfo(siaPath skymodules.SiaPath, offline map[string]bool, goodForRenew map[string]bool, contracts map[string]skymodules.RenterContract) (skymodules.FileInfo, error) {
	...
	if err != nil {
		return skymodules.FileInfo{}, errors.AddContext(err, "failed to get n redundancy")
	}
	uploadProgress, uploadedBytes, err := n.UploadProgressAndBytes()
	if err != nil {
		return skymodules.FileInfo{}, errors.AddContext(err, "failed to get upload progress and bytes")
	}
...
func (n *FileNode) managedRename(newName string, oldParent, newParent *DirNode) error {
	...
	newPath := filepath.Join(newParent.absPath(), newName) + skymodules.SiaFileExtension
	// Rename the file.
