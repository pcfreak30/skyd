...
func customTestFileAndWAL(siaFilePath, source string, rc skymodules.ErasureCoder, sk crypto.CipherKey, fileSize uint64, numChunks int, fileMode os.FileMode) (*SiaFile, *writeaheadlog.WAL, string) {
...
func newTestFileParams(minChunks int, partialChunk bool) (string, skymodules.SiaPath, string, skymodules.ErasureCoder, crypto.CipherKey, uint64, int, os.FileMode) {
	rc, err := skymodules.NewRSCode(10, 20)
	if err != nil {
		panic(err)
	}
...
func TestRename(t *testing.T) {
	...
	newSiaFilePath := strings.TrimSuffix(entry.SiaFilePath(), skymodules.SiaFileExtension) + "_renamed" + skymodules.SiaFileExtension
