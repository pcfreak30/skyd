...
func TestProjectDownloadChunk_initialWorkerHeap(t *testing.T) {
	...
	// manually manipulate the resolve time of worker 1 to be 800ms in the past,
	// we expect the complete time to be 1800ms in the future as the worker's
	...
	expectedCompleteTime := time.Until(unresolvedWorkers[0].staticExpectedResolvedTime) + time.Hour + 200*time.Millisecond
...
func TestProjectDownloadChunk_createInitialWorkerSet(t *testing.T) {
	...
	// algorithm.
	ec, err := skymodules.NewRSSubCode(3, 12, crypto.SegmentSize)
	if err != nil {
		t.Fatal(err)
	}
