...
func (pdc *projectDownloadChunk) printDistributions() {
	...
	for _, lwi := range pdc.launchedWorkers {
		...
		log.Printf("HS distribution: \np50: %v\np75: %v\np90: %v\np99: %v\np999: %v\n", hsPercentiles[4], hsPercentiles[5], hsPercentiles[0], hsPercentiles[1], hsPercentiles[2])
		log.Printf("RJ distribution: \np50: %v\np75: %v\np90: %v\np99: %v\np999: %v\n", rjPercentiles[4], rjPercentiles[5], rjPercentiles[0], rjPercentiles[1], rjPercentiles[2])
	}
}
