...
func readDataPieces(r io.Reader, ec skymodules.ErasureCoder, pieceSize uint64) ([][]byte, uint64, error) {
...
func (r *Renter) managedDownloadLogicalChunkDataFromSkynet(chunk *unfinishedUploadChunk, downloadLength uint64) ([][]byte, error) {
	...
	onlyOnePieceNeeded := ec.MinPieces() == 1 && mk.Type() == crypto.TypePlain
...
func (r *Renter) managedUpdateUploadChunkStuckStatus(uc *unfinishedUploadChunk) {
	...
	successfulRepair := !skymodules.NeedsRepair(health)
