...
type file struct {
	...
	erasureCode skymodules.ErasureCoder  // Static - can be accessed without lock.
...
func (f *file) UnmarshalSia(r io.Reader) error {
	...
	case "Reed-Solomon":
		...
		rsc, err := skymodules.NewRSCode(int(nData), int(nParity))
		if err != nil {
			return err
		}
		f.erasureCode = rsc
...
// them into memory.
func (r *Renter) compatV137ConvertSiaFiles(tracking map[string]v137TrackedFile, oldContracts []skymodules.RenterContract) error {
	...
	for _, fi := range fis {
		...
		if fi.Name() == skymodules.FileSystemRoot || fi.Name() == "contracts" {
			continue
		}
		// Delete the folder.
...
// files.
func (r *Renter) compatV137loadSiaFilesFromReader(reader io.Reader, tracking map[string]v137TrackedFile, oldContracts []skymodules.RenterContract) ([]string, error) {
	// read header
	var header [15]byte
	var version string
	var numFiles uint64
	...
	for i := range files {
		...
		for {
			siaPath, err := skymodules.NewSiaPath(files[i].name)
			if err != nil {
				return nil, err
			}
...
func (r *Renter) convertPersistVersionFrom140To142(path string) error {
	...
	newHomePath := skymodules.HomeFolder.SiaDirSysPath(fsRoot)
	newSiaFilesPath := skymodules.UserFolder.SiaDirSysPath(fsRoot)
	newSnapshotsPath := skymodules.BackupFolder.SiaDirSysPath(fsRoot)
...
func (r *Renter) convertPersistVersionFrom133To140(path string, oldContracts []skymodules.RenterContract) error {
	metadata := persist.Metadata{
		Header:  settingsMetadata.Header,
