...
func (nh *notificationHandler) managedHandleNotification(stream siamux.Stream, budget *modules.RPCBudget, limit *modules.BudgetLimit) {
	...
	defer func() {
		if err := stream.Close(); err != nil {
			w.renter.staticLog.Print("managedHandleNotification: failed to close stream: ", err)
		}
	}()
	...
	if err != nil {
		w.renter.staticLog.Print("managedHandleNotification: failed to set deadlien on stream: ", err)
		return
	}
	...
	if err != nil {
		w.renter.staticLog.Print("managedHandleNotification: failed to read notification type: ", err)
		return
	}
	...
	case modules.SubscriptionResponseSubscriptionSuccess:
		if err := nh.managedHandleSubscriptionSuccess(stream, limit); err != nil {
			w.renter.staticLog.Print("managedHAndleSubscriptionSuccess:", err)
		}
		return
	case modules.SubscriptionResponseRegistryValue:
		if err := nh.managedHandleRegistryEntry(stream, budget, limit); err != nil {
			w.renter.staticLog.Print("managedHandleRegistryEntry:", err)
		}
		return
	default:
	}
	...
	w.renter.staticLog.Print("managedHandleNotification: unknown notification type")
	if err := nh.staticStream.Close(); err != nil {
		w.renter.staticLog.Debugln("managedHandleNotification: failed to close subscription:", err)
	}
}
...
func (w *worker) managedPriceTableForSubscription(duration time.Duration) *modules.RPCPriceTable {
	for {
		...
		w.renter.staticLog.Printf("managedPriceTableForSubscription: pt not ready yet for worker %v", w.staticHostPubKeyStr)
...
func (w *worker) threadedSubscriptionLoop() {
	...
	for {
		...
		if err != nil {
			...
			w.renter.staticLog.Printf("Worker %v: failed to begin subscription: %v", w.staticHostPubKeyStr, err)
		...
		if err != nil {
			w.renter.staticLog.Printf("Worker %v: subscription got interrupted: %v", w.staticHostPubKeyStr, errSubscription)
			subInfo.managedIncrementCooldown()
			continue
		}
	}
