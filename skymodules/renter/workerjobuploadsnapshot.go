...
func (j *jobUploadSnapshot) callWeight() uint64 {
	return uploadSnapshotQueueWeight
}

// staticMinHostVersion returns the minimum host version needed to execute the
// job.
func (j *jobUploadSnapshot) staticMinHostVersion() string {
...
func (j *jobUploadSnapshot) callExecute() {
	...
	meta, ok := j.staticMetadata.(skymodules.UploadedBackup)
	if !ok {
...
func (w *worker) UploadSnapshot(ctx context.Context, meta skymodules.UploadedBackup, dotSia []byte) error {
