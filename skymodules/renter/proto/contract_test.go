...
func TestContractUncommittedTxn(t *testing.T) {
	...
	t.Run("RecordPaymentIntent", func(t *testing.T) {
		updateFunc := func(sc *SafeContract) (*unappliedWalTxn, []crypto.Hash, contractHeader, error) {
			...
			txn, err := sc.RecordPaymentIntent(fcr, amount, skymodules.SpendingDetails{
...
func TestContractRecordAndCommitPaymentIntent(t *testing.T) {
	...
	processTxnWithRevision := func(rev types.FileContractRevision, amount types.Currency, details skymodules.SpendingDetails, commit bool) {
	...
	processTxnWithRevision(rev, amount, skymodules.SpendingDetails{
		FundAccountSpending: amount,
		MaintenanceSpending: skymodules.MaintenanceSpending{FundAccountCost: rpcCost},
	}, true)
	...
	processTxnWithRevision(rev, amount, skymodules.SpendingDetails{}, true)
	...
	processTxnWithRevision(rev, amount, skymodules.SpendingDetails{
		MaintenanceSpending: skymodules.MaintenanceSpending{UpdatePriceTableCost: rpcCost},
	}, true)
	...
	processTxnWithRevision(rev, amount, skymodules.SpendingDetails{
		MaintenanceSpending: skymodules.MaintenanceSpending{AccountBalanceCost: rpcCost},
	}, false)
