...
func (hd *Downloader) Download(root crypto.Hash, offset, length uint32) (_ skymodules.RenterContract, _ []byte, err error) {
	...
	if hd.deps.Disrupt("InterruptDownloadBeforeSendingRevision") {
		return skymodules.RenterContract{}, nil,
...
// Downloader.
func (cs *ContractSet) NewDownloader(host skymodules.HostDBEntry, id types.FileContractID, currentHeight types.BlockHeight, hdb hostDB, cancel <-chan struct{}) (_ *Downloader, err error) {
	sc, ok := cs.Acquire(id)
	if !ok {
