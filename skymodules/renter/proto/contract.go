...
func (c *SafeContract) metadata() skymodules.RenterContract {
	h := c.header
...
func (c *SafeContract) RecordPaymentIntent(rev types.FileContractRevision, amount types.Currency, details skymodules.SpendingDetails) (*unappliedWalTxn, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *SafeContract) Utility() skymodules.ContractUtility {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// using the set's WAL.
func (cs *ContractSet) managedInsertContract(h contractHeader, roots []crypto.Hash) (skymodules.RenterContract, error) {
...
func (cs *ContractSet) managedApplyInsertContractUpdate(update writeaheadlog.Update) (skymodules.RenterContract, error) {
	// Sanity check update.
	if update.Name != updateNameInsertContract {
		return skymodules.RenterContract{}, fmt.Errorf("can't call managedApplyInsertContractUpdate on update of type '%v'", update.Name)
	}
	// Decode update.
	...
	// create the files.
	headerFile, err := os.OpenFile(headerFilePath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, skymodules.DefaultFilePerm)
	if err != nil {
		return skymodules.RenterContract{}, err
	}
	rootsFile, err := os.OpenFile(rootsFilePath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, skymodules.DefaultFilePerm)
	if err != nil {
		return skymodules.RenterContract{}, err
	}
	// write header
	...
	if cs.staticDeps.Disrupt("InterruptContractInsertion") {
		return skymodules.RenterContract{}, errors.New("interrupted")
	}
	// write roots
	...
	if build.Release == "testing" {
		...
		if err != nil {
			return skymodules.RenterContract{}, errors.AddContext(err, "failed to create a refcounter")
		}
	}
	sc := &SafeContract{
		header:           h,
...
func (cs *ContractSet) loadSafeContract(headerFileName, rootsFileName, refCountFileName string, walTxns map[types.FileContractID][]*unappliedWalTxn) (err error) {
	...
	rootsFile, err := os.OpenFile(rootsFileName, os.O_RDWR, skymodules.DefaultFilePerm)
	if err != nil {
		return err
	}
	defer func() {
...
type V130CachedRevision struct {
	...
	MerkleRoots skymodules.MerkleRootSet   `json:"merkleroots"`
}
