...
func (cs *ContractSet) Renew(oldContract *SafeContract, params skymodules.ContractParams, txnBuilder transactionBuilder, tpool transactionPool, hdb hostDB, cancel <-chan struct{}) (rc skymodules.RenterContract, formationTxnSet []types.Transaction, err error) {
...
func (cs *ContractSet) managedNewRenew(oldContract *SafeContract, params skymodules.ContractParams, txnBuilder transactionBuilder, tpool transactionPool, hdb hostDB, cancel <-chan struct{}) (rc skymodules.RenterContract, formationTxnSet []types.Transaction, err error) {
	// for convenience
	...
	ourSKNew, ourPKNew := skymodules.GenerateContractKeyPair(params.RenterSeed, fcTxn)
	...
	si, hk := skymodules.PrefixedSignedIdentifier(params.RenterSeed, fcTxn, host.PublicKey)
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.New("failed to sign transaction: " + err.Error())
	}
...
// empty.
func (cs *ContractSet) managedNewRenewAndClear(oldContract *SafeContract, params skymodules.ContractParams, txnBuilder transactionBuilder, tpool transactionPool, hdb hostDB, cancel <-chan struct{}) (rc skymodules.RenterContract, formationTxnSet []types.Transaction, err error) {
	// for convenience
	contract := oldContract.header
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "Unable to create final revision")
	}
...
func rhp2BaseCosts(lastRev types.FileContractRevision, host skymodules.HostDBEntry, endHeight types.BlockHeight) (basePrice, baseCollateral types.Currency) {
...
func createRenewedContract(lastRev types.FileContractRevision, uh types.UnlockHash, params skymodules.ContractParams, txnFee, basePrice, baseCollateral types.Currency, tpool transactionPool) (types.FileContract, error) {
...
func (cs *ContractSet) RenewContract(conn net.Conn, fcid types.FileContractID, params skymodules.ContractParams, txnBuilder skymodules.TransactionBuilder, tpool skymodules.TransactionPool, hdb hostDB, pt *skymodules.RPCPriceTable) (_ skymodules.RenterContract, _ []types.Transaction, err error) {
	...
	if !ok {
		return skymodules.RenterContract{}, nil, errors.New("RenewContract: failed to acquire contract to renew")
	}
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "Unable to create new contract")
	}
	...
	txnSet, err := prepareTransactionSet(txnBuilder)
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "failed to prepare txnSet with finalRev and new contract")
	}
	...
	defer func() {
		if err != nil {
			...
			err = errors.Compose(err, skymodules.ErrHostFault)
		} else if err == nil {
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "failed to write RPCRenewContractRequest")
	}
	...
	signedTxnSet, err := txnBuilder.Sign(true)
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "failed to sign transaction set")
	}
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "failed to send RPCRenewContractRenterSignatures to host")
	}
	...
	if err != nil {
		return skymodules.RenterContract{}, nil, errors.AddContext(err, "failed to submit txnSet for renewal to blockchain")
	}
