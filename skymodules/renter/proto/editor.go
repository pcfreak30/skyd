...
func (he *Editor) Upload(data []byte) (_ skymodules.RenterContract, _ crypto.Hash, err error) {
	...
	if contract.RenterFunds().Cmp(sectorPrice) < 0 {
		return skymodules.RenterContract{}, crypto.Hash{}, errors.New("contract has insufficient funds to support upload")
	}
	...
	if err != nil {
		return skymodules.RenterContract{}, crypto.Hash{}, errors.AddContext(err, "Error creating new upload revision")
	}
	...
	if he.deps.Disrupt("InterruptUploadBeforeSendingRevision") {
		return skymodules.RenterContract{}, crypto.Hash{},
...
// an Editor.
func (cs *ContractSet) NewEditor(host skymodules.HostDBEntry, id types.FileContractID, currentHeight types.BlockHeight, hdb hostDB, cancel <-chan struct{}) (_ *Editor, err error) {
	sc, ok := cs.Acquire(id)
	if !ok {
...
func initiateRevisionLoop(host skymodules.HostDBEntry, contract *SafeContract, rpc types.Specifier, cancel <-chan struct{}, rl *ratelimit.RateLimit) (net.Conn, chan struct{}, error) {
	c, err := (&net.Dialer{
		Cancel:  cancel,
	...
	conn = ratelimit.NewRLConn(conn, skymodules.GlobalRateLimits, cancel)
