...
func (cs *ContractSet) FormContract(params skymodules.ContractParams, txnBuilder transactionBuilder, tpool transactionPool, hdb hostDB, cancel <-chan struct{}) (rc skymodules.RenterContract, formationTxnSet []types.Transaction, sweepTxn types.Transaction, sweepParents []types.Transaction, err error) {
	...
	renterPayout, hostPayout, _, err := skymodules.RenterPayoutsPreTax(host, funding, txnFee, types.ZeroCurrency, types.ZeroCurrency, period, expectedStorage)
	if err != nil {
	...
	if types.PostTax(startHeight, totalPayout).Cmp(hostPayout) < 0 {
		return skymodules.RenterContract{}, nil, types.Transaction{}, nil, errors.New("not enough money to pay both siafund fee and also host payout")
	}
	...
	// Create our key.
	ourSK, ourPK := skymodules.GenerateContractKeyPair(params.RenterSeed, fcTxn)
