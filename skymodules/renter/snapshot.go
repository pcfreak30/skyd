...
func (r *Renter) UploadedBackups() ([]skymodules.UploadedBackup, []types.SiaPublicKey, error) {
...
// of operations.
func (r *Renter) BackupsOnHost(hostKey types.SiaPublicKey) ([]skymodules.UploadedBackup, error) {
	if err := r.tg.Add(); err != nil {
		return nil, err
	}
	defer r.tg.Done()
...
func (r *Renter) managedUploadBackup(src, name string) error {
	...
	// Prepare the siapath.
	sp, err := skymodules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
	...
	ec, err := skymodules.NewRSSubCode(int(dataPieces), int(parityPieces), crypto.SegmentSize)
	if err != nil {
		return err
	}
	...
	meta := skymodules.UploadedBackup{
		Name:           name,
...
func (r *Renter) DownloadBackup(dst string, name string) (err error) {
	...
	backupSiaPath, err := skymodules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
	...
	// Load the .sia file.
	siaPath, err := skymodules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
...
func (r *Renter) managedUploadSnapshot(meta skymodules.UploadedBackup, dotSia []byte) error {
...
func (r *Renter) managedDownloadSnapshot(uid [16]byte) (ub skymodules.UploadedBackup, dotSia []byte, err error) {
	if err := r.tg.Add(); err != nil {
		return skymodules.UploadedBackup{}, nil, err
	}
	defer r.tg.Done()
	...
	return skymodules.UploadedBackup{}, nil, errors.New("could not download backup from any host")
}
...
func (r *Renter) threadedSynchronizeSnapshots() {
	...
	// has that we don't.
	calcOverlap := func(entryTable []snapshotEntry, known map[[16]byte]struct{}) (unknown []skymodules.UploadedBackup, missing [][16]byte) {
		...
		for _, e := range entryTable {
			if _, ok := known[e.UID]; !ok {
				unknown = append(unknown, skymodules.UploadedBackup{
	...
	for {
		...
		root := skymodules.BackupFolder
		var mu sync.Mutex
		flf := func(info skymodules.FileInfo) {
			...
			id = r.mu.RLock()
			var meta skymodules.UploadedBackup
			found := false
			for _, meta = range r.persist.UploadedBackups {
				sp, _ := info.SiaPath.Rebase(skymodules.BackupFolder, skymodules.RootSiaPath())
			...
			if skymodules.NeedsRepair(info.Health) {
