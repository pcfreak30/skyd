...
func (d *directory) managedHeapHealth() (float64, bool) {
	...
	if skymodules.NeedsRepair(remoteHealth) {
...
type directoryHeap struct {
	...
	// heap
	heapDirectories map[skymodules.SiaPath]*directory
...
func (dh *directoryHeap) managedReset() {
	dh.mu.Lock()
	defer dh.mu.Unlock()
	dh.heapDirectories = make(map[skymodules.SiaPath]*directory)
...
func (dh *directoryHeap) managedPushDirectory(siaPath skymodules.SiaPath, metadata siadir.Metadata, explored bool) {
	d := &directory{
...
func (r *Renter) managedPushUnexploredDirectory(siaPath skymodules.SiaPath) (err error) {
