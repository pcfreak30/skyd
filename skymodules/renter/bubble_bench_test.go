...
// on renter methods
func newBenchmarkRenterWithDependency(name string, deps skymodules.Dependencies) (*Renter, error) {
	testdir := build.TempDir("renter", name)
	rt, err := newRenterTesterNoRenter(testdir)
	if err != nil {
		return nil, err
	}
	r, err := newRenterWithDependency(rt.gateway, rt.cs, rt.wallet, rt.tpool, rt.mux, filepath.Join(testdir, skymodules.RenterDir), deps)
	if err != nil {
		return nil, err
	}
	return r, nil
...
func BenchmarkBubbleMetadata(b *testing.B) {
	...
	// Create Directory
	dirSiaPath, err := skymodules.NewSiaPath("root")
	if err != nil {
		b.Fatal(err)
	}
	err = r.CreateDir(dirSiaPath, skymodules.DefaultDirPerm)
	if err != nil {
		b.Fatal(err)
	}
