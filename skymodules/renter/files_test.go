...
func (r *Renter) createRenterTestFile(siaPath skymodules.SiaPath) (*filesystem.FileNode, error) {
...
func (r *Renter) createRenterTestFileWithParamsAndSize(siaPath skymodules.SiaPath, rsc skymodules.ErasureCoder, ct crypto.CipherType, size uint64) (*filesystem.FileNode, error) {
	...
	err := r.staticFileSystem.NewSiaFile(up.SiaPath, up.Source, up.ErasureCode, crypto.GenerateSiaKey(ct), size, persist.DefaultDiskPermissionsTest)
	if err != nil {
		return nil, err
	}
...
func TestRenterFileListLocalPath(t *testing.T) {
	...
	files, err := rt.renter.FileListCollect(skymodules.RootSiaPath(), true, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(files) != 1 {
...
func TestRenterDeleteFile(t *testing.T) {
	...
	siaPath, err := skymodules.NewSiaPath("dne")
	if err != nil {
		t.Fatal(err)
	}
	...
	siaPathOne, err := skymodules.NewSiaPath("one")
	if err != nil {
		t.Fatal(err)
	}
	...
	siaPath1, err := skymodules.NewSiaPath("1")
	if err != nil {
		t.Fatal(err)
	}
	...
	var walkStr string
	rt.renter.staticFileSystem.Walk(skymodules.RootSiaPath(), func(path string, _ os.FileInfo, _ error) error {
...
func TestRenterDeleteFileMissingParent(t *testing.T) {
	...
	// Put a file in the renter.
	siaPath, err := skymodules.NewSiaPath("parent/file")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = rt.renter.staticFileSystem.NewSiaFile(up.SiaPath, up.Source, up.ErasureCode, crypto.GenerateSiaKey(crypto.RandomCipherType()), 1000, persist.DefaultDiskPermissionsTest)
	if err != nil {
		t.Fatal(err)
	}
	// Delete the parent.
...
func TestRenterRenameFile(t *testing.T) {
	...
	siaPath1a, err := skymodules.NewSiaPath("1a")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := sp.FromSysPath(entry.SiaFilePath(), sfs.DirPath(skymodules.RootSiaPath())); err != nil {
		t.Fatal(err)
	}
	...
	var sp2 skymodules.SiaPath
	if err := sp2.FromSysPath(entry2.SiaFilePath(), sfs.DirPath(skymodules.RootSiaPath())); err != nil {
		t.Fatal(err)
	}
	...
	siaPath1b, err := skymodules.NewSiaPath("1b")
	if err != nil {
		t.Fatal(err)
	}
	...
	siaPathWithDir, err := skymodules.NewSiaPath("new/name/with/dir/test")
	if err != nil {
		t.Fatal(err)
	}
	...
	for !siaPathWithDir.Equals(skymodules.RootSiaPath()) {
...
func TestRenterFileDir(t *testing.T) {
	...
	siaPath, err := skymodules.NewSiaPath(fileName)
	if err != nil {
		t.Fatal(err)
	}
	...
	renterDir := filepath.Join(rt.dir, skymodules.RenterDir)
	siapathRootDir := filepath.Join(renterDir, skymodules.FileSystemRoot)
