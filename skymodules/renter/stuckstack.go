...
type (
	...
	stuckStack struct {
		stack    []skymodules.SiaPath
		siaPaths map[skymodules.SiaPath]struct{}
...
func callNewStuckStack() stuckStack {
	return stuckStack{
		stack:    make([]skymodules.SiaPath, 0, maxSuccessfulStuckRepairFiles),
		siaPaths: make(map[skymodules.SiaPath]struct{}),
	}
}
...
func (ss *stuckStack) managedPop() (sp skymodules.SiaPath) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
...
func (ss *stuckStack) managedPush(siaPath skymodules.SiaPath) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
