...
// mockGatewayCountBroadcasts implements skymodules.Gateway to mock the Broadcast
// method.
...
// Broadcast is a mock implementation of skymodules.Gateway.Broadcast that
// increments a counter denoting the number of times it's been called.
func (g *mockGatewayCountBroadcasts) Broadcast(name string, obj interface{}, peers []skymodules.Peer) {
	g.mu.Lock()
	g.numBroadcasts++
	g.mu.Unlock()
...
func TestSendBlocksBroadcastsOnce(t *testing.T) {
	...
	cst1, err := blankConsensusSetTester(t.Name()+"1", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	cst2, err := blankConsensusSetTester(t.Name()+"2", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestIntegrationRPCSendBlocks(t *testing.T) {
	...
	for i, tt := range tests {
		...
		remoteCST, err := blankConsensusSetTester(filepath.Join(t.Name()+" - remote", strconv.Itoa(i)), skymodules.ProdDependencies)
		if err != nil {
			t.Fatalf("test #%d, %v: %v", i, tt.msg, err)
		}
		// Create the "local" peer.
		localCST, err := blankConsensusSetTester(filepath.Join(t.Name()+" - local", strconv.Itoa(i)), skymodules.ProdDependencies)
		if err != nil {
			t.Fatalf("test #%d, %v: %v", i, tt.msg, err)
		}
...
func TestRPCSendBlockSendsOnlyNecessaryBlocks(t *testing.T) {
	...
	// Create the "remote" peer.
	cst, err := blankConsensusSetTester(t.Name()+"- remote", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	testdir := build.TempDir(skymodules.ConsensusDir, t.Name()+" - local")
	...
	cs, errChan := New(g, false, filepath.Join(testdir, skymodules.ConsensusDir))
	...
	err = cs.gateway.RPC(cst.cs.gateway.Address(), "SendBlocks", func(conn skymodules.PeerConn) error {
...
// RPCAddr implements this method of the skymodules.PeerConn interface.
func (pc mockPeerConn) RPCAddr() skymodules.NetAddress {
...
// Read is a mock implementation of skymodules.PeerConn.Read that always returns
// an error.
func (mockPeerConnFailingReader) Read([]byte) (int, error) {
	return 0, errFailingReader
}

// Write is a mock implementation of skymodules.PeerConn.Write that always returns
// an error.
...
func TestSendBlk(t *testing.T) {
	...
	cst, err := blankConsensusSetTester(t.Name(), skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestIntegrationSendBlkRPC(t *testing.T) {
	...
	if !errors.Contains(err, skymodules.ErrBlockKnown) && !errors.Contains(err, skymodules.ErrNonExtendingBlock) {
		t.Errorf("cst1 should reject known blocks: expected error '%v', got '%v'", skymodules.ErrBlockKnown, err)
	}
...
func (g *mockGatewayCallsRPC) RPC(addr skymodules.NetAddress, name string, fn skymodules.RPCFunc) error {
	g.rpcCalled <- name
	return nil
}
...
func TestRelayHeader(t *testing.T) {
	...
	tests := []struct {
		...
		// Test that rpcRelayHeader rejects known blocks.
		{
			...
			errWant: skymodules.ErrBlockKnown,
...
func TestIntegrationRelaySynchronize(t *testing.T) {
	...
	cst3, err := blankConsensusSetTester(t.Name()+"3", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
// mockPeerConnMockReadWrite is a mock implementation of skymodules.PeerConn that
...
func TestIntegrationSendBlocksStalls(t *testing.T) {
	...
	cstLocal, err := blankConsensusSetTester(t.Name()+"- local", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	cstRemote, err := blankConsensusSetTester(t.Name()+"- remote", skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
