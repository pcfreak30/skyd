...
func TestCommitDelayedSiacoinOutputDiffBadMaturity(t *testing.T) {
	...
	dscod := skymodules.DelayedSiacoinOutputDiff{
		Direction:      skymodules.DiffApply,
		ID:             id,
		SiacoinOutput:  dsco,
		MaturityHeight: maturityHeight,
	}
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		commitDelayedSiacoinOutputDiff(tx, dscod, skymodules.DiffApply)
		return nil
	})
}
...
func TestCommitNodeDiffs(t *testing.T) {
	...
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		commitDiffSet(tx, pb, skymodules.DiffRevert) // pull the block node out of the consensus set.
		return nil
	})
	...
	scod0 := skymodules.SiacoinOutputDiff{
		Direction: skymodules.DiffApply,
		ID:        scoid,
	}
	scod1 := skymodules.SiacoinOutputDiff{
		Direction: skymodules.DiffRevert,
		ID:        scoid,
	}
	fcid := types.FileContractID{'2'}
	fcd0 := skymodules.FileContractDiff{
		Direction: skymodules.DiffApply,
		ID:        fcid,
	}
	fcd1 := skymodules.FileContractDiff{
		Direction: skymodules.DiffRevert,
		ID:        fcid,
	}
	sfoid := types.SiafundOutputID{'3'}
	sfod0 := skymodules.SiafundOutputDiff{
		Direction: skymodules.DiffApply,
		ID:        sfoid,
	}
	sfod1 := skymodules.SiafundOutputDiff{
		Direction: skymodules.DiffRevert,
		ID:        sfoid,
	}
	...
	sfpd := skymodules.SiafundPoolDiff{
		Direction: skymodules.DiffApply,
	...
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		createUpcomingDelayedOutputMaps(tx, pb, skymodules.DiffApply)
		return nil
	})
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		commitNodeDiffs(tx, pb, skymodules.DiffApply)
		return nil
	})
	...
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		commitNodeDiffs(tx, pb, skymodules.DiffRevert)
		return nil
	})
	exists = cst.cs.db.inSiacoinOutputs(scoid)
	if exists {
...
func TestCommitSiacoinOutputDiff(t *testing.T) {
	...
	scod := skymodules.SiacoinOutputDiff{
		Direction:     skymodules.DiffApply,
		ID:            id,
		SiacoinOutput: sco,
	}
	cst.cs.commitSiacoinOutputDiff(scod, skymodules.DiffApply)
	...
	// Rewind the diff.
	cst.cs.commitSiacoinOutputDiff(scod, skymodules.DiffRevert)
	...
	cst.cs.commitSiacoinOutputDiff(scod, skymodules.DiffApply)
	scod.Direction = skymodules.DiffRevert
	cst.cs.commitSiacoinOutputDiff(scod, skymodules.DiffApply)
	if cst.cs.db.lenSiacoinOutputs() != initialScosLen {
		t.Error("siacoin output diff set did not increase in size")
	}
...
func TestCommitFileContractDiff(t *testing.T) {
	...
	fcd := skymodules.FileContractDiff{
		Direction:    skymodules.DiffApply,
		ID:           id,
		FileContract: fc,
	}
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffApply)
	...
	// Rewind the diff.
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffRevert)
	...
	// Restore the diff and then apply the inverse diff.
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffApply)
	fcd.Direction = skymodules.DiffRevert
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffApply)
	if cst.cs.db.lenFileContracts() != initialFcsLen {
		t.Error("siacoin output diff set did not increase in size")
	}
	...
	fcd.Direction = skymodules.DiffApply                      // Object currently exists, but make the direction 'apply'.
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffRevert) // revert the application.
	cst.cs.commitFileContractDiff(fcd, skymodules.DiffRevert) // revert the application again, in error.
}
*/
...
func TestCommitSiafundOutputDiff(t *testing.T) {
	...
	sfod := skymodules.SiafundOutputDiff{
		Direction:     skymodules.DiffApply,
		ID:            id,
		SiafundOutput: sfo,
	}
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffApply)
	...
	// Rewind the diff.
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffRevert)
	...
	// Restore the diff and then apply the inverse diff.
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffApply)
	sfod.Direction = skymodules.DiffRevert
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffApply)
	if cst.cs.db.lenSiafundOutputs() != initialScosLen {
		t.Error("siafund output diff set did not increase in size")
	}
	...
	// that was already removed)
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffApply) // Remove the object.
	cst.cs.commitSiafundOutputDiff(sfod, skymodules.DiffApply) // Remove the object again.
}
*/
...
func TestCommitDelayedSiacoinOutputDiff(t *testing.T) {
	...
	cst.cs.commitDelayedSiacoinOutputDiff(dscod, skymodules.DiffApply)
	...
	// Rewind the diff.
	cst.cs.commitDelayedSiacoinOutputDiff(dscod, skymodules.DiffRevert)
	...
	// Restore the diff and then apply the inverse diff.
	cst.cs.commitDelayedSiacoinOutputDiff(dscod, skymodules.DiffApply)
	dscod.Direction = skymodules.DiffRevert
	cst.cs.commitDelayedSiacoinOutputDiff(dscod, skymodules.DiffApply)
	if cst.cs.db.lenDelayedSiacoinOutputsHeight(maturityHeight) != initialDscosLen {
		t.Error("siacoin output diff set did not increase in size")
	}
	...
	// that already exists)
	dscod.Direction = skymodules.DiffApply                             // set the direction to apply
	cst.cs.commitDelayedSiacoinOutputDiff(dscod, skymodules.DiffApply) // apply an already existing delayed output.
}
*/
...
func TestCommitSiafundPoolDiff(t *testing.T) {
	...
	sfpd1 := skymodules.SiafundPoolDiff{
		Direction: skymodules.DiffApply,
		Previous:  initial,
		Adjusted:  adjusted1,
	}
	sfpd2 := skymodules.SiafundPoolDiff{
		Direction: skymodules.DiffApply,
		Previous:  adjusted1,
		Adjusted:  adjusted2,
	}
	sfpd3 := skymodules.SiafundPoolDiff{
		Direction: skymodules.DiffApply,
		Previous:  adjusted2,
		Adjusted:  adjusted3,
	}
	cst.cs.commitSiafundPoolDiff(sfpd1, skymodules.DiffApply)
	if cst.cs.siafundPool.Cmp(adjusted1) != 0 {
		t.Error("siafund pool was not adjusted correctly")
	}
	cst.cs.commitSiafundPoolDiff(sfpd2, skymodules.DiffApply)
	if cst.cs.siafundPool.Cmp(adjusted2) != 0 {
		t.Error("second siafund pool adjustment was flawed")
	}
	cst.cs.commitSiafundPoolDiff(sfpd3, skymodules.DiffApply)
	if cst.cs.siafundPool.Cmp(adjusted3) != 0 {
		t.Error("second siafund pool adjustment was flawed")
	}
	cst.cs.commitSiafundPoolDiff(sfpd3, skymodules.DiffRevert)
	if cst.cs.siafundPool.Cmp(adjusted2) != 0 {
		t.Error("reverting second adjustment was flawed")
	}
	cst.cs.commitSiafundPoolDiff(sfpd2, skymodules.DiffRevert)
	if cst.cs.siafundPool.Cmp(adjusted1) != 0 {
		t.Error("reverting second adjustment was flawed")
	}
	cst.cs.commitSiafundPoolDiff(sfpd1, skymodules.DiffRevert)
	...
	defer func() {
		...
		sfpd2.Direction = skymodules.DiffRevert
		cst.cs.commitSiafundPoolDiff(sfpd2, skymodules.DiffApply)
	}()
	cst.cs.commitSiafundPoolDiff(sfpd1, skymodules.DiffApply)
	cst.cs.commitSiafundPoolDiff(sfpd2, skymodules.DiffApply)
	negativeAdjustment := adjusted2.Sub(types.NewCurrency64(100))
	negativeSfpd := skymodules.SiafundPoolDiff{
		Previous: adjusted3,
		Adjusted: negativeAdjustment,
	}
	cst.cs.commitSiafundPoolDiff(negativeSfpd, skymodules.DiffApply)
}
*/
...
func TestDeleteObsoleteDelayedOutputMapsSanity(t *testing.T) {
	...
	err = cst.cs.db.Update(func(tx *bolt.Tx) error {
		return commitDiffSet(tx, pb, skymodules.DiffRevert)
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	defer func() {
		...
		err = cst.cs.db.Update(func(tx *bolt.Tx) error {
			return createUpcomingDelayedOutputMaps(tx, pb, skymodules.DiffApply)
		})
		if err != nil {
			t.Fatal(err)
		}
		err = cst.cs.db.Update(func(tx *bolt.Tx) error {
			return commitNodeDiffs(tx, pb, skymodules.DiffApply)
		})
		if err != nil {
			t.Fatal(err)
		}
		err = cst.cs.db.Update(func(tx *bolt.Tx) error {
			return deleteObsoleteDelayedOutputMaps(tx, pb, skymodules.DiffRevert)
		})
		if err != nil {
			t.Fatal(err)
		}
	...
	err = cst.cs.db.Update(func(tx *bolt.Tx) error {
		return deleteObsoleteDelayedOutputMaps(tx, pb, skymodules.DiffApply)
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateAndApplyDiffSanity(t *testing.T) {
	...
	pb := cst.cs.currentProcessedBlock()
	cst.cs.commitDiffSet(pb, skymodules.DiffRevert)
