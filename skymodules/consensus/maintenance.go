...
func applyMaturedSiacoinOutputs(tx *bolt.Tx, pb *processedBlock) {
	...
	var scods []skymodules.SiacoinOutputDiff
	var dscods []skymodules.DelayedSiacoinOutputDiff
	...
	for _, scod := range scods {
		...
		commitSiacoinOutputDiff(tx, scod, skymodules.DiffApply)
	}
...
// contract expiring.
func applyMissedStorageProof(tx *bolt.Tx, pb *processedBlock, fcid types.FileContractID) (dscods []skymodules.DelayedSiacoinOutputDiff, fcd skymodules.FileContractDiff) {
	// Sanity checks.
...
func applyFileContractMaintenance(tx *bolt.Tx, pb *processedBlock) {
	...
	var dscods []skymodules.DelayedSiacoinOutputDiff
	var fcds []skymodules.FileContractDiff
	...
	for _, fcd := range fcds {
		...
		commitFileContractDiff(tx, fcd, skymodules.DiffApply)
	}
