...
type (
	...
	changeNode struct {
		Entry changeEntry
		Next  skymodules.ConsensusChangeID
	}
)
...
func appendChangeLog(tx *bolt.Tx, ce changeEntry) error {
	...
	ceid := ce.ID()
	cn := changeNode{Entry: ce, Next: skymodules.ConsensusChangeID{}}
	...
	var tailID skymodules.ConsensusChangeID
	copy(tailID[:], cl.Get(ChangeLogTailID))
	if tailID != (skymodules.ConsensusChangeID{}) {
...
// existence.
func getEntry(tx *bolt.Tx, id skymodules.ConsensusChangeID) (ce changeEntry, exists bool) {
	var cn changeNode
	cl := tx.Bucket(ChangeLog)
...
func (ce *changeEntry) ID() skymodules.ConsensusChangeID {
	return skymodules.ConsensusChangeID(crypto.HashObject(ce))
}
