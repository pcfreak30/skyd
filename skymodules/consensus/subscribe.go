...
// provided block.
func computeConsensusChangeDiffs(pb *processedBlock, apply bool) skymodules.ConsensusChangeDiffs {
	if apply {
		return skymodules.ConsensusChangeDiffs{
	...
	cd := skymodules.ConsensusChangeDiffs{
		SiacoinOutputDiffs:        make([]skymodules.SiacoinOutputDiff, len(pb.SiacoinOutputDiffs)),
		FileContractDiffs:         make([]skymodules.FileContractDiff, len(pb.FileContractDiffs)),
		SiafundOutputDiffs:        make([]skymodules.SiafundOutputDiff, len(pb.SiafundOutputDiffs)),
		DelayedSiacoinOutputDiffs: make([]skymodules.DelayedSiacoinOutputDiff, len(pb.DelayedSiacoinOutputDiffs)),
		SiafundPoolDiffs:          make([]skymodules.SiafundPoolDiff, len(pb.SiafundPoolDiffs)),
	}
...
func (cs *ConsensusSet) computeConsensusChange(tx *bolt.Tx, ce changeEntry) (skymodules.ConsensusChange, error) {
	cc := skymodules.ConsensusChange{
		ID: ce.ID(),
	}
	for _, revertedBlockID := range ce.RevertedBlocks {
		...
		if err != nil {
			...
			return skymodules.ConsensusChange{}, err
		}
...
func (cs *ConsensusSet) managedInitializeSubscribe(subscriber skymodules.ConsensusSetSubscriber, start skymodules.ConsensusChangeID,
	cancel <-chan struct{}) (skymodules.ConsensusChangeID, error) {
	if start == skymodules.ConsensusChangeRecent {
		cs.mu.Lock()
		defer cs.mu.Unlock()
	...
	cs.mu.RLock()
	err := cs.db.View(func(tx *bolt.Tx) error {
		if start == skymodules.ConsensusChangeBeginning {
			// Special case: for skymodules.ConsensusChangeBeginning, create an
			...
			if !exists {
				// skymodules.ErrInvalidConsensusChangeID is a named error that
				...
				return skymodules.ErrInvalidConsensusChangeID
			}
	...
	cs.mu.RUnlock()
	if err != nil {
		return skymodules.ConsensusChangeID{}, err
	}
...
// change.
func (cs *ConsensusSet) recentConsensusChangeID() (cid skymodules.ConsensusChangeID, err error) {
	err = cs.db.View(func(tx *bolt.Tx) error {
...
// As a special case, using an empty id as the start will have all the changes
// sent to the modules starting with the genesis block.
func (cs *ConsensusSet) ConsensusSetSubscribe(subscriber skymodules.ConsensusSetSubscriber, start skymodules.ConsensusChangeID,
...
func (cs *ConsensusSet) Unsubscribe(subscriber skymodules.ConsensusSetSubscriber) {
	if cs.tg.Add() != nil {
		return
	}
	defer cs.tg.Done()
