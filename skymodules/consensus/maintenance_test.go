...
func TestApplyMinerPayouts(t *testing.T) {
	...
	if pb.DelayedSiacoinOutputDiffs[0].Direction != skymodules.DiffApply {
...
func TestApplyFoundationSubsidy(t *testing.T) {
	...
	apply := func(height types.BlockHeight) (dscod skymodules.DelayedSiacoinOutputDiff, created bool) {
		err := cst.cs.db.Update(func(tx *bolt.Tx) error {
			pb := &processedBlock{
				Height: height,
			}
