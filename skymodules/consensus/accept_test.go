...
func TestUnitValidateHeader(t *testing.T) {
	...
	var tests = []struct {
		...
		// Test that blocks with too large of a target are rejected.
		{
			...
			errWant:                skymodules.ErrBlockUnsolved,
...
func TestBlockKnownHandling(t *testing.T) {
	...
	if err != nil && !errors.Contains(err, skymodules.ErrNonExtendingBlock) {
		t.Fatal(err)
	}
...
func TestMissedTarget(t *testing.T) {
	...
	if !errors.Contains(err, skymodules.ErrBlockUnsolved) {
		t.Fatalf("expected %v, got %v", skymodules.ErrBlockUnsolved, err)
	}
}
...
// mockGatewayDoesBroadcast implements skymodules.Gateway to mock the Broadcast
// method.
...
func (g *mockGatewayDoesBroadcast) Broadcast(name string, obj interface{}, peers []skymodules.Peer) {
...
type blockCountingSubscriber struct {
	changes []skymodules.ConsensusChangeID
...
func (bcs *blockCountingSubscriber) ProcessConsensusChange(cc skymodules.ConsensusChange) {
...
func TestChainedAcceptBlock(t *testing.T) {
	...
	cst2.cs.ConsensusSetSubscribe(&bcs, skymodules.ConsensusChangeBeginning, cst2.cs.tg.StopChan())
