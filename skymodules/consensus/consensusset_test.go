...
// genesis block.
func blankConsensusSetTester(name string, deps skymodules.Dependencies) (*consensusSetTester, error) {
	testdir := build.TempDir(skymodules.ConsensusDir, name)
	...
	cs, errChan := NewCustomConsensusSet(g, false, filepath.Join(testdir, skymodules.ConsensusDir), deps)
	if err := <-errChan; err != nil {
		return nil, err
	}
	tp, err := transactionpool.New(cs, g, filepath.Join(testdir, skymodules.ConsensusDir))
	if err != nil {
		return nil, err
	}
...
func createConsensusSetTester(name string) (*consensusSetTester, error) {
	cst, err := blankConsensusSetTester(name, skymodules.ProdDependencies)
	if err != nil {
		return nil, err
	}
	cst.addSiafunds()
