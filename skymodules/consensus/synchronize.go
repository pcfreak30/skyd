...
func (cs *ConsensusSet) managedReceiveBlocks(conn skymodules.PeerConn) (returnErr error) {
	...
	for moreAvailable {
		...
		// ignored.
		if acceptErr != nil && !errors.Contains(acceptErr, skymodules.ErrNonExtendingBlock) && !errors.Contains(acceptErr, skymodules.ErrBlockKnown) {
			return acceptErr
		}
	}
	return nil
...
func (cs *ConsensusSet) threadedReceiveBlocks(conn skymodules.PeerConn) error {
	err := conn.SetDeadline(time.Now().Add(sendBlocksTimeout))
	if err != nil {
		return err
	}
...
func (cs *ConsensusSet) rpcSendBlocks(conn skymodules.PeerConn) error {
	err := conn.SetDeadline(time.Now().Add(sendBlocksTimeout))
	if err != nil {
		return err
	}
...
func (cs *ConsensusSet) threadedRPCRelayHeader(conn skymodules.PeerConn) error {
...
func (cs *ConsensusSet) rpcSendBlk(conn skymodules.PeerConn) error {
...
func (cs *ConsensusSet) managedReceiveBlock(id types.BlockID) skymodules.RPCFunc {
	return func(conn skymodules.PeerConn) error {
