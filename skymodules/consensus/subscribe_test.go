...
type mockSubscriber struct {
	updates []skymodules.ConsensusChange
}
...
func (ms *mockSubscriber) ProcessConsensusChange(cc skymodules.ConsensusChange) {
...
func (ms *mockSubscriber) copySub() (cms mockSubscriber) {
	cms.updates = make([]skymodules.ConsensusChange, len(ms.updates))
	copy(cms.updates, ms.updates)
	return cms
}

// TestInvalidConsensusChangeSubscription checks that the consensus set returns
// skymodules.ErrInvalidConsensusChangeID in the event of a subscriber using an
// unrecognized id.
func TestInvalidConsensusChangeSubscription(t *testing.T) {
	...
	badCCID := skymodules.ConsensusChangeID{255, 255, 255}
...
func TestInvalidToValidSubscription(t *testing.T) {
	...
	err = cst.cs.ConsensusSetSubscribe(&ms, skymodules.ConsensusChangeBeginning, cst.cs.tg.StopChan())
	if err != nil {
		t.Fatal(err)
	}
...
func TestModuleDesync(t *testing.T) {
	...
	updates := make([]skymodules.ConsensusChange, 0)
	cst.cs.mu.Lock()
...
func TestPerBlockDiffs(t *testing.T) {
	...
	for _, cc := range ms.updates {
		var ccd skymodules.ConsensusChangeDiffs
