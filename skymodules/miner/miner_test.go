...
func createMinerTester(name string) (*minerTester, error) {
	testdir := build.TempDir(skymodules.MinerDir, name)
	...
	m, err := New(cs, tp, w, filepath.Join(testdir, skymodules.MinerDir))
	if err != nil {
		return nil, err
	}
...
func TestIntegrationBlocksMined(t *testing.T) {
	...
	if !errors.Contains(err, skymodules.ErrNonExtendingBlock) {
		t.Fatal(err)
	}
	...
	rebootMiner, err := New(mt.cs, mt.tpool, mt.wallet, filepath.Join(mt.persistDir, skymodules.MinerDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationAutoRescan(t *testing.T) {
	...
	m, err := New(mt.cs, mt.tpool, mt.wallet, filepath.Join(mt.persistDir, skymodules.MinerDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationStartupRescan(t *testing.T) {
	...
	// first few blocks.
	if mt.miner.persist.RecentChange == (skymodules.ConsensusChangeID{}) || mt.miner.persist.Height == 0 || mt.miner.persist.Target == (types.Target{}) {
