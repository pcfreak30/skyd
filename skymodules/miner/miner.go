...
type Miner struct {
	...
	fullSets           map[skymodules.TransactionSetID][]int
...
func (m *Miner) startupRescan() error {
	...
	err := func() error {
		...
		m.persist.RecentChange = skymodules.ConsensusChangeBeginning
		m.persist.Height = 0
	...
	err = m.cs.ConsensusSetSubscribe(m, skymodules.ConsensusChangeBeginning, m.tg.StopChan())
	if err != nil {
		return err
	}
...
func New(cs skymodules.ConsensusSet, tpool skymodules.TransactionPool, w skymodules.Wallet, persistDir string) (*Miner, error) {
	...
	m := &Miner{
		...
		fullSets:  make(map[skymodules.TransactionSetID][]int),
