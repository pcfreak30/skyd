...
func TestAcceptPeer(t *testing.T) {
	...
	for i := 0; i < fullyConnectedThreshold+1; i++ {
		addr := skymodules.NetAddress(fmt.Sprintf("1.2.3.%d", i))
		p := &peer{
			Peer: skymodules.Peer{
				NetAddress: addr,
				Inbound:    false,
	...
	for i := 0; i < fullyConnectedThreshold+1; i++ {
		addr := skymodules.NetAddress(fmt.Sprintf("127.0.0.1:%d", i))
		p := &peer{
			Peer: skymodules.Peer{
				NetAddress: addr,
				Inbound:    true,
...
func TestAcceptPeerSameHost(t *testing.T) {
	...
	duplicatePeer := func() *peer {
		addr := skymodules.NetAddress(fmt.Sprintf("1.2.3.4:%d", port))
		port++
		return &peer{
			Peer: skymodules.Peer{
				NetAddress: addr,
...
func TestListen(t *testing.T) {
	...
	addr := skymodules.NetAddress(conn.LocalAddr().String())
	...
	header.NetAddress = skymodules.NetAddress(conn.LocalAddr().String())
	err = exchangeOurHeader(conn, header)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConnect(t *testing.T) {
	...
	bootstrap.mu.Lock()
	bootstrap.handlers[handlerName("ShareNodes")] = func(skymodules.PeerConn) error {
		return nil
	}
	bootstrap.mu.Unlock()
	// connect
...
func TestConnectRejectsInvalidAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			addr:    skymodules.NetAddress(net.JoinHostPort("localhost", g2Port)),
			wantErr: true,
...
func TestConnectRejectsVersions(t *testing.T) {
	...
	for testIndex, tt := range tests {
		...
		err = g.Connect(skymodules.NetAddress(listener.Addr().String()))
		switch {
		...
		<-doneChan
		g.Disconnect(skymodules.NetAddress(listener.Addr().String()))
	}
}
...
func TestPeerManager(t *testing.T) {
	...
	g1.mu.Lock()
	g1.nodes = map[skymodules.NetAddress]*node{}
