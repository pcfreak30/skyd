...
func (g *Gateway) addNode(addr skymodules.NetAddress) error {
	if addr == g.myAddr {
...
func (g *Gateway) staticPingNode(addr skymodules.NetAddress) (err error) {
	...
	ourHeader := sessionHeader{
		...
		UniqueID:   g.staticID,
		NetAddress: skymodules.NetAddress(conn.LocalAddr().String()),
	}
	...
	if err := encoding.WriteObject(conn, skymodules.StopResponse); err != nil {
...
func (g *Gateway) removeNode(addr skymodules.NetAddress) error {
...
func (g *Gateway) randomNode() (skymodules.NetAddress, error) {
	if len(g.nodes) == 0 {
		return "", errNoPeers
	}
...
func (g *Gateway) shareNodes(conn skymodules.PeerConn) error {
	conn.SetDeadline(time.Now().Add(connStdDeadline))
	remoteNA := skymodules.NetAddress(conn.RemoteAddr().String())

	// Assemble a list of nodes to send to the peer.
	var nodes []skymodules.NetAddress
	func() {
		g.mu.RLock()
		defer g.mu.RUnlock()

		// Gather candidates for sharing.
		gnodes := make([]skymodules.NetAddress, 0, len(g.nodes))
		for node := range g.nodes {
...
func (g *Gateway) requestNodes(conn skymodules.PeerConn) error {
	conn.SetDeadline(time.Now().Add(connStdDeadline))

	var nodes []skymodules.NetAddress
	if err := encoding.ReadObject(conn, &nodes, maxSharedNodes*skymodules.MaxEncodedNetAddressLength); err != nil {
		return err
	}
