...
func TestRandomNode(t *testing.T) {
	...
	// Test with 3 nodes.
	nodes := map[skymodules.NetAddress]int{
...
func TestShareNodes(t *testing.T) {
	...
	g1.mu.Unlock()
	g2.mu.Lock()
	g2.nodes = map[skymodules.NetAddress]*node{}
	g2.mu.Unlock()

	// SharePeers should now return no peers
	var nodes []skymodules.NetAddress
	err = g1.RPC(g2.Address(), "ShareNodes", func(conn skymodules.PeerConn) error {
		return encoding.ReadObject(conn, &nodes, maxSharedNodes*skymodules.MaxEncodedNetAddressLength)
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 1; i < int(maxSharedNodes)+11; i++ {
		g2.mu.Lock()
		err := g2.addNode(skymodules.NetAddress("111.111.111.111:" + strconv.Itoa(i)))
		g2.mu.Unlock()
		if err != nil {
			t.Fatal(err)
		}
