...
func TestRegisterRPC(t *testing.T) {
	...
	g.RegisterRPC("Foo", func(conn skymodules.PeerConn) error { return nil })
	defer func() {
		if r := recover(); r == nil {
...
func TestUnregisterRPC(t *testing.T) {
	...
	dummyFunc := func(conn skymodules.PeerConn) error {
		var str string
		return encoding.ReadObject(conn, &str, 11)
	}

	// Register RPC and check that calling it succeeds.
	g1.RegisterRPC("Foo", func(conn skymodules.PeerConn) error {
...
func TestRegisterConnectCall(t *testing.T) {
	...
	g.RegisterConnectCall("Foo", func(conn skymodules.PeerConn) error { return nil })
	defer func() {
		if r := recover(); r == nil {
...
func TestUnregisterConnectCallPanics(t *testing.T) {
	...
	g1.RegisterConnectCall("Foo", func(conn skymodules.PeerConn) error {
		rpcChan <- struct{}{}
		return nil
	})
...
func TestRPC(t *testing.T) {
	...
	g2.RegisterRPC("Foo", func(conn skymodules.PeerConn) error {
		var i uint64
	...
	var foo string
	err = g1.RPC(g2.Address(), "Foo", func(conn skymodules.PeerConn) error {
	...
	err = g1.RPC(g2.Address(), "Foo", func(skymodules.PeerConn) error {
...
func TestThreadedHandleConn(t *testing.T) {
	...
	rpcFn := func(fn func(skymodules.PeerConn) error) error {
	...
	// bad rpcID
	err = rpcFn(func(conn skymodules.PeerConn) error {
...
func TestBroadcast(t *testing.T) {
	...
	g2.RegisterRPC("Recv", func(conn skymodules.PeerConn) error {
		encoding.ReadObject(conn, &g2Payload, 100)
		g2DoneChan <- struct{}{}
		return nil
	})
	g3.RegisterRPC("Recv", func(conn skymodules.PeerConn) error {
...
func TestOutboundAndInboundRPCs(t *testing.T) {
	...
	g1.RegisterRPC("recv", func(conn skymodules.PeerConn) error {
		rpcChanG1 <- struct{}{}
		return nil
	})
	g2.RegisterRPC("recv", func(conn skymodules.PeerConn) error {
	...
	err = g1.RPC(g2.Address(), "recv", func(conn skymodules.PeerConn) error { return nil })
	if err != nil {
		t.Fatal(err)
	}
	<-rpcChanG2
	...
	var addr skymodules.NetAddress
	for pAddr := range g2.peers {
		addr = pAddr
		break
	}
	err = g2.RPC(addr, "recv", func(conn skymodules.PeerConn) error { return nil })
	if err != nil {
		t.Fatal(err)
	}
	<-rpcChanG1
...
func TestCallingRPCFromRPC(t *testing.T) {
	...
	g1.RegisterRPC("FOO", func(conn skymodules.PeerConn) error {
		err := g1.RPC(conn.RPCAddr(), "BAR", func(conn skymodules.PeerConn) error { return nil })
		errChan <- err
		return err
	})

	barChan := make(chan struct{})
	g2.RegisterRPC("BAR", func(conn skymodules.PeerConn) error {
		barChan <- struct{}{}
		return nil
	})
	...
	err = g2.RPC(g1.Address(), "FOO", func(conn skymodules.PeerConn) error {
		return nil
	})
	if err != nil {
		t.Fatal(err)
...
func TestRPCRatelimit(t *testing.T) {
	...
	for i := 0; i < callVolume; i++ {
		...
		go func() {
			...
			_ = g1.RPC(g2.Address(), "recv", func(conn skymodules.PeerConn) error {
				buf := make([]byte, 2)
				_, err := conn.Read(buf)
				if err != nil {
					return err
