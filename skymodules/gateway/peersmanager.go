...
func (g *Gateway) managedPeerManagerConnect(addr skymodules.NetAddress) {
...
func (g *Gateway) buildPeerManagerNodeList() []skymodules.NetAddress {
	// flatten the node map, inserting in random order
	nodes := make([]skymodules.NetAddress, len(g.nodes))
