...
func (g *Gateway) managedRPC(addr skymodules.NetAddress, name string, fn skymodules.RPCFunc) (err error) {
	g.mu.RLock()
...
func (g *Gateway) RPC(addr skymodules.NetAddress, name string, fn skymodules.RPCFunc) error {
...
func (g *Gateway) RegisterRPC(name string, fn skymodules.RPCFunc) {
	g.mu.Lock()
	defer g.mu.Unlock()
...
// upon connecting.
func (g *Gateway) RegisterConnectCall(name string, fn skymodules.RPCFunc) {
	g.mu.Lock()
	defer g.mu.Unlock()
...
func (g *Gateway) threadedHandleConn(conn skymodules.PeerConn) {
	defer func() {
		_ = conn.Close()
	}()
	...
	if errors.Contains(err, skymodules.ErrDuplicateTransactionSet) || errors.Contains(err, skymodules.ErrBlockKnown) {
		err = nil
	}
	if err != nil {
...
// an RPCFunc.
func (g *Gateway) Broadcast(name string, obj interface{}, peers []skymodules.Peer) {
	if g.threads.Add() != nil {
		return
	}
	defer g.threads.Done()
	...
	fn := func(conn skymodules.PeerConn) error {
	...
	for _, p := range peers {
		wg.Add(1)
		go func(addr skymodules.NetAddress) {
			defer wg.Done()
