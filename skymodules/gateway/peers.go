...
func (p *peer) open() (skymodules.PeerConn, error) {
	...
	conn = ratelimit.NewRLConn(conn, skymodules.GlobalRateLimits, nil)
	return &peerConn{conn, p.NetAddress}, nil
}

func (p *peer) accept() (skymodules.PeerConn, error) {
...
// to a peer.
func (g *Gateway) callInitRPCs(addr skymodules.NetAddress) {
	for name, fn := range g.initRPCs {
		go func(name string, fn skymodules.RPCFunc) {
...
func (g *Gateway) randomOutboundPeer() (skymodules.NetAddress, error) {
	// Get the list of outbound peers.
	var addrs []skymodules.NetAddress
...
func (g *Gateway) threadedAcceptConn(conn net.Conn) {
	...
	addr := skymodules.NetAddress(conn.RemoteAddr().String())
...
func (g *Gateway) managedAcceptConnPeer(conn net.Conn, remoteVersion string) error {
	...
	remoteIP := skymodules.NetAddress(conn.RemoteAddr().String()).Host()
	remotePort := remoteHeader.NetAddress.Port()
	remoteAddr := skymodules.NetAddress(net.JoinHostPort(remoteIP, remotePort))
	g.log.Debugln("Making connection with remote peer", remoteAddr)

	// Accept the peer.
	peer := &peer{
		Peer: skymodules.Peer{
			Inbound: true,
...
func (g *Gateway) acceptPeer(p *peer) {
	...
	var addrs, preferredAddrs []skymodules.NetAddress
	for addr, peer := range g.peers {
...
func exchangeOurHeader(conn net.Conn, ourHeader sessionHeader) error {
	...
	} else if response == skymodules.StopResponse {
		return errors.New("peer did not want a connection")
	} else if response != skymodules.AcceptResponse {
...
func exchangeRemoteHeader(conn net.Conn, ourHeader sessionHeader) (sessionHeader, error) {
	...
	} else if err := encoding.WriteObject(conn, skymodules.AcceptResponse); err != nil {
...
func (g *Gateway) managedConnectPeer(conn net.Conn, remoteVersion string, remoteAddr skymodules.NetAddress) error {
...
func (g *Gateway) managedConnect(addr skymodules.NetAddress) error {
...
// Gateway's peer list.
func (g *Gateway) Connect(addr skymodules.NetAddress) error {
...
// Gateway's peer list.
func (g *Gateway) Disconnect(addr skymodules.NetAddress) error {
...
// from the blocklist.
func (g *Gateway) ConnectManual(addr skymodules.NetAddress) error {
...
func (g *Gateway) DisconnectManual(addr skymodules.NetAddress) error {
...
func (g *Gateway) Online() (online bool) {
	defer func() {
		if online {
			g.staticAlerter.UnregisterAlert(skymodules.AlertIDGatewayOffline)
		} else {
			g.staticAlerter.RegisterAlert(skymodules.AlertIDGatewayOffline, AlertMSGGatewayOffline, "", skymodules.SeverityWarning)
		}
	}()
...
func (g *Gateway) Peers() []skymodules.Peer {
	g.mu.RLock()
	defer g.mu.RUnlock()
	var peers []skymodules.Peer
	for _, p := range g.peers {
