...
// Gateway implements the skymodules.Gateway interface.
type Gateway struct {
	listener net.Listener
	m        *connmonitor.Monitor
	myAddr   skymodules.NetAddress
	port     string
	...
	handlers map[rpcID]skymodules.RPCFunc
	initRPCs map[string]skymodules.RPCFunc
	...
	nodes     map[skymodules.NetAddress]*node
	peers     map[skymodules.NetAddress]*peer
...
func (g *Gateway) Address() skymodules.NetAddress {
	g.mu.RLock()
	defer g.mu.RUnlock()
	return g.myAddr
}
...
func New(addr string, bootstrap bool, persistDir string) (*Gateway, error) {
	return NewCustomGateway(addr, bootstrap, persistDir, skymodules.ProdDependencies)
}

// NewCustomGateway returns an initialized Gateway with custom dependencies.
func NewCustomGateway(addr string, bootstrap bool, persistDir string, deps skymodules.Dependencies) (*Gateway, error) {
	...
	g := &Gateway{
		handlers: make(map[rpcID]skymodules.RPCFunc),
		initRPCs: make(map[string]skymodules.RPCFunc),

		blocklist: make(map[string]struct{}),
		nodes:     make(map[skymodules.NetAddress]*node),
		peers:     make(map[skymodules.NetAddress]*peer),

		persistDir:    persistDir,
		staticAlerter: skymodules.NewAlerter("gateway"),
		staticDeps:    deps,
	}
	...
	if bootstrap {
		for _, addr := range skymodules.BootstrapPeers {
			err := g.addNode(addr)
	...
	g.myAddr = skymodules.NetAddress(net.JoinHostPort(host, port))
...
// enforce that Gateway satisfies the skymodules.Gateway interface
var _ skymodules.Gateway = (*Gateway)(nil)
