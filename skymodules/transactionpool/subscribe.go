...
func (tp *TransactionPool) updateSubscribersTransactions() {
	diff := new(skymodules.TransactionPoolDiff)
	...
	for id := range tp.subscriberSets {
		...
		// fields filled out.
		diff.RevertedTransactions = append(diff.RevertedTransactions, skymodules.TransactionSetID(id))
	}

	// Clear the subscriber sets map.
	for _, revert := range diff.RevertedTransactions {
		delete(tp.subscriberSets, skymodules.TransactionSetID(revert))
	}
	...
	for id, set := range tp.transactionSets {
		...
		ut := &skymodules.UnconfirmedTransactionSet{
			Change: tp.transactionSetDiffs[id],
			ID:     skymodules.TransactionSetID(id),
...
func (tp *TransactionPool) TransactionPoolSubscribe(subscriber skymodules.TransactionPoolSubscriber) {
	tp.mu.Lock()
	defer tp.mu.Unlock()
	...
	diff := new(skymodules.TransactionPoolDiff)
	diff.AppliedTransactions = make([]*skymodules.UnconfirmedTransactionSet, 0, len(tp.subscriberSets))
...
func (tp *TransactionPool) Unsubscribe(subscriber skymodules.TransactionPoolSubscriber) {
	tp.mu.Lock()
	defer tp.mu.Unlock()
