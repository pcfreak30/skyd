...
type (
	...
	TransactionPool struct {
		...
		knownObjects        map[ObjectID]skymodules.TransactionSetID
		subscriberSets      map[skymodules.TransactionSetID]*skymodules.UnconfirmedTransactionSet
		transactionHeights  map[types.TransactionID]types.BlockHeight
		transactionSets     map[skymodules.TransactionSetID][]types.Transaction
		transactionSetDiffs map[skymodules.TransactionSetID]*skymodules.ConsensusChange
		...
		// subscriber.
		subscribers []skymodules.TransactionPoolSubscriber
...
// Enforce that TransactionPool satisfies the skymodules.TransactionPool interface.
var _ skymodules.TransactionPool = (*TransactionPool)(nil)

// New creates a transaction pool that is ready to receive transactions.
func New(cs skymodules.ConsensusSet, g skymodules.Gateway, persistDir string) (*TransactionPool, error) {
	return NewCustomTPool(cs, g, persistDir, skymodules.ProdDependencies)
}

// NewCustomTPool creates a transaction pool with custom dependencies.
func NewCustomTPool(cs skymodules.ConsensusSet, g skymodules.Gateway, persistDir string, deps skymodules.Dependencies) (*TransactionPool, error) {
	...
	tp := &TransactionPool{
		...
		knownObjects:        make(map[ObjectID]skymodules.TransactionSetID),
		subscriberSets:      make(map[skymodules.TransactionSetID]*skymodules.UnconfirmedTransactionSet),
		transactionHeights:  make(map[types.TransactionID]types.BlockHeight),
		transactionSets:     make(map[skymodules.TransactionSetID][]types.Transaction),
		transactionSetDiffs: make(map[skymodules.TransactionSetID]*skymodules.ConsensusChange),
...
func (tp *TransactionPool) printConflicts(ts []types.Transaction) {
	...
	conflictSets := make(map[skymodules.TransactionSetID]bool)
