...
// database.
func (tp *TransactionPool) getRecentConsensusChange(tx *bolt.Tx) (cc skymodules.ConsensusChangeID, err error) {
	ccBytes := tx.Bucket(bucketRecentConsensusChange).Get(fieldRecentConsensusChange)
	if ccBytes == nil {
		return skymodules.ConsensusChangeID{}, errNilConsensusChange
	}
	copy(cc[:], ccBytes)
	return cc, nil
}
...
func (tp *TransactionPool) putRecentConsensusChange(tx *bolt.Tx, cc skymodules.ConsensusChangeID) error {
