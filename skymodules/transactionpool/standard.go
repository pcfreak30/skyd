...
func isStandardTransaction(t types.Transaction) (uint64, error) {
	...
	if tlen > skymodules.TransactionSizeLimit {
		return 0, skymodules.ErrLargeTransaction
	}
	...
	for _, arb := range t.ArbitraryData {
		...
		copy(prefix[:], arb)
		if prefix == skymodules.PrefixHostAnnouncement ||
			prefix == skymodules.PrefixNonSia ||
			prefix == skymodules.PrefixFileContractIdentifier ||
			prefix == types.SpecifierFoundation {
			continue
		}

		return 0, skymodules.ErrInvalidArbPrefix
	}
...
func isStandardTransactionSet(ts []types.Transaction) (uint64, error) {
	...
	for i := range ts {
		...
		totalSize += tSize
		if totalSize > skymodules.TransactionSetSizeLimit {
			return 0, skymodules.ErrLargeTransactionSet
		}
	}
