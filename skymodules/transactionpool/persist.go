...
func (tp *TransactionPool) resetDB(tx *bolt.Tx) error {
	...
	err = tp.putRecentConsensusChange(tx, skymodules.ConsensusChangeBeginning)
	if err != nil {
		return err
	}
...
func (tp *TransactionPool) initPersist() error {
	...
	var cc skymodules.ConsensusChangeID
	...
	if errors.Contains(err, errNilConsensusChange) {
		err = tp.putRecentConsensusChange(tp.dbTx, skymodules.ConsensusChangeBeginning)
	}
	if err != nil {
	...
	go func() {
		...
		if errors.Contains(err, skymodules.ErrInvalidConsensusChangeID) {
			...
			freshScanErr := tp.consensusSet.ConsensusSetSubscribe(tp, skymodules.ConsensusChangeBeginning, tp.tg.StopChan())
