...
func (tp *TransactionPool) purge() {
	tp.knownObjects = make(map[ObjectID]skymodules.TransactionSetID)
	tp.transactionSets = make(map[skymodules.TransactionSetID][]types.Transaction)
	tp.transactionSetDiffs = make(map[skymodules.TransactionSetID]*skymodules.ConsensusChange)
...
func (tp *TransactionPool) ProcessConsensusChange(cc skymodules.ConsensusChange) {
	tp.mu.Lock()
