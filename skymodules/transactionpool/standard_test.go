...
func TestIntegrationLargeTransactions(t *testing.T) {
	...
	arbData := make([]byte, skymodules.TransactionSizeLimit)
	copy(arbData, skymodules.PrefixNonSia[:])
	...
	if !errors.Contains(err, skymodules.ErrLargeTransaction) {
		t.Fatal(err)
	}

	// Create a large transaction set and try to get it accepted.
	var tset []types.Transaction
	for i := 0; i <= skymodules.TransactionSetSizeLimit/10e3; i++ {
	...
	if !errors.Contains(err, skymodules.ErrLargeTransactionSet) {
		t.Fatal(err)
	}
}
