...
func (tp *TransactionPool) checkTransactionSetComposition(ts []types.Transaction) (uint64, error) {
	...
	setID := skymodules.TransactionSetID(crypto.HashObject(ts))
	_, exists := tp.transactionSets[setID]
	if exists {
		return 0, skymodules.ErrDuplicateTransactionSet
	}
...
func (tp *TransactionPool) handleConflicts(ts []types.Transaction, conflicts []skymodules.TransactionSetID, txnFn func([]types.Transaction) (skymodules.ConsensusChange, error)) ([]types.Transaction, error) {
	// Create a list of all the transaction ids that compose the set of
	// conflicts.
	conflictMap := make(map[types.TransactionID]skymodules.TransactionSetID)
	...
	if len(dedupSet) == 0 {
		return nil, skymodules.ErrDuplicateTransactionSet
	}
	...
	if len(dedupSet) < len(ts) {
		...
		var conflicts []skymodules.TransactionSetID
	...
	supersetMap := make(map[skymodules.TransactionSetID]struct{})
	...
	if err != nil {
		return nil, skymodules.NewConsensusConflict("provided transaction set has prereqs, but is still invalid: " + err.Error())
	}
	...
	setID := skymodules.TransactionSetID(crypto.HashObject(superset))
...
func (tp *TransactionPool) acceptTransactionSet(ts []types.Transaction, txnFn func([]types.Transaction) (skymodules.ConsensusChange, error)) (superset []types.Transaction, err error) {
	if len(ts) == 0 {
	...
	cc, err := txnFn(ts)
	if err != nil {
		return nil, skymodules.NewConsensusConflict("provided transaction set is invalid: " + err.Error())
	}
...
func (tp *TransactionPool) submitTransactionSet(ts []types.Transaction) ([]types.Transaction, error) {
	...
	cs, ok := tp.consensusSet.(interface {
		LockedTryTransactionSet(fn func(func(txns []types.Transaction) (skymodules.ConsensusChange, error)) error) error
	})
	if !ok {
	...
	var superset []types.Transaction
	var acceptErr error
	err := cs.LockedTryTransactionSet(func(txnFn func(txns []types.Transaction) (skymodules.ConsensusChange, error)) error {
		tp.mu.Lock()
		defer tp.mu.Unlock()

		// Attempt to get the transaction set into the transaction pool.
		superset, acceptErr = tp.acceptTransactionSet(ts, txnFn)
		if errors.Contains(acceptErr, skymodules.ErrDuplicateTransactionSet) {
			tp.log.Debugln("Transaction set is a duplicate:", acceptErr)
			return acceptErr
		}
		if acceptErr != nil {
			tp.log.Debugln("Transaction set broadcast has failed:", acceptErr)
			if build.DEBUG && skymodules.IsConsensusConflict(acceptErr) {
...
// other peers.
func (tp *TransactionPool) relayTransactionSet(conn skymodules.PeerConn) error {
	if err := tp.tg.Add(); err != nil {
		return err
	}
	defer tp.tg.Done()
