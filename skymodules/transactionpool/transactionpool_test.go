...
func blankTpoolTester(name string) (*tpoolTester, error) {
	// Initialize the skymodules.
	testdir := build.TempDir(skymodules.TransactionPoolDir, name)
	...
	tp, err := New(cs, g, filepath.Join(testdir, skymodules.TransactionPoolDir))
	if err != nil {
		return nil, err
	}
...
func TestIntegrationNewNilInputs(t *testing.T) {
	...
	testdir := build.TempDir(skymodules.TransactionPoolDir, t.Name())
	g, err := gateway.New("localhost:0", false, filepath.Join(testdir, skymodules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	tpDir := filepath.Join(testdir, skymodules.TransactionPoolDir)
