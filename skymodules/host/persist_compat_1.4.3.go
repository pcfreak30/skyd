...
func (h *Host) upgradeFromV120ToV143() error {
	...
	p := new(persistence)
	err := h.dependencies.LoadFile(skymodules.Hostv120PersistMetadata, p, filepath.Join(h.persistDir, settingsFile))
	if err != nil {
		return build.ExtendErr("could not load persistence object", err)
	}

	// Add the ephemeral account defaults
	p.Settings.EphemeralAccountExpiry = skymodules.DefaultEphemeralAccountExpiry
	p.Settings.MaxEphemeralAccountBalance = skymodules.DefaultMaxEphemeralAccountBalance
	...
	err = persist.SaveJSON(skymodules.Hostv143PersistMetadata, h.persistData(), filepath.Join(h.persistDir, settingsFile))
	if err != nil {
