...
func newTestTPool(name string, deps skymodules.Dependencies) (func() error, *transactionpool.TransactionPool, error) {
	...
	// Create the tpool.
	tp, err := transactionpool.NewCustomTPool(cs, g, filepath.Join(testdir, skymodules.TransactionPoolDir), deps)
	if err != nil {
		return nil, nil, err
	}
	closefn := func() error {
...
func (ht *hostTester) newTesterStorageObligation() (storageObligation, error) {
	...
	_ = builder.AddFileContract(types.FileContract{
		...
		WindowEnd:   ht.host.blockHeight + revisionSubmissionBuffer + skymodules.DefaultWindowSize + 2,
...
func TestPruneStaleStorageObligations(t *testing.T) {
	...
	endblock := ht.host.blockHeight + revisionSubmissionBuffer + skymodules.DefaultWindowSize + 2 + wallet.RespendTimeout + 1
...
func TestSingleSectorStorageObligationStack(t *testing.T) {
	...
	for i := 0; i <= int(skymodules.DefaultWindowSize); i++ {
		_, err := ht.miner.AddBlock()
		if err != nil {
			t.Fatal(err)
		}
