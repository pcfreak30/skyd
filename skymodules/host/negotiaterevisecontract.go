...
func cachedMerkleRoot(roots []crypto.Hash) crypto.Hash {
	...
	for 1<<log2SectorSize < (skymodules.SectorSize / crypto.SegmentSize) {
		log2SectorSize++
	}
...
func (h *Host) managedRevisionIteration(conn net.Conn, so *storageObligation, finalIter bool) error {
	...
	conn.SetDeadline(time.Now().Add(skymodules.NegotiateFileContractRevisionTime))
	...
	var modifications []skymodules.RevisionAction
	...
	err = encoding.ReadObject(conn, &revision, skymodules.NegotiateMaxFileContractRevisionSize)
	if err != nil {
	...
	err = func() error {
		for _, modification := range modifications {
			...
			if modification.Type == skymodules.ActionInsert {
			...
			if uint64(len(modification.Data)) > skymodules.SectorSize {
				return ErrLargeSector
			}

			switch modification.Type {
			case skymodules.ActionDelete:
			...
			case skymodules.ActionInsert:
				...
				blockBytesCurrency := types.NewCurrency64(uint64(blocksRemaining)).Mul64(skymodules.SectorSize)
			...
			case skymodules.ActionModify:
				...
				// a risk of overflow.
				if modification.Offset > skymodules.SectorSize || modification.Offset+uint64(len(modification.Data)) > skymodules.SectorSize {
	...
	if err != nil {
		skymodules.WriteNegotiationRejection(conn, err) // Error is ignored so that the error type can be preserved in extendErr.
	...
	err = encoding.ReadObject(conn, &renterSig, skymodules.NegotiateMaxTransactionSignatureSize)
	if err != nil {
	...
	if finalIter {
		err = skymodules.WriteNegotiationStop(conn)
	} else {
		err = skymodules.WriteNegotiationAcceptance(conn)
	}
	if err != nil {
...
func verifyRevision(so storageObligation, revision types.FileContractRevision, blockHeight types.BlockHeight, expectedExchange, expectedCollateral types.Currency) error {
	...
	if revision.NewFileSize != uint64(len(so.SectorRoots))*skymodules.SectorSize {
