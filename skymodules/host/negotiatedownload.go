...
func (h *Host) managedDownloadIteration(conn net.Conn, so *storageObligation) error {
	...
	conn.SetDeadline(time.Now().Add(skymodules.NegotiateDownloadTime))
	...
	// pays for them.
	var requests []skymodules.DownloadAction
	var paymentRevision types.FileContractRevision
	err = encoding.ReadObject(conn, &requests, skymodules.NegotiateMaxDownloadActionRequestSize)
	if err != nil {
		return extendErr("failed to read download requests:", ErrorConnection(err.Error()))
	}
	err = encoding.ReadObject(conn, &paymentRevision, skymodules.NegotiateMaxFileContractRevisionSize)
	if err != nil {
	...
	err = func() error {
		...
		for _, request := range requests {
			if request.Length > skymodules.SectorSize || request.Offset+request.Length > skymodules.SectorSize {
	...
	if err != nil {
		skymodules.WriteNegotiationRejection(conn, err) // Error not reported to preserve type in extendErr
	...
	err = encoding.ReadObject(conn, &renterSignature, skymodules.NegotiateMaxTransactionSignatureSize)
	if err != nil {
	...
	if err != nil {
		return extendErr("failed to modify storage obligation: ", ErrorInternal(skymodules.WriteNegotiationRejection(conn, err).Error()))
	}
