...
func TestHostWorkingStatus(t *testing.T) {
	...
	// Disrupt() pattern.
	// if ht.host.WorkingStatus() != skymodules.HostWorkingStatusChecking {
	// 	t.Fatal("expected working state to initially be skymodules.HostWorkingStatusChecking")
	// }
	...
	for i := 0; i < 5; i++ {
		...
		for start := time.Now(); time.Since(start) < 30*time.Second; time.Sleep(time.Millisecond * 10) {
			if ht.host.WorkingStatus() == skymodules.HostWorkingStatusWorking {
				success = true
				break
			}
		}
		...
		success = false
		for start := time.Now(); time.Since(start) < 30*time.Second; time.Sleep(time.Millisecond * 10) {
			if ht.host.WorkingStatus() == skymodules.HostWorkingStatusNotWorking {
				success = true
				break
			}
		}
...
func TestHostConnectabilityStatus(t *testing.T) {
	...
	// case.  This check is disabled for now, but can be fixed by using the
	// Disrupt() pattern.
	// if ht.host.ConnectabilityStatus() != skymodules.HostConnectabilityStatusChecking {
	// 		t.Fatal("expected connectability state to initially be ConnectablityStateChecking")
	// }

	success := false
	for start := time.Now(); time.Since(start) < 30*time.Second; time.Sleep(time.Millisecond * 10) {
		if ht.host.ConnectabilityStatus() == skymodules.HostConnectabilityStatusConnectable {
			success = true
			break
		}
	}
...
func TestUnrecognizedRPCID(t *testing.T) {
	...
	err = skymodules.RPCWrite(stream, randomRPCID)
	if err != nil {
		t.Fatal(err)
	}
