...
func rhp2RenewBaseCollateral(so storageObligation, settings skymodules.HostExternalSettings, fc types.FileContract) types.Currency {
...
func rhp2RenewBasePrice(so storageObligation, settings skymodules.HostExternalSettings, fc types.FileContract) types.Currency {
	if fc.WindowEnd <= so.proofDeadline() {
		return types.NewCurrency64(0)
	}
	timeExtension := fc.WindowEnd - so.proofDeadline()
...
// settings.
func rhp2RenewContractCollateral(so storageObligation, settings skymodules.HostExternalSettings, fc types.FileContract) (types.Currency, error) {
...
// expected to add to the file contract based on the file contract and host
// settings.
func renewContractCollateral(pt *skymodules.RPCPriceTable, rev types.FileContractRevision, fc types.FileContract) (types.Currency, error) {
	if fc.ValidHostPayout().Cmp(pt.ContractPrice) < 0 {
		return types.Currency{}, errors.New("ContractPrice higher than ValidHostOutput")
	}

	diff := fc.ValidHostPayout().Sub(pt.ContractPrice)
	rbc, _ := skymodules.RenewBaseCosts(rev, pt, fc.WindowStart)
	if diff.Cmp(rbc) < 0 {
		return types.Currency{}, errors.New("ValidHostOutput smaller than ContractPrice + RenewBasePrice")
	}
...
// contract.
func (h *Host) managedAddRenewCollateral(hostCollateral types.Currency, so storageObligation, txnSet []types.Transaction) (builder skymodules.TransactionBuilder, newParents []types.Transaction, newInputs []types.SiacoinInput, newOutputs []types.SiacoinOutput, err error) {
...
func (h *Host) managedRPCRenewContractRHP2(conn net.Conn) error {
	...
	conn.SetDeadline(time.Now().Add(skymodules.NegotiateRenewContractTime))
	...
	err = encoding.ReadObject(conn, &txnSet, skymodules.NegotiateMaxFileContractSetLen)
	if err != nil {
		return extendErr("unable to read transaction set: ", ErrorConnection(err.Error()))
	}
	err = encoding.ReadObject(conn, &renterPK, skymodules.NegotiateMaxSiaPubkeySize)
	if err != nil {
	...
	if err != nil {
		skymodules.WriteNegotiationRejection(conn, err) // Error is ignored to preserve type for extendErr
	...
	err = skymodules.WriteNegotiationAcceptance(conn)
	if err != nil {
	...
	err = encoding.ReadObject(conn, &renterTxnSignatures, skymodules.NegotiateMaxTransactionSignatureSize)
	if err != nil {
		return extendErr("failed to read renter transaction signatures: ", ErrorConnection(err.Error()))
	}
	err = encoding.ReadObject(conn, &renterRevisionSignature, skymodules.NegotiateMaxTransactionSignatureSize)
	if err != nil {
...
func (h *Host) managedVerifyRenewedContract(so storageObligation, txnSet []types.Transaction, renterPK types.SiaPublicKey) (types.Currency, error) {
	...
	defer func() {
		if registerHostInsufficientCollateral {
			h.staticAlerter.RegisterAlert(skymodules.AlertIDHostInsufficientCollateral, AlertMSGHostInsufficientCollateral, "", skymodules.SeverityWarning)
		} else {
			h.staticAlerter.UnregisterAlert(skymodules.AlertIDHostInsufficientCollateral)
		}
	}()
	...
	// blockchain.
	setFee := skymodules.CalculateFee(txnSet)
