...
func TestV120HostUpgrade(t *testing.T) {
	...
	persistDir := build.TempDir(skymodules.HostDir, t.Name())
	hostPersistDir := build.TempDir(skymodules.HostDir, t.Name(), skymodules.HostDir)
	...
	if !his.MaxEphemeralAccountBalance.Equals(skymodules.DefaultMaxEphemeralAccountBalance) {
	...
	err = persist.LoadJSON(skymodules.Hostv151PersistMetadata, struct{}{}, filepath.Join(hostPersistDir, skymodules.HostSettingsFile))
	if err != nil {
		t.Fatal(err)
	}
}
