...
func recoverFromError(ctx context.Context, t *testing.T, pair *renterHostPair, stats *rpcStats, errChan chan error, cancel context.CancelFunc) {
	...
	for err := range errChan {
		...
		if !recovered && skymodules.IsPriceTableInvalidErr(err) {
			var payByFC bool
...
func newTestReadSectorProgram(pt *skymodules.RPCPriceTable, root crypto.Hash, full bool) testMDMProgram {
	var offset uint64
	length := skymodules.SectorSize
	if !full {
		offset = fastrand.Uint64n((skymodules.SectorSize/crypto.SegmentSize)-1) * crypto.SegmentSize
...
// this sector.
func newTestHasSectorProgram(pt *skymodules.RPCPriceTable, root crypto.Hash) testMDMProgram {
