...
type persistence struct {
	...
	RecentChange skymodules.ConsensusChangeID `json:"recentchange"`

	// Host Identity.
	Announced        bool                            `json:"announced"`
	AutoAddress      skymodules.NetAddress           `json:"autoaddress"`
	FinancialMetrics skymodules.HostFinancialMetrics `json:"financialmetrics"`
	PublicKey        types.SiaPublicKey              `json:"publickey"`
	RevisionNumber   uint64                          `json:"revisionnumber"`
	SecretKey        crypto.SecretKey                `json:"secretkey"`
	Settings         skymodules.HostInternalSettings `json:"settings"`
...
func (h *Host) establishDefaults() error {
	...
	h.settings = skymodules.HostInternalSettings{
		MaxDownloadBatchSize: uint64(skymodules.DefaultMaxDownloadBatchSize),
		MaxDuration:          skymodules.DefaultMaxDuration,
		MaxReviseBatchSize:   uint64(skymodules.DefaultMaxReviseBatchSize),
		WindowSize:           skymodules.DefaultWindowSize,

		Collateral:       skymodules.DefaultCollateral,
		CollateralBudget: defaultCollateralBudget,
		MaxCollateral:    skymodules.DefaultMaxCollateral,

		MinBaseRPCPrice:           skymodules.DefaultBaseRPCPrice,
		MinContractPrice:          skymodules.DefaultContractPrice,
		MinDownloadBandwidthPrice: skymodules.DefaultDownloadBandwidthPrice,
		MinSectorAccessPrice:      skymodules.DefaultSectorAccessPrice,
		MinStoragePrice:           skymodules.DefaultStoragePrice,
		MinUploadBandwidthPrice:   skymodules.DefaultUploadBandwidthPrice,

		EphemeralAccountExpiry:     skymodules.DefaultEphemeralAccountExpiry,
		MaxEphemeralAccountBalance: skymodules.DefaultMaxEphemeralAccountBalance,
...
func (h *Host) load() error {
	...
	p := new(persistence)
	err = h.dependencies.LoadFile(skymodules.Hostv151PersistMetadata, p, filepath.Join(h.persistDir, settingsFile))
	if err == nil {
...
func (h *Host) saveSync() error {
	return persist.SaveJSON(skymodules.Hostv151PersistMetadata, h.persistData(), filepath.Join(h.persistDir, settingsFile))
}
