...
func updateRunningCosts(pt *skymodules.RPCPriceTable, runningCost, runningRefund, runningCollateral types.Currency, runningMemory uint64, cost, refund, collateral types.Currency, memory, time uint64) (types.Currency, types.Currency, types.Currency, uint64) {
	runningMemory = runningMemory + memory
	memoryCost := skymodules.MDMMemoryCost(pt, runningMemory, time)
...
func TestExecuteProgramWriteDeadline(t *testing.T) {
	...
	pb := skymodules.NewProgramBuilder(pt, types.BlockHeight(fastrand.Uint64n(1000))) // random duration since ReadSector doesn't depend on duration.
...
func TestExecuteReadSectorProgram(t *testing.T) {
	...
	if uint64(len(resp.Output)) != skymodules.SectorSize {
		t.Fatalf("expected returned data to have length %v but was %v", skymodules.SectorSize, len(resp.Output))
	}
	...
	if err == nil || !strings.Contains(err.Error(), skymodules.ErrInsufficientBandwidthBudget.Error()) {
...
func TestExecuteReadPartialSectorProgram(t *testing.T) {
	...
	totalSegments := skymodules.SectorSize / crypto.SegmentSize
...
func TestExecuteHasSectorProgram(t *testing.T) {
	...
	pt := rhp.managedPriceTable()
	pb := skymodules.NewProgramBuilder(pt, types.BlockHeight(fastrand.Uint64n(1000))) // random duration since HasSector doesn't depend on duration.
...
func verifyBalance(am *accountManager, id skymodules.AccountID, expected types.Currency) error {
...
func TestVerifyExecuteProgramRevision(t *testing.T) {
	...
	curr := types.FileContractRevision{
		...
		NewFileSize:       fastrand.Uint64n(1000) * skymodules.SectorSize,
	...
	newFileSize := curr.NewFileSize + skymodules.SectorSize
...
func TestExecuteAppendProgram(t *testing.T) {
	...
	pt := rhp.managedPriceTable()
	pb := skymodules.NewProgramBuilder(pt, duration)
	...
	// programs that don't require a snapshot return a 0 contract size.
	if resp.NewSize != skymodules.SectorSize {
		t.Fatalf("expected contract size to stay the same: %v != %v", skymodules.SectorSize, resp.NewSize)
	}
	// programs that don't require a snapshot return a zero hash.
...
func TestExecuteUpdateRegistryProgram(t *testing.T) {
	...
	is.RegistrySize = 64 * skymodules.RegistryEntrySize
	...
	pt := rhp.managedPriceTable()
	pb := skymodules.NewProgramBuilder(pt, types.BlockHeight(fastrand.Uint64n(1000))) // random duration since UpdateRegistry doesn't depend on duration.
	...
	rv2 := skymodules.NewSignedRegistryValue(tweak, data2, rev2, sig2)
...
func TestExecuteReadRegistryProgram(t *testing.T) {
	...
	// Create the 'UpdateRegistry' program.
	pt := rhp.managedPriceTable()
	pb := skymodules.NewProgramBuilder(pt, types.BlockHeight(fastrand.Uint64n(1000))) // random duration since ReadRegistry doesn't depend on duration.
