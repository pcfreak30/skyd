...
func (s *rpcSession) readRequest(resp interface{}, maxLen uint64) error {
	return skymodules.ReadRPCRequest(s.conn, s.aead, resp, maxLen)
}
...
func (s *rpcSession) writeResponse(resp interface{}) error {
	return skymodules.WriteRPCResponse(s.conn, s.aead, resp, nil)
}

// writeError sends an encrypted RPC error to the renter.
func (s *rpcSession) writeError(err error) error {
	return skymodules.WriteRPCResponse(s.conn, s.aead, nil, err)
}
...
// error or the renter sends skymodules.RPCLoopExit.
func (h *Host) managedRPCLoop(conn net.Conn) error {
	...
	var req skymodules.LoopKeyExchangeRequest
	...
	for _, c := range req.Ciphers {
		if c == skymodules.CipherChaCha20Poly1305 {
			supportsChaCha = true
		}
	}
	if !supportsChaCha {
		encoding.NewEncoder(conn).Encode(skymodules.LoopKeyExchangeResponse{
			Cipher: skymodules.CipherNoOverlap,
		})
	...
	resp := skymodules.LoopKeyExchangeResponse{
		Cipher:    skymodules.CipherChaCha20Poly1305,
		PublicKey: xpk,
		Signature: pubkeySig[:],
	}
	...
	challengeReq := skymodules.LoopChallengeRequest{
		Challenge: s.challenge,
	}
	if err := skymodules.WriteRPCMessage(conn, aead, challengeReq); err != nil {
		return err
	}
	...
	rpcs := map[types.Specifier]func(*rpcSession) error{
		skymodules.RPCLoopLock:               h.managedRPCLoopLock,
		skymodules.RPCLoopUnlock:             h.managedRPCLoopUnlock,
		skymodules.RPCLoopSettings:           h.managedRPCLoopSettings,
		skymodules.RPCLoopFormContract:       h.managedRPCLoopFormContract,
		skymodules.RPCLoopRenewContract:      h.managedRPCLoopRenewContract,
		skymodules.RPCLoopRenewClearContract: h.managedRPCLoopRenewAndClearContract,
		skymodules.RPCLoopWrite:              h.managedRPCLoopWrite,
		skymodules.RPCLoopRead:               h.managedRPCLoopRead,
		skymodules.RPCLoopSectorRoots:        h.managedRPCLoopSectorRoots,
	}
	for {
		conn.SetDeadline(time.Now().Add(rpcRequestInterval))
		id, err := skymodules.ReadRPCID(conn, aead)
		if err != nil {
			...
			return err
		} else if id == skymodules.RPCLoopExit {
			return nil
		}
