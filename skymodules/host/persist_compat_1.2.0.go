...
var (
	...
	minimumStorageFolderSize = contractManagerStorageFolderGranularity * skymodules.SectorSize
...
func (h *Host) loadCompatV100(p *persistence) error {
	...
	err := h.dependencies.LoadFile(skymodules.Hostv112PersistMetadata, &compatPersistence, filepath.Join(h.persistDir, settingsFile))
	if err != nil {
		return err
	}
...
func (h *Host) upgradeFromV112ToV120() error {
	...
	for _, sector := range sectors {
		for i := 0; i < sector.Count; i++ {
			if uint64(len(sector.Data)) == skymodules.SectorSize {
				wg.Add(1)
	...
	for {
		...
		for _, cmFolder := range cmFolders {
			...
			if cmFolder.Capacity < finalCapacity-(skymodules.SectorSize*contractManagerStorageFolderGranularity) {
				canGrow++
			}
		}
		...
		for _, cmFolder := range cmFolders {
			finalCapacity := smFolderCapacities[cmFolder.Path]
			if cmFolder.Capacity < finalCapacity-(skymodules.SectorSize*contractManagerStorageFolderGranularity) {
				err := h.ResizeStorageFolder(cmFolder.Index, cmFolder.Capacity+(skymodules.SectorSize*contractManagerStorageFolderGranularity), false)
				if err != nil {
	...
	p := new(persistence)
	err = h.dependencies.LoadFile(skymodules.Hostv112PersistMetadata, p, filepath.Join(h.persistDir, settingsFile))
	if err != nil {
	...
	err = persist.SaveJSON(skymodules.Hostv120PersistMetadata, h.persistData(), filepath.Join(h.persistDir, settingsFile))
	if err != nil {
	...
	for _, cmFolder := range cmFolders {
		finalCapacity := smFolderCapacities[cmFolder.Path]
		finalCapacity -= finalCapacity % (skymodules.SectorSize * contractManagerStorageFolderGranularity)
