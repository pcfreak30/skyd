...
func (h *Host) managedRPCExecuteProgram(stream siamux.Stream) error {
	...
	// Read request
	var epr skymodules.RPCExecuteProgramRequest
	err = skymodules.RPCRead(stream, &epr)
	if err != nil {
	...
	program := skymodules.Program(instructions)
	...
	err = skymodules.RPCWrite(buffer, ct)
	if err != nil {
	...
	for output = range outputs {
		...
		resp := skymodules.RPCExecuteProgramResponse{
		...
		err = skymodules.RPCWrite(buffer, resp)
		if err != nil {
			return errors.AddContext(err, "failed to send output to peer")
		}

		instructionSpecifier := program[numOutputs-1].Specifier
		readInstruction := instructionSpecifier == skymodules.SpecifierReadOffset || instructionSpecifier == skymodules.SpecifierReadSector
		updateRegistryInstruction := instructionSpecifier == skymodules.SpecifierUpdateRegistry
		...
		err = stream.SetWriteDeadline(time.Now().Add(skymodules.MDMProgramWriteResponseTime))
		if err != nil {
		...
		if h.dependencies.Disrupt("MDMProgramOutputDelayWrite") {
			// add a write delay
			time.Sleep(skymodules.MDMProgramWriteResponseTime * 2)
		}
		...
		if output.Batch && buffer.Len() < skymodules.MDMMaxBatchBufferSize {
			continue
		}
...
func (h *Host) managedFinalizeWriteProgram(stream io.ReadWriter, fcid types.FileContractID, finalize mdm.FnFinalize, lastOutput mdm.Output, bh types.BlockHeight) error {
	...
	var req skymodules.RPCExecuteProgramRevisionSigningRequest
	err = skymodules.RPCRead(stream, &req)
	if err != nil {
	...
	resp := skymodules.RPCExecuteProgramRevisionSigningResponse{
		Signature: txn.TransactionSignatures[1].Signature,
	}
	err = skymodules.RPCWrite(stream, resp)
	if err != nil {
