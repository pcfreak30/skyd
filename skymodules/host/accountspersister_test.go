...
func TestAccountsReload(t *testing.T) {
	...
	accounts := make(map[skymodules.AccountID]types.Currency)
	var i uint64
	for i = 1; i <= 10; i++ {
...
func TestFingerprintBucketsRotate(t *testing.T) {
	...
	unsyncHost := func(h *Host, m skymodules.TestMiner, blocks int) error {
		err := h.Close()
		if err != nil {
			return err
		}
...
func reopenHost(ht *hostTester) error {
	return reopenCustomHost(ht, new(skymodules.ProductionDependencies))
}
...
func reopenCustomHost(ht *hostTester, deps skymodules.Dependencies) error {
	host, err := NewCustomHost(deps, ht.cs, ht.gateway, ht.tpool, ht.wallet, ht.mux, "localhost:0", filepath.Join(ht.persistDir, skymodules.HostDir))
	if err != nil {
		return err
	}
	ht.host = host
...
func getAccountBalance(am *accountManager, id skymodules.AccountID) types.Currency {
	am.mu.Lock()
	defer am.mu.Unlock()
