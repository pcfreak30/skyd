...
// module
func (so *storageObligation) StorageObligation() skymodules.StorageObligation {
	valid, missed := so.payouts()
	return skymodules.StorageObligation{
...
func (h *Host) managedModifyStorageObligation(so storageObligation, sectorsRemoved []crypto.Hash, sectorsGained map[crypto.Hash][]byte) error {
	// Sanity check - all of the sector data should be skymodules.SectorSize
	for _, data := range sectorsGained {
		if uint64(len(data)) != skymodules.SectorSize {
...
func (h *Host) resetFinancialMetrics() error {
	...
	fm := skymodules.HostFinancialMetrics{}
	err := h.db.View(func(tx *bolt.Tx) error {
...
func (h *Host) threadedHandleActionItem(soid types.FileContractID) {
	...
	if !so.OriginConfirmed {
		...
		if err != nil {
			...
			_, t := err.(skymodules.ConsensusConflict)
			if t {
...
func (h *Host) managedBuildStorageProof(so storageObligation, segmentIndex uint64) (types.StorageProof, error) {
	...
	sectorIndex := segmentIndex / (skymodules.SectorSize / crypto.SegmentSize)
	...
	sectorSegment := segmentIndex % (skymodules.SectorSize / crypto.SegmentSize)
...
func (h *Host) StorageObligations() (sos []skymodules.StorageObligation) {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Host) StorageObligation(obligationID types.FileContractID) (skymodules.StorageObligation, error) {
	so, err := h.managedGetStorageObligation(obligationID)
	if err != nil {
		return skymodules.StorageObligation{}, errors.AddContext(err, "failed to fetch storage obligation")
	}
