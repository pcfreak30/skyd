...
func (h *Host) upgradeFromV143ToV151() error {
	...
	p := new(persistence)
	err := h.dependencies.LoadFile(skymodules.Hostv143PersistMetadata, p, filepath.Join(h.persistDir, settingsFile))
	if err != nil {
	...
	if shouldResetEphemeralAccountExpiry(p.Settings) {
		p.Settings.EphemeralAccountExpiry = skymodules.DefaultEphemeralAccountExpiry // reset to default
	}
...
func shouldResetEphemeralAccountExpiry(his skymodules.HostInternalSettings) bool {
	return his.EphemeralAccountExpiry != skymodules.CompatV1412DefaultEphemeralAccountExpiry && his.EphemeralAccountExpiry.Nanoseconds() != 0
}
