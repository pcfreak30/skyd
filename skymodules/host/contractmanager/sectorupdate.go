...
func (wal *writeAheadLog) managedAddPhysicalSector(id sectorID, data []byte) error {
	// Sanity check - data should have skymodules.SectorSize bytes.
	if uint64(len(data)) != skymodules.SectorSize {
		wal.cm.log.Critical("sector has the wrong size", skymodules.SectorSize, len(data))
	...
	for len(storageFolders) >= 1 {
		...
		err := func() error {
			...
			if sf == nil {
				...
				// sector.
				wal.mu.Unlock()
				return errors.New(skymodules.V1420HostOutOfStorageErrString)
			}
			defer sf.mu.RUnlock()
...
func (wal *writeAheadLog) managedAddVirtualSector(id sectorID, location sectorLocation) error {
	...
	if location.count == math.MaxUint64 {
		return skymodules.ErrMaxVirtualSectors
	}
	location.count++
...
func (cm *ContractManager) AddSector(root crypto.Hash, sectorData []byte) error {
	...
	defer func() {
		if registerHostDiskTrouble {
			cm.staticAlerter.RegisterAlert(skymodules.AlertIDHostDiskTrouble, AlertMSGHostDiskTrouble, "", skymodules.SeverityCritical)
		}
	}()
