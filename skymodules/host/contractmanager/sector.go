...
func readPartialSector(f skymodules.File, sectorIndex uint32, offset, length uint64) ([]byte, error) {
	if offset+length > skymodules.SectorSize {
		return nil, errors.New("readPartialSector: read is out of bounds")
	}
	b := make([]byte, length)
	_, err := f.ReadAt(b, int64(uint64(sectorIndex)*skymodules.SectorSize+offset))
	if err != nil {
...
// location.
func readSector(f skymodules.File, sectorIndex uint32) ([]byte, error) {
	return readPartialSector(f, sectorIndex, 0, skymodules.SectorSize)
}

// readFullMetadata will read a full sector metadata file into memory.
func readFullMetadata(f skymodules.File, numSectors int) ([]byte, error) {
...
// index.
func writeSector(f skymodules.File, sectorIndex uint32, data []byte) error {
	_, err := f.WriteAt(data, int64(uint64(sectorIndex)*skymodules.SectorSize))
	if err != nil {
...
// to disk.
func writeSectorMetadata(f skymodules.File, sectorIndex uint32, id sectorID, count uint16) error {
...
func (cm *ContractManager) ReadSector(root crypto.Hash) ([]byte, error) {
	return cm.ReadPartialSector(root, 0, skymodules.SectorSize)
}
