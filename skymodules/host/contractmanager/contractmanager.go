...
func newContractManager(dependencies skymodules.Dependencies, persistDir string) (_ *ContractManager, err error) {
	cm := &ContractManager{
		...
		staticAlerter: skymodules.NewAlerter("contractmanager"),
	}
	cm.wal.cm = cm
...
func New(persistDir string) (*ContractManager, error) {
	return newContractManager(new(skymodules.ProductionDependencies), persistDir)
}

// NewCustomContractManager returns a ContractManager with custom dependencies.
func NewCustomContractManager(dependencies skymodules.Dependencies, persistDir string) (*ContractManager, error) {
	return newContractManager(dependencies, persistDir)
}

// Alerts implements the skymodules.Alerter interface for the contract manager
func (cm *ContractManager) Alerts() (crit, err, warn []skymodules.Alert) {
