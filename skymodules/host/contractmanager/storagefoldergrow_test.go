...
func TestGrowStorageFolder(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderOne, skymodules.SectorSize*storageFolderGranularity)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity {
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity {
...
func (dig *dependencyIncompleteGrow) CreateFile(s string) (skymodules.File, error) {
...
func TestGrowStorageFolderIncompleteWrite(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderOne, skymodules.SectorSize*storageFolderGranularity*3)
	if err != nil {
		t.Fatal(err)
	}
	...
	sfIndex := sfs[0].Index
	// Verify that the storage folder has the correct capacity.
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*3 {
		t.Error("new storage folder is reporting the wrong capacity")
	}
	...
	// fail.
	err = cmt.cm.ResizeStorageFolder(sfIndex, skymodules.SectorSize*storageFolderGranularity*25, false)
	if err == nil {
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity*3 {
		t.Error("sector file is the wrong size:", sfi.Size(), skymodules.SectorSize*storageFolderGranularity*3)
	}
...
func TestGrowStorageFolderWAL(t *testing.T) {
	...
	// Verify that the storage folder has the correct capacity.
	sfs = cmt.cm.StorageFolders()
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*25 {
		t.Error("new storage folder is reporting the wrong capacity", sfs[0].Capacity/skymodules.SectorSize, storageFolderGranularity*25)
	}
	// Verify that the on-disk files are the right size.
	mfn := filepath.Join(storageFolderOne, metadataFile)
	sfn := filepath.Join(storageFolderOne, sectorFile)
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity*25 {
		t.Error("sector file is the wrong size:", sfi.Size(), skymodules.SectorSize*storageFolderGranularity*25)
	}
}
