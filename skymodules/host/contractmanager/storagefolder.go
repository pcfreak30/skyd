...
var (
	...
	minFolderSize = MinimumSectorsPerStorageFolder * skymodules.SectorSize
	...
	ErrSmallStorageFolder = fmt.Errorf("minimum allowed size for a storage folder is %v (%v bytes)", skymodules.FilesizeUnits(minFolderSize), minFolderSize)

	maxFolderSize = MaximumSectorsPerStorageFolder * skymodules.SectorSize
	...
	ErrLargeStorageFolder = fmt.Errorf("maximum allowed size for a storage folder is %v (%v bytes)", skymodules.FilesizeUnits(maxFolderSize), maxFolderSize)
...
type storageFolder struct {
	...
	metadataFile skymodules.File
	sectorFile   skymodules.File
}
...
func (cm *ContractManager) ResizeStorageFolder(index uint16, newSize uint64, force bool) error {
	...
	if newSize/skymodules.SectorSize < MinimumSectorsPerStorageFolder {
		return ErrSmallStorageFolder
	}
	if newSize/skymodules.SectorSize > MaximumSectorsPerStorageFolder {
		return ErrLargeStorageFolder
	}

	oldSize := uint64(len(sf.usage)) * storageFolderGranularity * skymodules.SectorSize
	if oldSize == newSize {
		return ErrNoResize
	}
	newSectorCount := uint32(newSize / skymodules.SectorSize)
	if oldSize > newSize {
...
func (cm *ContractManager) StorageFolders() []skymodules.StorageFolderMetadata {
	err := cm.tg.Add()
	if err != nil {
		return nil
	}
	...
	var smfs []skymodules.StorageFolderMetadata
	for _, sf := range cm.storageFolders {
		// Grab the non-computational data.
		sfm := skymodules.StorageFolderMetadata{
			...
			Capacity:          skymodules.SectorSize * 64 * uint64(len(sf.usage)),
			CapacityRemaining: ((64 * uint64(len(sf.usage))) - sf.sectors) * skymodules.SectorSize,
			Index:             sf.index,
			Path:              sf.path,
		}
