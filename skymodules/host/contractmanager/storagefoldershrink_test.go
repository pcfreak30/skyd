...
func TestShrinkStorageFolder(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderOne, skymodules.SectorSize*storageFolderGranularity*8)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*8 {
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity*8 {
		t.Error("sector file is the wrong size")
	}

	// Decrease the size of the storage folder.
	err = cmt.cm.ResizeStorageFolder(sfIndex, skymodules.SectorSize*storageFolderGranularity*2, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity*2 {
		t.Error("sector file is the wrong size")
	}
...
func TestShrinkStorageFolderWithSectors(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderTwo, skymodules.SectorSize*storageFolderGranularity*3)
	if err != nil {
		t.Fatal(err)
	}
	...
	if capacity != skymodules.SectorSize*storageFolderGranularity*5 {
		t.Error("new storage folder is reporting the wrong capacity")
	}
	if capacityRemaining != skymodules.SectorSize*storageFolderGranularity*2 {
...
func TestShrinkStorageFolderIncompleteWrite(t *testing.T) {
	...
	capacity := sfs[0].Capacity + sfs[1].Capacity
	capacityRemaining := sfs[0].CapacityRemaining + sfs[1].CapacityRemaining
	if capacity != skymodules.SectorSize*storageFolderGranularity*11 {
		t.Error("new storage folder is reporting the wrong capacity")
	}
	if capacityRemaining != skymodules.SectorSize*storageFolderGranularity*8 {
		t.Error("new storage folder capacity remaining is reporting the wrong remaining capacity")
	}
	mfn := filepath.Join(storageFolderOne, metadataFile)
	sfn := filepath.Join(storageFolderOne, sectorFile)
...
func TestShrinkStorageFolderIncompleteWriteForce(t *testing.T) {
	...
	// Decrease the size of the storage folder.
	err = cmt.cm.ResizeStorageFolder(sfIndex, skymodules.SectorSize*storageFolderGranularity*2, true)
	if err != nil {
		t.Fatal("expected a failure")
	}
	// Verify that the capacity and file sizes are correct.
	...
	// sectors.
	if capacityRemaining != capacity-remainingSectors*skymodules.SectorSize {
		t.Error(capacityRemaining/skymodules.SectorSize, capacity/skymodules.SectorSize, remainingSectors)
	}
...
func TestShrinkStorageFolderShutdownAfterMove(t *testing.T) {
	...
	capacityRemaining = sfs[0].CapacityRemaining + sfs[1].CapacityRemaining
	if capacity != skymodules.SectorSize*storageFolderGranularity*11 {
		t.Error("new storage folder is reporting the wrong capacity", capacity/skymodules.SectorSize, storageFolderGranularity*11)
	}
	if capacityRemaining != skymodules.SectorSize*storageFolderGranularity*8 {
		t.Error("new storage folder capacity remaining is reporting the wrong remaining capacity")
	}
...
func TestShrinkSingleStorageFolder(t *testing.T) {
	...
	// Decrease the size of the storage folder.
	err = cmt.cm.ResizeStorageFolder(sfIndex, skymodules.SectorSize*storageFolderGranularity*4, false)
	if err != nil {
		t.Fatal(err)
	}
	// Verify that the capacity and file sizes are correct.
	...
	if sfs[0].CapacityRemaining != skymodules.SectorSize*storageFolderGranularity*1 {
		t.Error("new storage folder capacity remaining is reporting the wrong remaining capacity")
	}
	mfi, err := os.Stat(mfn)
	if err != nil {
	...
	if uint64(sfi.Size()) != skymodules.SectorSize*storageFolderGranularity*4 {
		t.Error("sector file is the wrong size")
	}
