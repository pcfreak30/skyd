...
func TestRemoveStorageFolderConcurrentAddSector(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderTwo, skymodules.SectorSize*storageFolderGranularity*15)
	if err != nil {
		t.Fatal(err)
	}
	sfs := cmt.cm.StorageFolders()
	...
	err = cmt.cm.AddStorageFolder(storageFolderThree, skymodules.SectorSize*storageFolderGranularity*25)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = cmt.cm.AddStorageFolder(storageFolderFour, skymodules.SectorSize*storageFolderGranularity*50)
	if err != nil {
		t.Fatal(err)
	}
	...
	if totalConsumed != uint64(len(roots))*skymodules.SectorSize {
