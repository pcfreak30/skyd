...
func newContractManagerTester(name string) (*contractManagerTester, error) {
	...
	testdir := build.TempDir(skymodules.ContractManagerDir, name)
	cm, err := New(filepath.Join(testdir, skymodules.ContractManagerDir))
	if err != nil {
		return nil, err
	}
...
func newMockedContractManagerTester(d skymodules.Dependencies, name string) (*contractManagerTester, error) {
	if testing.Short() {
		panic("use of newContractManagerTester during short testing")
	}

	testdir := build.TempDir(skymodules.ContractManagerDir, name)
	cm, err := newContractManager(d, filepath.Join(testdir, skymodules.ContractManagerDir))
	if err != nil {
		return nil, err
	}
	cmt := &contractManagerTester{
...
func TestNewContractManager(t *testing.T) {
	...
	parentDir := build.TempDir(skymodules.ContractManagerDir, "TestNewContractManager")
	cmDir := filepath.Join(parentDir, skymodules.ContractManagerDir)
	cm, err := New(cmDir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewContractManagerErroredStartup(t *testing.T) {
	...
	testdir := build.TempDir(skymodules.ContractManagerDir, "TestNewContractManagerErroredStartup")
	cmd := filepath.Join(testdir, skymodules.ContractManagerDir)
