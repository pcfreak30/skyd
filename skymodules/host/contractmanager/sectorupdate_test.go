...
func TestAddSector(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir, skymodules.SectorSize*64)
	if err != nil {
		t.Fatal(err)
	}
	...
	partialSectorData, err := cmt.cm.ReadPartialSector(root, 0, skymodules.SectorSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = cmt.cm.ReadPartialSector(root, 0, skymodules.SectorSize+1)
	if err == nil {
		t.Fatal("ReadPartialSector should fail")
	}
}
...
func TestAddSectorFillFolder(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir, skymodules.SectorSize*storageFolderGranularity)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddSectorRecovery(t *testing.T) {
	...
	if sfs[0].Capacity != sfs[0].CapacityRemaining+skymodules.SectorSize {
		t.Error("One sector's worth of capacity should be consumed:", (sfs[0].Capacity-sfs[0].CapacityRemaining)/skymodules.SectorSize)
	}
	// Break the rules slightly - make the test brittle by looking at the
	// internals directly to determine that the sector got added to the right
	// locations, and that the usage information was updated correctly.
...
func TestDeleteSector(t *testing.T) {
	...
	if sfs[0].Capacity != sfs[0].CapacityRemaining+skymodules.SectorSize {
		t.Error("One sector's worth of capacity should be consumed:", sfs[0].Capacity/skymodules.SectorSize, sfs[0].CapacityRemaining/skymodules.SectorSize)
	}
	// Break the rules slightly - make the test brittle by looking at the
	// internals directly to determine that the sector got added to the right
	// locations, and that the usage information was updated correctly.
...
func TestSectorBalancing(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir2, skymodules.SectorSize*64)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir3, skymodules.SectorSize*64*2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sfs[0].Capacity+sfs[1].Capacity+sfs[2].Capacity != sfs[0].CapacityRemaining+sfs[1].CapacityRemaining+sfs[2].CapacityRemaining+skymodules.SectorSize*20 {
...
func (d *dependencyFailingWrites) CreateFile(s string) (skymodules.File, error) {
...
func TestFailingStorageFolder(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir, skymodules.SectorSize*64*2)
	if err != nil {
		t.Fatal(err)
	}
	// Add a second storage folder.
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir2, skymodules.SectorSize*64*2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sfs[0].Capacity+sfs[1].Capacity != sfs[0].CapacityRemaining+sfs[1].CapacityRemaining+skymodules.SectorSize*50 {
		t.Error("expecting 20 sectors consumed:", sfs[0].Capacity+sfs[1].Capacity, sfs[0].CapacityRemaining+sfs[1].CapacityRemaining-skymodules.SectorSize*50)
	}
	// Break the rules slightly - make the test brittle by looking at the
	// internals directly to determine that the sector got added to the right
	// locations, and that the usage information was updated correctly.
	...
	*d.triggered = true
	...
	if strings.Contains(sfs[0].Path, "storageFolderTwo") {
		...
		// sectors.
		if sfs[0].CapacityRemaining+skymodules.SectorSize*50 >= sfs[0].Capacity {
			t.Error("expecting more than 50 sectors in sfs0")
		}
		if sfs[1].CapacityRemaining+skymodules.SectorSize*50 <= sfs[1].Capacity {
...
func TestAddVirtualSectorOverflow(t *testing.T) {
	...
	loaded, err := newOverflowMap(overflowFilePath, skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
