...
func TestLoadMissingStorageFolder(t *testing.T) {
	...
	err = cmt.cm.AddStorageFolder(storageFolderDir, skymodules.SectorSize*storageFolderGranularity*2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*2 {
	...
	if sfs[0].Capacity != sfs[0].CapacityRemaining+skymodules.SectorSize {
	...
	cmt.cm, err = newContractManager(d, filepath.Join(cmt.persistDir, skymodules.ContractManagerDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	err = cmt.cm.AddStorageFolder(storageFolderTwo, skymodules.SectorSize*storageFolderGranularity*2)
	if err != nil {
		t.Fatal(err)
	}
	// Add a sector to the storage folder.
	...
	for i := range sfs {
		if sfs[i].Capacity != sfs[i].CapacityRemaining+skymodules.SectorSize {
	...
	if sfs[0].Capacity != sfs[0].CapacityRemaining+skymodules.SectorSize*3 && sfs[1].Capacity != sfs[1].CapacityRemaining+skymodules.SectorSize*3 {
		t.Error("One sector's worth of capacity should be consumed")
	}

	// Try to shrink the missing storage folder.
	err = cmt.cm.ResizeStorageFolder(sfOneIndex, skymodules.SectorSize*storageFolderGranularity, false)
	if err == nil {
		t.Fatal("should not be able to resize a missing storage folder")
	}
	err = cmt.cm.ResizeStorageFolder(sfOneIndex, skymodules.SectorSize*storageFolderGranularity, true)
	if err == nil {
		t.Fatal("should not be able to resize a missing storage folder")
	}
	...
	if sfs[0].Index == sfOneIndex && sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*2 {
		t.Error("Storage folder has wrong size after failing to resize")
	}
	if sfs[1].Index == sfOneIndex && sfs[1].Capacity != skymodules.SectorSize*storageFolderGranularity*2 {
		t.Error("Storage folder has wrong size after failing to resize")
	}

	// Try to grow the missing storage folder.
	err = cmt.cm.ResizeStorageFolder(sfOneIndex, skymodules.SectorSize*storageFolderGranularity*4, false)
	if err == nil {
		t.Fatal("should not be able to resize a missing storage folder")
	}
	err = cmt.cm.ResizeStorageFolder(sfOneIndex, skymodules.SectorSize*storageFolderGranularity*4, true)
	if err == nil {
		t.Fatal("should not be able to resize a missing storage folder")
	}
	...
	// sectors.
	err = cmt.cm.ResizeStorageFolder(sfTwoIndex, skymodules.SectorSize*storageFolderGranularity*4, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = cmt.cm.AddStorageFolder(storageFolderThree, skymodules.SectorSize*storageFolderGranularity)
	if err != nil {
		t.Fatal(err)
	}
	// Shrink the second storage folder such that some of the sectors are forced
	// to move.
	err = cmt.cm.ResizeStorageFolder(sfTwoIndex, skymodules.SectorSize*storageFolderGranularity*3, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = cmt.cm.ResizeStorageFolder(sfTwoIndex, skymodules.SectorSize*storageFolderGranularity*2, false)
	if err == nil {
	...
	// Shrink the second storage folder again, such that there is not enough
	// room in the available storage folders to accept the data.
	err = cmt.cm.ResizeStorageFolder(sfTwoIndex, skymodules.SectorSize*storageFolderGranularity, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	var sfOne skymodules.StorageFolderMetadata
	for _, sf := range sfs {
	...
	if sfOne.Capacity != sfOne.CapacityRemaining+skymodules.SectorSize {
		cmt.cm.wal.mu.Lock()
...
func TestFolderRechecker(t *testing.T) {
	...
	// Re-open the contract manager.
	cmt.cm, err = New(filepath.Join(cmt.persistDir, skymodules.ContractManagerDir))
	if err != nil {
		t.Fatal(err)
	}
	// The contract manager should still be reporting the storage folder, but
	...
	err = cmt.cm.ResizeStorageFolder(sfs[0].Index, skymodules.SectorSize*storageFolderGranularity*4, false)
	if err != nil {
		t.Fatal(err)
	}
	sfs = cmt.cm.StorageFolders()
	...
	if sfs[0].Capacity != sfs[0].CapacityRemaining+skymodules.SectorSize*2 {
		t.Error("One sector's worth of capacity should be consumed:", sfs[0].Capacity, sfs[0].CapacityRemaining)
	}
	if sfs[0].Capacity != skymodules.SectorSize*storageFolderGranularity*4 {
