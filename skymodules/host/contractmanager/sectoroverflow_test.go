...
func testDir(testName string) string {
	path := build.TempDir(skymodules.ContractManagerDir, testName)
...
func TestNewOverflowFile(t *testing.T) {
	...
	// Create the file.
	f, err := newOverflowMap(filePath, skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = newOverflowMap(filePath, skymodules.ProdDependencies)
...
func TestSetOverflow(t *testing.T) {
	...
	f2, err := newOverflowMap(filePath, skymodules.ProdDependencies)
	if err != nil {
		t.Fatal(err)
	}
