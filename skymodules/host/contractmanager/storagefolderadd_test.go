...
func (*dependencyLargeFolder) CreateFile(s string) (skymodules.File, error) {
...
func TestAddLargeStorageFolder(t *testing.T) {
	...
	addErr := cmt.cm.AddStorageFolder(storageFolderDir, skymodules.SectorSize*storageFolderGranularity*16) // Total size must exceed the limit of the limitFile.
...
func TestAddStorageFolderConcurrent(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		err := cmt.cm.AddStorageFolder(storageFolderTwo, skymodules.SectorSize*storageFolderGranularity*8)
		if err != nil {
			t.Error(err)
		}
	}()
	go func() {
		defer wg.Done()
		err = cmt.cm.AddStorageFolder(storageFolderThree, skymodules.SectorSize*storageFolderGranularity*8)
		if err != nil {
			t.Error(err)
		}
	}()
...
func (d *dependencyBlockSFOne) CreateFile(s string) (skymodules.File, error) {
...
func TestAddStorageFolderBlocking(t *testing.T) {
	...
	sfOneSize := skymodules.SectorSize * storageFolderGranularity * 8
	go func() {
...
func TestAddStorageFolderConsecutive(t *testing.T) {
	...
	// Spin off the first goroutine, and then wait until write has been called
	// on the underlying file.
	sfSize := skymodules.SectorSize * storageFolderGranularity * 8
...
func TestAddStorageFolderReload(t *testing.T) {
	...
	sfSize := skymodules.SectorSize * storageFolderGranularity * 24
	err = cmt.cm.AddStorageFolder(storageFolderOne, sfSize)
	if err != nil {
		t.Fatal(err)
	}
