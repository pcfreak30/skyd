...
func (h *Host) threadedTrackWorkingStatus(closeChan chan struct{}) {
	...
	if settingsCalls-prevSettingsCalls >= workingStatusThreshold {
		h.workingStatus = skymodules.HostWorkingStatusWorking
	}
	...
	for {
		...
		if settingsCalls-prevSettingsCalls >= workingStatusThreshold {
			...
			h.workingStatus = skymodules.HostWorkingStatusNotWorking
		}
		h.mu.Unlock()
	}
}
...
func (h *Host) threadedTrackConnectabilityStatus(closeChan chan struct{}) {
	...
	for {
		...
		var status skymodules.HostConnectabilityStatus
		if err != nil {
			status = skymodules.HostConnectabilityStatusNotConnectable
		} else {
			conn.Close()
			status = skymodules.HostConnectabilityStatusConnectable
		}
		h.mu.Lock()
...
func (h *Host) initNetworking(address string) (err error) {
	...
	h.workingStatus = skymodules.HostWorkingStatusChecking

	// Set the initial connectability state of the host
	h.connectabilityStatus = skymodules.HostConnectabilityStatusChecking
	...
	if build.Release == "testing" {
		...
		h.autoAddress = skymodules.NetAddress(net.JoinHostPort("localhost", h.port))
	}
	...
	if !h.dependencies.Disrupt("DisableHostSiamux") {
		err = h.staticMux.NewListener(skymodules.HostSiaMuxSubscriberName, h.threadedHandleStream)
		if err != nil {
			return errors.AddContext(err, "Failed to subscribe to the SiaMux")
		}
		// Close the listener when h.tg.OnStop is called.
		h.tg.OnStop(func() {
			h.staticMux.CloseListener(skymodules.HostSiaMuxSubscriberName)
		})
	}
...
func (h *Host) threadedHandleStream(stream siamux.Stream) {
	...
	err = skymodules.RPCRead(stream, &rpcID)
	if err != nil {
		err = errors.AddContext(err, "Failed to read RPC id")
		if wErr := skymodules.RPCWriteError(stream, err); wErr != nil {
	...
	if err != nil {
		err = errors.Compose(err, skymodules.RPCWriteError(stream, err))
		atomic.AddUint64(&h.atomicErroredCalls, 1)
		h.managedLogError(err)
	}
}
...
func (h *Host) NetAddress() skymodules.NetAddress {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Host) NetworkMetrics() skymodules.HostNetworkMetrics {
	h.mu.RLock()
	defer h.mu.RUnlock()
	return skymodules.HostNetworkMetrics{
