...
func (h *Host) managedRPCLoopSettings(s *rpcSession) error {
	...
	s.extendDeadline(skymodules.NegotiateSettingsTime)
	...
	resp := skymodules.LoopSettingsResponse{
		Settings: js,
	}
...
func (h *Host) managedRPCLoopLock(s *rpcSession) error {
	s.extendDeadline(skymodules.NegotiateRecentRevisionTime)
	...
	// Read the request.
	var req skymodules.LoopLockRequest
	if err := s.readRequest(&req, skymodules.RPCMinLen); err != nil {
	...
	if err != nil || h.dependencies.Disrupt("loopLockNoRecordOfThatContract") {
		s.writeError(errors.New(skymodules.V1420ContractNotRecognizedErrString))
	...
	hash := crypto.HashAll(skymodules.RPCChallengePrefix, challenge)
	...
	// Write the response.
	resp := skymodules.LoopLockResponse{
...
func (h *Host) managedRPCLoopWrite(s *rpcSession) error {
	s.extendDeadline(skymodules.NegotiateFileContractRevisionTime)
	// Read the request.
	var req skymodules.LoopWriteRequest
	if err := s.readRequest(&req, skymodules.SectorSize*5); err != nil {
	...
	// sent immediately.
	var sigResponse skymodules.LoopWriteResponse
	if !req.MerkleProof {
		if err := s.readResponse(&sigResponse, skymodules.RPCMinLen); err != nil {
			return err
		}
	}
	...
	for _, action := range req.Actions {
		...
		case skymodules.WriteActionAppend:
			if uint64(len(action.Data)) != skymodules.SectorSize {
			...
			// Update finances
			bandwidthRevenue = bandwidthRevenue.Add(settings.UploadBandwidthPrice.Mul64(skymodules.SectorSize))
		...
		case skymodules.WriteActionUpdate:
			...
			if sectorIndex >= uint64(len(newRoots)) {
				...
				err = errors.Compose(err, s.writeError(err))
				return err
			} else if offset+uint64(len(action.Data)) > skymodules.SectorSize {
	...
	if len(newRoots) > len(s.so.SectorRoots) {
		bytesAdded := skymodules.SectorSize * uint64(len(newRoots)-len(s.so.SectorRoots))
	...
	if req.MerkleProof {
		...
		merkleResp = skymodules.LoopWriteMerkleProof{
		...
		if proofSize < skymodules.RPCMinLen {
			proofSize = skymodules.RPCMinLen
		}
	...
	for _, action := range req.Actions {
		if action.Type == skymodules.WriteActionAppend {
			newRevision.NewFileSize += skymodules.SectorSize
		} else if action.Type == skymodules.WriteActionTrim {
			newRevision.NewFileSize -= skymodules.SectorSize * action.A
		}
	}
	...
	if req.MerkleProof {
		if err := s.writeResponse(merkleResp); err != nil {
			return err
		} else if err := s.readResponse(&sigResponse, skymodules.RPCMinLen); err != nil {
			return err
		}
	}
	...
	// Send the response.
	resp := skymodules.LoopWriteResponse{
...
func (h *Host) managedRPCLoopRead(s *rpcSession) error {
	s.extendDeadline(skymodules.NegotiateDownloadTime)

	// Read the request.
	var req skymodules.LoopReadRequest
	if err := s.readRequest(&req, skymodules.RPCMinLen); err != nil {
		// Reading may have failed due to a closed connection; regardless, it
		// doesn't hurt to try and tell the renter about it.
		err = errors.Compose(err, s.writeError(err))
	...
	go func() {
		...
		err := s.readResponse(&id, skymodules.RPCMinLen)
		if err != nil {
			stopSignal <- err
		} else if id != skymodules.RPCLoopReadStop {
	...
	for _, sec := range req.Sections {
		var err error
		switch {
		case uint64(sec.Offset)+uint64(sec.Length) > skymodules.SectorSize:
	...
	for i, sec := range req.Sections {
		...
		resp := skymodules.LoopReadResponse{
			Signature:   nil,
			Data:        data,
			MerkleProof: proof,
		}
...
func (h *Host) managedRPCLoopFormContract(s *rpcSession) error {
	...
	s.extendDeadline(skymodules.NegotiateFileContractTime)

	// Read the contract request.
	var req skymodules.LoopFormContractRequest
	if err := s.readRequest(&req, skymodules.TransactionSetSizeLimit); err != nil {
		err = errors.Compose(err, s.writeError(err))
		return err
	}
	...
	resp := skymodules.LoopContractAdditions{
		Parents: newParents,
		Inputs:  newInputs,
		Outputs: newOutputs,
	}
	...
	// revision.
	var renterSigs skymodules.LoopContractSignatures
	if err := s.readResponse(&renterSigs, skymodules.RPCMinLen); err != nil {
		err = errors.Compose(err, s.writeError(err))
		return err
	}
	...
	hostSigs := skymodules.LoopContractSignatures{
...
func (h *Host) managedRPCLoopRenewContract(s *rpcSession) error {
	// NOTE: this RPC contains two request/response exchanges.
	s.extendDeadline(skymodules.NegotiateRenewContractTime)

	// Read the renewal request.
	var req skymodules.LoopRenewContractRequest
	if err := s.readRequest(&req, skymodules.TransactionSetSizeLimit); err != nil {
		err = errors.Compose(err, s.writeError(err))
		return err
	}
...
func (h *Host) managedRPCLoopSectorRoots(s *rpcSession) error {
	...
	// Read the request.
	var req skymodules.LoopSectorRootsRequest
	if err := s.readRequest(&req, skymodules.RPCMinLen); err != nil {
		// Reading may have failed due to a closed connection; regardless, it
		// doesn't hurt to try and tell the renter about it.
		err = errors.Compose(err, s.writeError(err))
	...
	if responseSize < skymodules.RPCMinLen {
		responseSize = skymodules.RPCMinLen
	}
	...
	// send the response
	resp := skymodules.LoopSectorRootsResponse{
		Signature:   txn.TransactionSignatures[1].Signature,
...
func (h *Host) managedRPCLoopRenewAndClearContract(s *rpcSession) error {
	...
	// Read the renewal request.
	var req skymodules.LoopRenewAndClearContractRequest
	if err := s.readRequest(&req, skymodules.TransactionSetSizeLimit); err != nil {
		err = errors.Compose(err, s.writeError(err))
		return err
	}
	...
	// transaction and a signature for the implicit no-op file contract
	// revision.
	var renterSigs skymodules.LoopRenewAndClearContractSignatures
	if err := s.readResponse(&renterSigs, skymodules.RPCMinLen); err != nil {
		err = errors.Compose(err, s.writeError(err))
		return err
	}
	...
	// Send our signatures for the contract transaction and initial revision.
	hostSigs := skymodules.LoopRenewAndClearContractSignatures{
		ContractSignatures: hostTxnSignatures,
		RevisionSignature:  hostRevisionSignature,
