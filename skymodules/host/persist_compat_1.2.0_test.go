...
func loadExistingHostWithNewDeps(modulesDir, siaMuxDir, hostDir string) (closeFn, skymodules.Host, error) {
	// Create the siamux
	mux, err := skymodules.NewSiaMux(siaMuxDir, modulesDir, "localhost:0", "localhost:0")
	if err != nil {
		return nil, nil, err
	}

	// Create the host dependencies.
	g, err := gateway.New("localhost:0", false, filepath.Join(modulesDir, skymodules.GatewayDir))
	if err != nil {
		return nil, nil, err
	}
	cs, errChan := consensus.New(g, false, filepath.Join(modulesDir, skymodules.ConsensusDir))
	if err := <-errChan; err != nil {
		return nil, nil, err
	}
	tp, err := transactionpool.New(cs, g, filepath.Join(modulesDir, skymodules.TransactionPoolDir))
	if err != nil {
		return nil, nil, err
	}
	w, err := wallet.New(cs, tp, filepath.Join(modulesDir, skymodules.WalletDir))
	if err != nil {
		return nil, nil, err
	}

	// Create the host.
	h, err := NewCustomHost(skymodules.ProdDependencies, cs, g, tp, w, mux, "localhost:0", hostDir)
	if err != nil {
		return nil, nil, err
	}
...
func TestV112StorageManagerUpgrade(t *testing.T) {
	...
	legacyHost := build.TempDir(skymodules.HostDir, t.Name(), skymodules.HostDir)
	...
	modulesDir := build.TempDir(skymodules.HostDir, testDir)
	...
	dst := filepath.Join(modulesDir, skymodules.HostDir, settingsFile)
	dstDir := filepath.Join(modulesDir, skymodules.HostDir)
	...
	siaMuxDir := filepath.Join(modulesDir, skymodules.SiaMuxDir)
	...
	if capacityRemaining != oldCapacityRemaining {
		t.Error("capacity remaining statistics do not match up", capacityRemaining/skymodules.SectorSize, oldCapacityRemaining/skymodules.SectorSize)
	}
	if oldUsed != capacityUsed {
		t.Error("storage folders have different usage values", capacityUsed/skymodules.SectorSize, oldUsed/skymodules.SectorSize)
	}
}
