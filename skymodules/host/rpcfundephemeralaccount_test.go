...
func TestFundEphemeralAccountRPC(t *testing.T) {
	...
	runWithRequest := func(req skymodules.PayByContractRequest) (*skymodules.PayByContractResponse, *skymodules.FundAccountResponse, error) {
		...
		far := skymodules.FundAccountRequest{Account: pair.staticAccountID}
		err = skymodules.RPCWrite(stream, far)
		if err != nil {
			return nil, nil, err
		}
		...
		err = skymodules.RPCWriteAll(stream, pRequest, req)
		if err != nil {
			return nil, nil, err
		}
	...
	verifyResponse := func(rev types.FileContractRevision, payByResponse *skymodules.PayByContractResponse, fundResponse *skymodules.FundAccountResponse, prevBalance, prevPotAccFunding, funding types.Currency) error {
