...
func assertSubscriptionInfos(host *Host, spk types.SiaPublicKey, tweak crypto.Hash, n int) ([]*subscriptionInfo, error) {
	sid := skymodules.RegistrySubscriptionID(spk, tweak)
	host.staticRegistrySubscriptions.mu.Lock()
...
func readAndAssertRegistryValueNotification(spk types.SiaPublicKey, rv skymodules.SignedRegistryValue, r io.Reader) error {
	var snt skymodules.RPCRegistrySubscriptionNotificationType
	err := skymodules.RPCRead(r, &snt)
	if err != nil {
		return err
	}
	if snt.Type != skymodules.SubscriptionResponseRegistryValue {
		return errors.New("notification has wrong type")
	}
	var sneu skymodules.RPCRegistrySubscriptionNotificationEntryUpdate
	err = skymodules.RPCRead(r, &sneu)
	if err != nil {
		return err
	}
...
func readAndAssertOkResponse(r io.Reader) error {
	...
	if snt.Type != skymodules.SubscriptionResponseSubscriptionSuccess {
		return errors.New("notification has wrong type")
	}
	return nil
}
...
func TestRPCSubscribe(t *testing.T) {
	...
	is.RegistrySize += (skymodules.RegistryEntrySize * 100)
...
func testRPCSubscribeBasic(t *testing.T, rhp *renterHostPair) {
	...
	expectedBalance := skymodules.DefaultHostExternalSettings().MaxEphemeralAccountBalance
	...
	runningCost := skymodules.MDMSubscribeCost(pt, 1, 1)
	...
	runningCost = runningCost.Add(skymodules.MDMSubscriptionMemoryCost(pt, 1))
...
func testRPCSubscribeBeforeAvailable(t *testing.T, rhp *renterHostPair) {
	...
	runningCost := skymodules.MDMSubscribeCost(pt, 0, 1)
...
func testRPCSubscribeSessionTimeout(t *testing.T, rhp *renterHostPair) {
	...
	time.Sleep(skymodules.SubscriptionPeriod + time.Millisecond*100)
...
func testRPCSubscribeExtendTimeout(t *testing.T, rhp *renterHostPair) {
	...
	n := 3
	for i := 0; i < n; i++ {
		time.Sleep(skymodules.SubscriptionPeriod / 2)
		err = skymodules.RPCExtendSubscription(stream, pt)
		if err != nil {
			t.Fatal(err)
		}
	...
	err = build.Retry(10, time.Second, func() error {
		...
		cost := bandwidthCost.Add(skymodules.MDMSubscribeCost(pt, 0, 1).Mul64(2))
		cost = cost.Add(skymodules.MDMSubscriptionMemoryCost(pt, 1).Mul64(uint64(n)))
...
func testRPCSubscribeConcurrent(t *testing.T, rhp *renterHostPair) {
	...
	updateWG.Add(1)
	go func(rv skymodules.SignedRegistryValue) {
		defer updateWG.Done()
		for {
			select {
			case <-ticker.C:
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		bandwidthCost := upCost.Add(downCost)
		cost := bandwidthCost.Add(skymodules.MDMSubscribeCost(pt, 1, 1))
		cost = cost.Add(skymodules.MDMSubscriptionMemoryCost(pt, 1).Mul64(uint64(n)))
