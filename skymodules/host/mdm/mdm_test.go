...
type (
	...
	TestHost struct {
		...
		registry        map[crypto.Hash]skymodules.SignedRegistryValue
		mu              sync.Mutex
	}
...
func newCustomTestHost(generateSectors bool) *TestHost {
	return &TestHost{
		...
		registry:        make(map[crypto.Hash]skymodules.SignedRegistryValue),
...
func (h *TestHost) RegistryGet(pubKey types.SiaPublicKey, tweak crypto.Hash) (skymodules.SignedRegistryValue, bool) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *TestHost) RegistryUpdate(rv skymodules.SignedRegistryValue, pubKey types.SiaPublicKey, expiry types.BlockHeight) (skymodules.SignedRegistryValue, error) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (so *TestStorageObligation) ContractSize() uint64 {
	return uint64(len(so.sectorRoots)) * skymodules.SectorSize
}
...
func newTestPriceTable() *skymodules.RPCPriceTable {
...
func (mdm *MDM) ExecuteProgramWithBuilderCustomBudget(tb *testProgramBuilder, so *TestStorageObligation, duration types.BlockHeight, finalized bool) ([]Output, *skymodules.RPCBudget, error) {
	// Execute the program.
...
func (mdm *MDM) ExecuteProgramWithBuilderManualFinalize(tb *testProgramBuilder, so *TestStorageObligation, duration types.BlockHeight, finalized bool) (FnFinalize, *skymodules.RPCBudget, []Output, error) {
