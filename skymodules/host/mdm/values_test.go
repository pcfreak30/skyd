...
type (
	...
	TestValues struct {
		...
		staticPT       *skymodules.RPCPriceTable
...
func NewTestValues(pt *skymodules.RPCPriceTable, duration types.BlockHeight) TestValues {
	return TestValues{
		readonly:          true,
		staticDuration:    duration,
		staticPT:          pt,
		memory:            skymodules.MDMInitMemory(),
...
func (v *TestValues) AddAppendInstruction(data []byte) {
	memory := skymodules.MDMAppendMemory()
	collateral := skymodules.MDMAppendCollateral(v.staticPT)
	cost, refund := skymodules.MDMAppendCost(v.staticPT, v.staticDuration)
	time := uint64(skymodules.MDMTimeAppend)
	newData := len(data)
	readonly := false
	batch := false
...
func (v *TestValues) AddDropSectorsInstruction(numSectors uint64) {
	collateral := skymodules.MDMDropSectorsCollateral()
	cost := skymodules.MDMDropSectorsCost(v.staticPT, numSectors)
	memory := skymodules.MDMDropSectorsMemory()
	time := skymodules.MDMDropSectorsTime(numSectors)
	newData := 8
	readonly := false
	batch := false
...
func (v *TestValues) AddHasSectorInstruction() {
	collateral := skymodules.MDMHasSectorCollateral()
	cost := skymodules.MDMHasSectorCost(v.staticPT)
	memory := skymodules.MDMHasSectorMemory()
	time := uint64(skymodules.MDMTimeHasSector)
...
func (v *TestValues) AddReadOffsetInstruction(length uint64) {
	collateral := skymodules.MDMReadCollateral()
	cost := skymodules.MDMReadCost(v.staticPT, length)
	memory := skymodules.MDMReadMemory()
	time := uint64(skymodules.MDMTimeReadOffset)
	newData := 8 + 8
	readonly := true
	batch := false
	v.addInstruction(collateral, cost, types.ZeroCurrency, types.ZeroCurrency, memory, time, newData, readonly, batch)
...
func (v *TestValues) AddReadSectorInstruction(length uint64) {
	...
	cost := skymodules.MDMReadCost(v.staticPT, length)
	memory := skymodules.MDMReadMemory()
	time := uint64(skymodules.MDMTimeReadSector)
...
func (v *TestValues) AddRevisionInstruction() {
	collateral := skymodules.MDMRevisionCollateral()
	cost := skymodules.MDMRevisionCost(v.staticPT)
	memory := skymodules.MDMRevisionMemory()
	time := uint64(skymodules.MDMTimeRevision)
	readonly := true
	batch := false
...
func (v *TestValues) AddSwapSectorInstruction() {
	collateral := skymodules.MDMSwapSectorCollateral()
	cost := skymodules.MDMSwapSectorCost(v.staticPT)
	memory := skymodules.MDMSwapSectorMemory()
	time := uint64(skymodules.MDMTimeSwapSector)
	newData := 8 + 8
	readonly := false
	batch := false
	v.addInstruction(collateral, cost, types.ZeroCurrency, types.ZeroCurrency, memory, time, newData, readonly, batch)
...
// track of running values.
func (v *TestValues) AddUpdateRegistryInstruction(spk types.SiaPublicKey, rv skymodules.SignedRegistryValue) {
	memory := skymodules.MDMUpdateRegistryMemory()
	collateral := skymodules.MDMUpdateRegistryCollateral()
	cost, refund := skymodules.MDMUpdateRegistryCost(v.staticPT)
	time := uint64(skymodules.MDMTimeUpdateRegistry)
...
func (v *TestValues) AddReadRegistryInstruction(spk types.SiaPublicKey, refunded bool) {
	memory := skymodules.MDMReadRegistryMemory()
	collateral := skymodules.MDMReadRegistryCollateral()
	cost, refund := skymodules.MDMReadRegistryCost(v.staticPT)
	time := uint64(skymodules.MDMTimeReadRegistry)
...
func (v TestValues) Cost() (cost, failureRefund, collateral, instructionRefund types.Currency) {
	...
	cost = skymodules.MDMInitCost(v.staticPT, uint64(*v.programDataLength), uint64(*v.numInstructions))
...
func (v TestValues) Budget(finalized bool) *skymodules.RPCBudget {
	cost, _, _, _ := v.Cost()
	// Add the cost of finalizing the program.
	if !v.readonly && finalized {
		cost = cost.Add(skymodules.MDMMemoryCost(v.staticPT, v.memory, skymodules.MDMTimeCommit))
	}
	return skymodules.NewBudget(cost)
}
...
func (v *TestValues) addInstruction(collateral, cost, failureRefund, successRefund types.Currency, memory, time uint64, newData int, readonly, batch bool) {
	...
	v.memory += memory
	memoryCost := skymodules.MDMMemoryCost(v.staticPT, v.memory, time)
