...
func TestInstructionAppendAndDropSectors(t *testing.T) {
	...
	expectedOutputs := []output{
		{
			NewSize:       skymodules.SectorSize,
			NewMerkleRoot: cachedMerkleRoot(merkleRoots1),
			Proof:         []crypto.Hash{},
		},
		{
			NewSize:       2 * skymodules.SectorSize,
			NewMerkleRoot: cachedMerkleRoot(merkleRoots2),
			Proof:         []crypto.Hash{},
		},
		{
			NewSize:       3 * skymodules.SectorSize,
	...
	// Update variables.
	numSectors = int(lastOutput.NewSize / skymodules.SectorSize)
