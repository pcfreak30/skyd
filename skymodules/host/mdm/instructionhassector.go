...
func (p *program) staticDecodeHasSectorInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierHasSector {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierHasSector, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCIHasSectorLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCIHasSectorLen, len(instruction.Args))
	}
	// Read args.
...
func (i *instructionHasSector) Collateral() types.Currency {
	return skymodules.MDMHasSectorCollateral()
}
...
func (i *instructionHasSector) Cost() (executionCost, _ types.Currency, err error) {
	executionCost = skymodules.MDMHasSectorCost(i.staticState.priceTable)
	return
}
...
func (i *instructionHasSector) Memory() uint64 {
	return skymodules.MDMHasSectorMemory()
}
...
func (i *instructionHasSector) Time() (uint64, error) {
	return skymodules.MDMTimeHasSector, nil
}
