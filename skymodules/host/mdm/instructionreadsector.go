...
func (p *program) staticDecodeReadSectorInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierReadSector {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierReadSector, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCIReadSectorLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCIReadSectorLen, len(instruction.Args))
	}
	// Read args.
...
func executeReadSector(previousOutput output, ps *programState, length, offset uint64, sectorRoot crypto.Hash, merkleProof bool) (output, []byte) {
	...
	var err error
	switch {
	case offset+length > skymodules.SectorSize:
		err = fmt.Errorf("request is out of bounds %v + %v = %v > %v", offset, length, offset+length, skymodules.SectorSize)
	case length == 0:
