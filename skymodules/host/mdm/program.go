...
func decodeInstruction(p *program, i skymodules.Instruction) (instruction, error) {
	switch i.Specifier {
	case skymodules.SpecifierAppend:
		return p.staticDecodeAppendInstruction(i)
	case skymodules.SpecifierDropSectors:
		return p.staticDecodeDropSectorsInstruction(i)
	case skymodules.SpecifierHasSector:
		return p.staticDecodeHasSectorInstruction(i)
	case skymodules.SpecifierReadSector:
		return p.staticDecodeReadSectorInstruction(i)
	case skymodules.SpecifierReadOffset:
		return p.staticDecodeReadOffsetInstruction(i)
	case skymodules.SpecifierRevision:
		return p.staticDecodeRevisionInstruction(i)
	case skymodules.SpecifierSwapSector:
		return p.staticDecodeSwapSectorInstruction(i)
	case skymodules.SpecifierUpdateRegistry:
		return p.staticDecodeUpdateRegistryInstruction(i)
	case skymodules.SpecifierReadRegistry:
...
func (mdm *MDM) ExecuteProgram(ctx context.Context, pt *skymodules.RPCPriceTable, p skymodules.Program, budget *skymodules.RPCBudget, collateralBudget types.Currency, sos StorageObligationSnapshot, duration types.BlockHeight, programDataLen uint64, data io.Reader) (_ FnFinalize, _ <-chan Output, err error) {
	...
	program := &program{
		...
		staticBudget:           budget,
		usedMemory:             skymodules.MDMInitMemory(),
	...
	err = program.addCost(skymodules.MDMInitCost(pt, program.staticData.Len(), uint64(len(program.instructions))))
	if err != nil {
		return nil, nil, errors.Compose(err, program.staticData.Close())
	}
...
func (p *program) addCollateral(collateral types.Currency) error {
	...
	if p.staticCollateralBudget.Cmp(additionalCollateral) < 0 {
		return skymodules.ErrMDMInsufficientCollateralBudget
	}
...
func (p *program) addCost(cost types.Currency) error {
	if !p.staticBudget.Withdraw(cost) {
		return skymodules.ErrMDMInsufficientBudget
	}
...
func (p *program) executeInstructions(ctx context.Context, fcSize uint64, fcRoot crypto.Hash) error {
	...
	for idx, i := range p.instructions {
		...
		memoryCost := skymodules.MDMMemoryCost(p.staticProgramState.priceTable, p.usedMemory, time)
...
func (p *program) managedFinalize(so StorageObligation) error {
	...
	memoryCost := skymodules.MDMMemoryCost(p.staticProgramState.priceTable, p.usedMemory, skymodules.MDMTimeCommit)
