...
func TestNewEmptyProgram(t *testing.T) {
	...
	budget := skymodules.NewBudget(skymodules.MDMInitCost(pt, 0, 0))
	_, _, err := mdm.ExecuteProgram(context.Background(), pt, []skymodules.Instruction{}, budget, types.ZeroCurrency, host.newTestStorageObligation(true), duration, 0, bytes.NewReader([]byte{}))
...
func TestNewProgramLowInitBudget(t *testing.T) {
	...
	// Execute the program.
	budget := skymodules.NewBudget(types.ZeroCurrency)
	...
	if !errors.Contains(err, skymodules.ErrMDMInsufficientBudget) {
...
func TestNewProgramLowBudget(t *testing.T) {
	...
	duration := types.BlockHeight(fastrand.Uint64n(5))
	pb := newTestProgramBuilder(pt, duration)
	pb.AddReadSectorInstruction(skymodules.SectorSize, 0, crypto.Hash{}, true)
	program, data := pb.Program()
	values := pb.Cost()
	...
	cost := skymodules.MDMInitCost(pt, dataLen, 1)
	budget := skymodules.NewBudget(cost)
	...
	for output := range outputs {
		if err := output.Error; errors.Contains(err, skymodules.ErrMDMInsufficientBudget) {
...
func TestNewProgramLowCollateralBudget(t *testing.T) {
	...
	numInsufficientBudgetErrs := 0
	for output := range outputs {
		if err := output.Error; errors.Contains(err, skymodules.ErrMDMInsufficientCollateralBudget) {
			numInsufficientBudgetErrs++
		} else if err != nil {
