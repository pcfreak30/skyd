...
func (p *program) staticDecodeDropSectorsInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierDropSectors {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierDropSectors, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCIDropSectorsLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCIDropSectorsLen, len(instruction.Args))
	}
	// Read args.
...
func (i *instructionDropSectors) Execute(prevOutput output) (output, types.Currency) {
	...
	// Verify input.
	oldNumSectors := prevOutput.NewSize / skymodules.SectorSize
	...
	return output{
		NewSize:       newNumSectors * skymodules.SectorSize,
...
func (i *instructionDropSectors) Collateral() types.Currency {
	return skymodules.MDMDropSectorsCollateral()
}
...
func (i *instructionDropSectors) Cost() (executionCost, _ types.Currency, err error) {
	...
	executionCost = skymodules.MDMDropSectorsCost(i.staticState.priceTable, numSectorsDropped)
	return
}
...
func (i *instructionDropSectors) Memory() uint64 {
	return skymodules.MDMDropSectorsMemory()
}
...
func (i *instructionDropSectors) Time() (uint64, error) {
	...
	return skymodules.MDMDropSectorsTime(numDropped), nil
}
