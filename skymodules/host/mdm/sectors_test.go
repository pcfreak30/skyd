...
func randomSectorData() []byte {
	return fastrand.Bytes(int(skymodules.SectorSize))
}
...
func TestTranslateOffsetToRoot(t *testing.T) {
	...
	assert(0, 0, 0)
	assert(1, 1, 0)
	assert(skymodules.SectorSize-1, skymodules.SectorSize-1, 0)
	assert(skymodules.SectorSize, 0, 1)
	assert(skymodules.SectorSize+1, 1, 1)
	assert(2*skymodules.SectorSize-1, skymodules.SectorSize-1, 1)

	// Test out-of-bounds
	_, _, err := s.translateOffset(2 * skymodules.SectorSize)
	if err == nil {
