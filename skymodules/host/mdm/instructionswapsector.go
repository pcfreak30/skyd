...
func (p *program) staticDecodeSwapSectorInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierSwapSector {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierSwapSector, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCISwapSectorLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCISwapSectorLen, len(instruction.Args))
	}
	// Read args.
...
func (i *instructionSwapSector) Collateral() types.Currency {
	return skymodules.MDMSwapSectorCollateral()
}
...
func (i *instructionSwapSector) Cost() (executionCost, storage types.Currency, err error) {
	executionCost = skymodules.MDMSwapSectorCost(i.staticState.priceTable)
	return
}
