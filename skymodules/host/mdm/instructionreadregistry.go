...
func (p *program) staticDecodeReadRegistryInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierReadRegistry {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierReadRegistry, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCIReadRegistryLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCIReadRegistryLen, len(instruction.Args))
	}
	// Read args.
...
func (i *instructionReadRegistry) Execute(prevOutput output) (output, types.Currency) {
	...
	if !found {
		_, refund := skymodules.MDMReadRegistryCost(i.staticState.priceTable)
		return out, refund
	}
...
func (i *instructionReadRegistry) Collateral() types.Currency {
	return skymodules.MDMReadRegistryCollateral()
}
...
func (i *instructionReadRegistry) Cost() (executionCost, refund types.Currency, err error) {
	executionCost, refund = skymodules.MDMReadRegistryCost(i.staticState.priceTable)
	return
}
