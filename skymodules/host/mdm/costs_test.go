...
func newTestWriteStorePriceTable() *skymodules.RPCPriceTable {
	...
	pt.WriteStoreCost = skymodules.DefaultStoragePrice
...
// misleading costs.
func appendTrueCost(pt *skymodules.RPCPriceTable) (types.Currency, types.Currency) {
	writeCost := pt.WriteLengthCost.Mul64(skymodules.SectorSizeStandard).Add(pt.WriteBaseCost)
	storeCost := pt.WriteStoreCost.Mul64(skymodules.SectorSizeStandard) // potential refund
...
func TestCosts(t *testing.T) {
	...
	sectorsPerTB := skymodules.BytesPerTerabyte.Div64(skymodules.SectorSizeStandard)
	...
	expectedCostPerTB := skymodules.DefaultStoragePrice.Mul(skymodules.BytesPerTerabyte)
