...
func (p *program) staticDecodeUpdateRegistryInstruction(instruction skymodules.Instruction) (instruction, error) {
	// Check specifier.
	if instruction.Specifier != skymodules.SpecifierUpdateRegistry {
		return nil, fmt.Errorf("expected specifier %v but got %v",
			skymodules.SpecifierUpdateRegistry, instruction.Specifier)
	}
	// Check args.
	if len(instruction.Args) != skymodules.RPCIUpdateRegistryLen {
		return nil, fmt.Errorf("expected instruction to have len %v but was %v",
			skymodules.RPCIUpdateRegistryLen, len(instruction.Args))
	}
	// Read args.
...
func (i *instructionUpdateRegistry) Execute(prevOutput output) (output, types.Currency) {
	...
	rv := skymodules.NewSignedRegistryValue(tweak, data, revision, signature)
...
func (i *instructionUpdateRegistry) Collateral() types.Currency {
	return skymodules.MDMUpdateRegistryCollateral()
}
...
func (i *instructionUpdateRegistry) Cost() (executionCost, storeCost types.Currency, err error) {
	executionCost, storeCost = skymodules.MDMUpdateRegistryCost(i.staticState.priceTable)
	return
}
