...
func (pth *priceTableHeap) PopExpired() (expired []skymodules.UniqueID) {
	pth.mu.Lock()
	defer pth.mu.Unlock()
...
func (h *Host) managedPriceTableForRenter() *skymodules.RPCPriceTable {
...
func (h *Host) managedRPCUpdatePriceTable(stream siamux.Stream) (err error) {
	...
	if err = skymodules.RPCWrite(stream, tracked); err != nil {
...
func (h *Host) staticReadPriceTableID(stream siamux.Stream) (*skymodules.RPCPriceTable, error) {
	// read the price table uid
	var uid skymodules.UniqueID
	err := skymodules.RPCRead(stream, &uid)
	if err != nil {
	...
	if h.dependencies.Disrupt("HostLosePriceTable") {
		return nil, errors.AddContext(skymodules.ErrPriceTableNotFound, fmt.Sprint(uid))
	}
	// if an empty price table was provided, we are not going to find a table.
	if uid == (skymodules.UniqueID{}) {
	...
	if time.Now().After(pt.Expiry()) {
		return nil, errors.AddContext(skymodules.ErrPriceTableExpired, fmt.Sprint(uid))
	}
