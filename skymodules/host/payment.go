...
// string otherwise.
func (h *Host) ProcessPayment(stream siamux.Stream, bh types.BlockHeight) (skymodules.PaymentDetails, error) {
	// read the PaymentRequest
	var pr skymodules.PaymentRequest
	if err := skymodules.RPCRead(stream, &pr); err != nil {
		return nil, errors.AddContext(err, "Could not read payment request")
	}

	// process payment depending on the payment method
	if pr.Type == skymodules.PayByEphemeralAccount {
	...
	return nil, errors.Compose(fmt.Errorf("Could not handle payment method %v", pr.Type), skymodules.ErrUnknownPaymentMethod)
}

// staticPayByEphemeralAccount processes a PayByEphemeralAccountRequest coming
// in over the given stream.
func (h *Host) staticPayByEphemeralAccount(stream siamux.Stream, bh types.BlockHeight) (skymodules.PaymentDetails, error) {
	// read the PayByEphemeralAccountRequest
	var req skymodules.PayByEphemeralAccountRequest
	if err := skymodules.RPCRead(stream, &req); err != nil {
...
// given stream.
func (h *Host) managedPayByContract(stream siamux.Stream, bh types.BlockHeight) (skymodules.PaymentDetails, error) {
	// read the PayByContractRequest
	var pbcr skymodules.PayByContractRequest
	if err := skymodules.RPCRead(stream, &pbcr); err != nil {
	...
	err = skymodules.RPCWrite(stream, skymodules.PayByContractResponse{
		Signature: sig,
	})
	if err != nil {
...
func (h *Host) managedFundAccount(stream siamux.Stream, request skymodules.FundAccountRequest, cost types.Currency) (types.Currency, error) {
	// read the PayByContractRequest
	var pbcr skymodules.PayByContractRequest
	if err := skymodules.RPCRead(stream, &pbcr); err != nil {
...
func revisionFromRequest(recent types.FileContractRevision, pbcr skymodules.PayByContractRequest) types.FileContractRevision {
	rev := recent
...
func signatureFromRequest(recent types.FileContractRevision, pbcr skymodules.PayByContractRequest) types.TransactionSignature {
...
type paymentDetails struct {
	account skymodules.AccountID
	amount  types.Currency
}

// newPaymentDetails returns a new paymentDetails object using the given values
func newPaymentDetails(account skymodules.AccountID, amountPaid types.Currency) *paymentDetails {
...
func (pd *paymentDetails) AccountID() skymodules.AccountID { return pd.account }
