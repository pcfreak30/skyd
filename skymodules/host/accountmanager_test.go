...
func TestAccountWithdrawalExpired(t *testing.T) {
	...
	err = callWithdraw(am, msg, sig, am.h.BlockHeight())
	if !errors.Contains(err, skymodules.ErrWithdrawalExpired) {
...
func TestAccountWithdrawalExtremeFuture(t *testing.T) {
	...
	err = callWithdraw(am, msg, sig, am.h.BlockHeight())
	if !errors.Contains(err, skymodules.ErrWithdrawalExtremeFuture) {
...
func TestAccountWithdrawalInvalidSignature(t *testing.T) {
	...
	if !errors.Contains(err, skymodules.ErrWithdrawalInvalidSignature) {
...
func TestAccountRiskBenchmark(t *testing.T) {
	...
	accountIDs := make([]skymodules.AccountID, acc)
...
func TestAccountWithdrawalBenchmark(t *testing.T) {
	...
	for _, config := range configurations {
		...
		msgs := make([][]*skymodules.WithdrawalMessage, threads)
		sigs := make([][]crypto.Signature, threads)
		for t := 0; t < threads; t++ {
			msgs[t] = make([]*skymodules.WithdrawalMessage, withdrawals/threads)
...
func TestAccountWithdrawalMultiple(t *testing.T) {
	...
	// Prepare withdrawals and signatures
	msgs := make([]*skymodules.WithdrawalMessage, withdrawals)
...
func TestAccountMaxEphemeralAccountRisk(t *testing.T) {
	...
	accountIDs := make([]skymodules.AccountID, buckets)
...
func TestAccountIndexRecycling(t *testing.T) {
	...
	// 64 bit field.
	expire := func(id skymodules.AccountID) bool {
...
func TestAccountWithdrawalsInactive(t *testing.T) {
	...
	am.callConsensusChanged(skymodules.ConsensusChange{Synced: false}, 0, 1)

	// Verify withdrawal is active
	msg, sig = prepareWithdrawal(accountID, oneCurrency, am.h.blockHeight, sk)
	err = callWithdraw(am, msg, sig, am.h.blockHeight)
	if !errors.Contains(err, skymodules.ErrWithdrawalsInactive) {
		t.Fatal(err)
	}
}
...
func callWithdraw(am *accountManager, msg *skymodules.WithdrawalMessage, sig crypto.Signature, bh types.BlockHeight) error {
...
// goroutines.
func callDeposit(am *accountManager, id skymodules.AccountID, amount types.Currency) error {
...
func prepareWithdrawal(id skymodules.AccountID, amount types.Currency, expiry types.BlockHeight, sk crypto.SecretKey) (*skymodules.WithdrawalMessage, crypto.Signature) {
	msg := &skymodules.WithdrawalMessage{
		Account: id,
		Expiry:  expiry,
		Amount:  amount,
	}
	copy(msg.Nonce[:], fastrand.Bytes(skymodules.WithdrawalNonceSize))
...
// sia public key
func prepareAccount() (_ crypto.SecretKey, aid skymodules.AccountID) {
