...
func TestPriceTableMarshaling(t *testing.T) {
	...
	var ptCopy skymodules.RPCPriceTable
...
func TestPriceTableMinHeap(t *testing.T) {
	...
	pt1 := hostRPCPriceTable{
		skymodules.RPCPriceTable{Validity: rpcPriceGuaranteePeriod},
...
func testUpdatePriceTableBasic(t *testing.T, rhp *renterHostPair) {
	...
	if !pt.RenewContractCost.Equals(skymodules.DefaultBaseRPCPrice) {
		t.Fatal("Wrong renew cost", pt.RenewContractCost, skymodules.DefaultBaseRPCPrice)
	}
}
...
func testUpdatePriceTableInsufficientPayment(t *testing.T, rhp *renterHostPair) {
	...
	if err == nil || !strings.Contains(err.Error(), skymodules.ErrInsufficientPaymentForRPC.Error()) {
		t.Fatalf("Expected error '%v', instead error was '%v'", skymodules.ErrInsufficientPaymentForRPC, err)
	}
}
...
func runUpdatePriceTableRPCWithRequest(rhp *renterHostPair, pbcRequest skymodules.PayByContractRequest) (_ *skymodules.RPCPriceTable, err error) {
