...
import (
	...
	"gitlab.com/skynetlabs/skyd/skymodules/host/mdm"
...
const (
	...
	dbFilename   = skymodules.HostDir + ".db"
	logFile      = skymodules.HostDir + ".log"
	settingsFile = skymodules.HostDir + ".json"
)
...
type Host struct {
	...
	dependencies  skymodules.Dependencies
	skymodules.StorageManager
	...
	recentChange skymodules.ConsensusChangeID
	...
	autoAddress          skymodules.NetAddress // Determined using automatic tooling in network.go
	financialMetrics     skymodules.HostFinancialMetrics
	settings             skymodules.HostInternalSettings
	revisionNumber       uint64
	workingStatus        skymodules.HostWorkingStatus
	connectabilityStatus skymodules.HostConnectabilityStatus
...
type hostPrices struct {
	current       skymodules.RPCPriceTable
	guaranteed    map[skymodules.UniqueID]*hostRPCPriceTable
	staticMinHeap priceTableHeap
	mu            sync.RWMutex
}

// managedCurrent returns the host's current price table
func (hp *hostPrices) managedCurrent() skymodules.RPCPriceTable {
	hp.mu.RLock()
	defer hp.mu.RUnlock()
	return hp.current
}
...
func (hp *hostPrices) managedGet(uid skymodules.UniqueID) (pt *hostRPCPriceTable, found bool) {
	hp.mu.RLock()
	defer hp.mu.RUnlock()
...
// given
func (hp *hostPrices) managedSetCurrent(pt skymodules.RPCPriceTable) {
	hp.mu.Lock()
	defer hp.mu.Unlock()
	hp.current = pt
}
...
func (h *Host) managedInternalSettings() skymodules.HostInternalSettings {
	h.mu.RLock()
	defer h.mu.RUnlock()
	return h.settings
}
...
func (h *Host) managedUpdatePriceTable() {
	...
	priceTable := skymodules.RPCPriceTable{
		...
		RenewContractCost: skymodules.DefaultBaseRPCPrice,
		...
		// prices.
		LatestRevisionCost: skymodules.DefaultBaseRPCPrice.Add(hes.DownloadBandwidthPrice.Mul64(skymodules.EstimatedFileContractTransactionSetSize)),
...
// the Host.
func newHost(dependencies skymodules.Dependencies, smDeps skymodules.Dependencies, cs skymodules.ConsensusSet, g skymodules.Gateway, tpool skymodules.TransactionPool, wallet skymodules.Wallet, mux *siamux.SiaMux, listenerAddress string, persistDir string) (_ *Host, err error) {
	...
	h := &Host{
		...
		tpool:                    tpool,
		wallet:                   wallet,
		staticAlerter:            skymodules.NewAlerter("host"),
		staticMux:                mux,
		dependencies:             dependencies,
		lockedStorageObligations: make(map[types.FileContractID]*lockedObligation),
		staticPriceTables: &hostPrices{
			guaranteed: make(map[skymodules.UniqueID]*hostRPCPriceTable),
			staticMinHeap: priceTableHeap{
...
func New(cs skymodules.ConsensusSet, g skymodules.Gateway, tpool skymodules.TransactionPool, wallet skymodules.Wallet, mux *siamux.SiaMux, address string, persistDir string) (*Host, error) {
	return newHost(skymodules.ProdDependencies, new(skymodules.ProductionDependencies), cs, g, tpool, wallet, mux, address, persistDir)
}

// NewCustomHost returns an initialized Host using the provided dependencies.
func NewCustomHost(deps skymodules.Dependencies, cs skymodules.ConsensusSet, g skymodules.Gateway, tpool skymodules.TransactionPool, wallet skymodules.Wallet, mux *siamux.SiaMux, address string, persistDir string) (*Host, error) {
	return newHost(deps, new(skymodules.ProductionDependencies), cs, g, tpool, wallet, mux, address, persistDir)
}
...
// injection into the host's subskymodules.
func NewCustomTestHost(deps skymodules.Dependencies, smDeps skymodules.Dependencies, cs skymodules.ConsensusSet, g skymodules.Gateway, tpool skymodules.TransactionPool, wallet skymodules.Wallet, mux *siamux.SiaMux, address string, persistDir string) (*Host, error) {
...
func (h *Host) ExternalSettings() skymodules.HostExternalSettings {
	err := h.tg.Add()
	if err != nil {
...
func (h *Host) PriceTable() skymodules.RPCPriceTable {
...
func (h *Host) WorkingStatus() skymodules.HostWorkingStatus {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Host) ConnectabilityStatus() skymodules.HostConnectabilityStatus {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Host) FinancialMetrics() skymodules.HostFinancialMetrics {
	err := h.tg.Add()
	if err != nil {
...
func (h *Host) SetInternalSettings(settings skymodules.HostInternalSettings) error {
	err := h.tg.Add()
	if err != nil {
		return err
	}
	...
	// entry.
	settings.RegistrySize = skymodules.RoundRegistrySize(settings.RegistrySize)
	if h.settings.RegistrySize != settings.RegistrySize {
		err := h.staticRegistry.Truncate(settings.RegistrySize/skymodules.RegistryEntrySize, false)
		if err != nil {
	...
	if h.settings.CustomRegistryPath != settings.CustomRegistryPath {
		...
		if path == "" {
			...
			path = filepath.Join(h.persistDir, skymodules.HostRegistryFile)
		}
...
func (h *Host) InternalSettings() skymodules.HostInternalSettings {
	err := h.tg.Add()
	if err != nil {
		return skymodules.HostInternalSettings{}
	}
	defer h.tg.Done()
...
func (h *Host) managedExternalSettings() skymodules.HostExternalSettings {
...
func (h *Host) RegistryGet(pubKey types.SiaPublicKey, tweak crypto.Hash) (skymodules.SignedRegistryValue, bool) {
	err := h.tg.Add()
	if err != nil {
		return skymodules.SignedRegistryValue{}, false
	}
	defer h.tg.Done()
...
func (h *Host) RegistryUpdate(rv skymodules.SignedRegistryValue, pubKey types.SiaPublicKey, expiry types.BlockHeight) (skymodules.SignedRegistryValue, error) {
	err := h.tg.Add()
	if err != nil {
		return skymodules.SignedRegistryValue{}, err
	}
	defer h.tg.Done()
...
func (h *Host) managedInitRegistry() error {
	...
	if err == nil && fi.Size() > skymodules.RegistryEntrySize {
		// We subtract -1 to account for the metadata page.
		onDiskEntries = (uint64(fi.Size()) / skymodules.RegistryEntrySize) - 1
	}

	// Also get the size in entries from the internal settings.
	settingsEntries := skymodules.RoundRegistrySize(is.RegistrySize) / skymodules.RegistryEntrySize
	...
	if onDiskEntries > settingsEntries {
		...
		h.mu.Lock()
		h.settings.RegistrySize = settingsEntries * skymodules.RegistryEntrySize
		h.mu.Unlock()
