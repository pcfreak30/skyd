...
func (h *Host) staticVerifyAnnouncementAddress(addr skymodules.NetAddress) error {
...
func (h *Host) managedAnnounce(addr skymodules.NetAddress) (err error) {
	...
	signedAnnouncement, err := skymodules.CreateAnnouncement(addr, pubKey, secKey)
	if err != nil {
		return err
	}
...
func (h *Host) Announce() error {
	...
	var annAddr skymodules.NetAddress
	if userSet != "" {
		annAddr = userSet
	} else {
		annAddr = autoSet
...
func (h *Host) AnnounceAddress(addr skymodules.NetAddress) error {
	err := h.tg.Add()
	if err != nil {
		return err
	}
