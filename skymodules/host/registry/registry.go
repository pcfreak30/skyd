...
const (
	...
	PersistedEntrySize = skymodules.RegistryEntrySize
)
...
func (v *value) update(rv skymodules.SignedRegistryValue, newExpiry types.BlockHeight, init bool) error {
...
func (r *Registry) Get(pubKey types.SiaPublicKey, tweak crypto.Hash) (skymodules.SignedRegistryValue, bool) {
	r.mu.Lock()
	...
	v.mu.Lock()
	defer v.mu.Unlock()
	return skymodules.NewSignedRegistryValue(v.tweak, v.data, v.revision, v.signature), true
}
...
func New(path string, maxEntries uint64) (_ *Registry, err error) {
	...
	f, err := os.OpenFile(path, os.O_RDWR, skymodules.DefaultFilePerm)
...
func (r *Registry) Update(rv skymodules.SignedRegistryValue, pubKey types.SiaPublicKey, expiry types.BlockHeight) (srv skymodules.SignedRegistryValue, _ error) {
	// Check the data against the limit.
	if len(rv.Data) > skymodules.RegistryDataSize {
		return skymodules.SignedRegistryValue{}, errTooMuchData
	}
	...
	if err := rv.Verify(pubKey.ToPublicKey()); err != nil {
		...
		return skymodules.SignedRegistryValue{}, errors.AddContext(err, "Update: failed to verify signature")
	}
	...
	if !exists {
		...
		if err != nil {
			r.mu.Unlock()
			return skymodules.SignedRegistryValue{}, errors.AddContext(err, "failed to create new value")
		}
	}
	...
	if exists {
		srv = skymodules.NewSignedRegistryValue(entry.tweak, entry.data, entry.revision, entry.signature)
	}
	// Update the entry.
	...
	if err != nil {
		...
		return skymodules.SignedRegistryValue{}, errors.New("failed to save new entry to disk")
	}
	entry.mu.Unlock()
	return srv, nil
}
...
func (r *Registry) newValue(rv skymodules.SignedRegistryValue, pubKey types.SiaPublicKey, expiry types.BlockHeight) (*value, error) {
...
func (r *Registry) Migrate(path string) error {
	...
	f, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE|os.O_EXCL, skymodules.DefaultFilePerm)
	if err != nil {
