...
func testDir(name string) string {
	...
	_ = os.RemoveAll(dir)
	err := os.MkdirAll(dir, skymodules.DefaultDirPerm)
	if err != nil {
		panic(err)
	}
	return dir
...
func TestDeleteEntry(t *testing.T) {
	...
	if !reflect.DeepEqual(oldRV, skymodules.SignedRegistryValue{}) {
...
func TestFullRegistry(t *testing.T) {
	...
	for _, val := range vals {
		...
		// Verify signatures.
		rv := skymodules.NewSignedRegistryValue(val.tweak, val.data, val.revision, val.signature)
...
func TestRegistryRace(t *testing.T) {
	...
	// Add 3 entries to it.
	numEntries := 3
	rvs := make([]skymodules.SignedRegistryValue, 0, numEntries)
	...
	worker := func(key types.SiaPublicKey, sk crypto.SecretKey, rv skymodules.SignedRegistryValue, nextExpiry, nextRevision *uint64) {
		for {
...
func BenchmarkRegistryUpdate(b *testing.B) {
	...
	// Add entries.
	rvs := make([]skymodules.SignedRegistryValue, 0, nEntries)
...
func TestTruncate(t *testing.T) {
	...
	numEntries := 64
	entries := make([]skymodules.SignedRegistryValue, 0, numEntries)
	keys := make([]types.SiaPublicKey, 0, numEntries)
	for i := 0; i < numEntries; i++ {
		rv, v, sk := randomValue(0)
		rv.Revision = 0 // set revision number to 0
