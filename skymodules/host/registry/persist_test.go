...
func randomValue(index int64) (skymodules.SignedRegistryValue, *value, crypto.SecretKey) {
	// Create in-memory value first.
	sk, pk := crypto.GenerateKeyPair()
	v := value{
		expiry:      types.BlockHeight(fastrand.Uint64n(math.MaxUint32)),
		staticIndex: index,
		data:        fastrand.Bytes(fastrand.Intn(skymodules.RegistryDataSize) + 1),
	...
	rv := skymodules.NewRegistryValue(v.tweak, v.data, v.revision).Sign(sk)
...
func TestPersistedEntryMarshalUnmarshal(t *testing.T) {
	...
	entry := persistedEntry{
		...
		DataLen:  skymodules.RegistryDataSize,
