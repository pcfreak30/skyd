...
type (
	...
	persistedEntry struct {
		...
		Tweak [skymodules.TweakSize]byte

		// value data
		Expiry    compressedBlockHeight
		DataLen   uint8
		Data      [skymodules.RegistryDataSize]byte
		Revision  uint64
...
func newPersistedEntry(value *value) (persistedEntry, error) {
	if len(value.data) > skymodules.RegistryDataSize {
...
func (entry persistedEntry) Value(index int64) (*value, error) {
	if entry.DataLen > skymodules.RegistryDataSize {
