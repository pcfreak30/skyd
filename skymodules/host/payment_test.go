...
func testPayByContract(t *testing.T, pair *renterHostPair) {
	...
	var payment skymodules.PaymentDetails
	...
	renterFunc := func() error {
		...
		err := skymodules.RPCWriteAll(rStream, pRequest, pbcRequest)
		if err != nil {
			return err
		}

		// receive PayByContractResponse
		err = skymodules.RPCRead(rStream, &payByResponse)
		if err != nil {
			return err
		}
		return nil
	...
	hostFunc := func() error {
		...
		if err != nil {
			skymodules.RPCWriteError(hStream, err)
		}
		return nil
	}
...
func testPayByEphemeralAccount(t *testing.T, pair *renterHostPair) {
	...
	renterFunc := func() error {
		...
		pRequest := skymodules.PaymentRequest{Type: skymodules.PayByEphemeralAccount}
		pbcRequest := skymodules.NewPayByEphemeralAccountRequest(accountID, host.blockHeight+6, amount, sk)
		return skymodules.RPCWriteAll(rStream, pRequest, pbcRequest)
	}
	hostFunc := func() error {
		// process payment request
		payment, err = host.ProcessPayment(hStream, host.BlockHeight())
...
func testUnknownPaymentMethodError(t *testing.T, pair *renterHostPair) {
	...
	err = run(func() error {
		// send PaymentRequest
		pr := skymodules.PaymentRequest{Type: invalidSpecifier}
		err := skymodules.RPCWriteAll(rStream, skymodules.RPCUpdatePriceTable, pr)
		if err != nil {
			return err
		}
		return skymodules.RPCRead(rStream, struct{}{})
	}, func() error {
		// process payment request
...
func newPayByContractRequest(rev types.FileContractRevision, sig crypto.Signature, refundAccount skymodules.AccountID) skymodules.PayByContractRequest {
	var req skymodules.PayByContractRequest
...
func TestStreams(t *testing.T) {
	...
	func() {
		defer wg.Done()
		req := skymodules.PaymentRequest{Type: skymodules.PayByContract}
		err := skymodules.RPCWrite(renter, req)
		if err != nil {
			t.Fatal(err)
		}
	}()
	...
	func() {
		defer wg.Done()
		err := skymodules.RPCRead(host, &pr)
		if err != nil {
			t.Fatal(err)
		}
	}()
...
func TestRevisionFromRequest(t *testing.T) {
	...
	pbcr := skymodules.PayByContractRequest{
