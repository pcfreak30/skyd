...
func TestV143HostUpgrade(t *testing.T) {
	...
	hostDir := build.TempDir(persistDir, skymodules.HostDir)
	...
	hp := persistence{}
	err = persist.LoadJSON(skymodules.Hostv143PersistMetadata, &hp, dst)
	if err != nil {
		t.Fatal(err)
	}
	...
	siaMuxDir := filepath.Join(persistDir, skymodules.SiaMuxDir)
	...
	smPK := skymodules.SiaPKToMuxPK(hp.PublicKey)
	...
	if his.EphemeralAccountExpiry != skymodules.DefaultEphemeralAccountExpiry {
		t.Fatal("EphemeralAccountExpiry not properly fixed after upgrade")
	}

	// sanity check the metadata version
	err = persist.LoadJSON(skymodules.Hostv151PersistMetadata, struct{}{}, filepath.Join(hostDir, skymodules.HostSettingsFile))
	if err != nil {
		t.Fatal(err)
	}
	...
	err = persist.SaveJSON(skymodules.Hostv143PersistMetadata, hp, dst)
	if err != nil {
		t.Fatal(err)
	}
...
func TestShouldResetEphemeralAccountExpiry(t *testing.T) {
	...
	his := skymodules.HostInternalSettings{}
