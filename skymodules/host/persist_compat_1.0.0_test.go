...
func TestHostPersistCompat100(t *testing.T) {
	...
	// set of keys
	siaMuxDir := filepath.Join(ht.persistDir, skymodules.SiaMuxDir)
	mux, err := skymodules.NewSiaMux(siaMuxDir, ht.persistDir, "localhost:0", "localhost:0")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	// Reload the host
	h, err := New(ht.cs, ht.gateway, ht.tpool, ht.wallet, mux, "localhost:0", filepath.Join(ht.persistDir, skymodules.HostDir))
	if err != nil {
		t.Fatal(err)
	}
