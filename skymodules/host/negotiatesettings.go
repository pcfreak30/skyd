...
func (h *Host) externalSettings(maxFeeEstimation types.Currency) skymodules.HostExternalSettings {
	// Increment the revision number for the external settings
	h.revisionNumber++

	totalStorage, remainingStorage := h.capacity()
	var netAddr skymodules.NetAddress
	...
	contractPrice := maxFeeEstimation.Mul64(skymodules.EstimatedFileContractRevisionAndProofTransactionSetSize)
	...
	return skymodules.HostExternalSettings{
...
func (h *Host) managedRPCSettings(conn net.Conn) error {
	...
	conn.SetDeadline(time.Now().Add(skymodules.NegotiateSettingsTime))
	...
	settings := skymodules.HostOldExternalSettings{
