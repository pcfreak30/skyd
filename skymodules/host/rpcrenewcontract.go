...
func (h *Host) managedRPCRenewContract(stream siamux.Stream) error {
	...
	if errors.Contains(err, errEmptyPriceTableUID) {
		...
		uptResp := skymodules.RPCUpdatePriceTableResponse{PriceTableJSON: ptBytes}
		if err = skymodules.RPCWrite(stream, uptResp); err != nil {
	...
	// Read request
	var req skymodules.RPCRenewContractRequest
	err = skymodules.RPCReadMaxLen(stream, &req, skymodules.RenewDecodeMaxLen)
	if err != nil {
	...
	setFee := skymodules.CalculateFee(txns)
	...
	err = skymodules.RPCWrite(stream, skymodules.RPCRenewContractCollateralResponse{
	...
	var renterSignatureResp skymodules.RPCRenewContractRenterSignatures
	err = skymodules.RPCReadMaxLen(stream, &renterSignatureResp, skymodules.RenewDecodeMaxLen)
	if err != nil {
		txnBuilder.Drop()
	...
	renewRevenue, renewRisk := skymodules.RenewBaseCosts(finalRevision, pt, fc.WindowStart)
	h.mu.RUnlock()
	...
	err = skymodules.RPCWrite(stream, skymodules.RPCRenewContractHostSignatures{
...
func addRevisionSignatures(txnBuilder skymodules.TransactionBuilder, renterSigBytes crypto.Signature, sk crypto.SecretKey, rpk crypto.PublicKey, bh types.BlockHeight) (crypto.Signature, error) {
	...
	err := skymodules.VerifyRenewalTransactionSignatures(revision, contract, txn.TransactionSignatures, bh)
	if err != nil {
...
func verifyRenewedContract(so storageObligation, newContract types.FileContract, oldRevision types.FileContractRevision, blockHeight types.BlockHeight, internalSettings skymodules.HostInternalSettings, unlockHash types.UnlockHash, pt *skymodules.RPCPriceTable, renterPK, hostPK types.SiaPublicKey, lockedCollateral types.Currency) (types.Currency, error) {
	...
	basePrice, baseCollateral := skymodules.RenewBaseCosts(oldRevision, pt, newContract.WindowStart)
