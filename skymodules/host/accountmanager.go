...
type (
	...
	accountManager struct {
		accounts                map[skymodules.AccountID]*account
	...
	blockedWithdrawal struct {
		withdrawal   *skymodules.WithdrawalMessage
		priority     int64
...
func (h *Host) newAccountManager() (_ *accountManager, err error) {
	am := &accountManager{
		accounts:           make(map[skymodules.AccountID]*account),
...
func (am *accountManager) callDeposit(id skymodules.AccountID, amount types.Currency, syncChan chan struct{}) error {
...
// syncChan.
func (am *accountManager) callRefund(id skymodules.AccountID, amount types.Currency) error {
	// Nothing to refund.
	if amount.IsZero() {
		return nil
	}
...
// this point
func (am *accountManager) managedDeposit(id skymodules.AccountID, amount types.Currency, refund bool, syncChan chan struct{}) error {
...
// funds.
func (am *accountManager) callWithdraw(msg *skymodules.WithdrawalMessage, sig crypto.Signature, priority int64, bh types.BlockHeight) error {
	// Gather some variables
	his := am.h.managedInternalSettings()
	maxRisk := his.MaxEphemeralAccountRisk
...
func (am *accountManager) callConsensusChanged(cc skymodules.ConsensusChange, oldHeight, newHeight types.BlockHeight) {
	am.mu.Lock()
	defer am.mu.Unlock()
...
func (am *accountManager) deposit(id skymodules.AccountID, amount, maxRisk, maxBalance types.Currency, blockHeight types.BlockHeight, refund bool, pr *persistResult, syncChan chan struct{}) error {
...
func (am *accountManager) managedWithdraw(msg *skymodules.WithdrawalMessage, fp crypto.Hash, priority int64, maxRisk types.Currency, blockHeight types.BlockHeight, commitResultChan chan error) (err error) {
	...
	if am.withdrawalsInactive {
		return skymodules.ErrWithdrawalsInactive
	}
...
func (am *accountManager) managedAccountPersistInfo(id skymodules.AccountID) *accountPersistInfo {
	am.mu.Lock()
	defer am.mu.Unlock()
...
func (am *accountManager) threadedSaveAccount(id skymodules.AccountID) (waiting int) {
...
func (am *accountManager) callAccountBalance(id skymodules.AccountID) types.Currency {
	am.mu.Lock()
	defer am.mu.Unlock()
...
// will be created.
func (am *accountManager) openAccount(id skymodules.AccountID) (*account, error) {
	if id.IsZeroAccount() {
...
func (ab *accountBitfield) buildIndex(accounts map[skymodules.AccountID]*account) {
	var maxIndex uint32
