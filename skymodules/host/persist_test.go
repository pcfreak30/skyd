...
func TestHostContractCountPersistence(t *testing.T) {
	...
	ht.host, err = New(ht.cs, ht.gateway, ht.tpool, ht.wallet, ht.mux, "localhost:0", filepath.Join(ht.persistDir, skymodules.HostDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestHostPriceRatios(t *testing.T) {
	...
	settings := ht.host.InternalSettings()
	rpcPrice := settings.MaxBaseRPCPrice().Mul64(skymodules.MaxBaseRPCPriceVsBandwidth)
	sectorPrice := settings.MaxSectorAccessPrice().Mul64(skymodules.MaxSectorAccessPriceVsBandwidth)
	...
	downloadPrice := settings.MinDownloadBandwidthPrice.Div64(skymodules.MaxBaseRPCPriceVsBandwidth)
	...
	settings = ht.host.InternalSettings()
	rpcPrice = downloadPrice.Mul64(skymodules.MaxBaseRPCPriceVsBandwidth)
	sectorPrice = downloadPrice.Mul64(skymodules.MaxSectorAccessPriceVsBandwidth)
