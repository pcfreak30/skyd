...
func contractCollateral(settings skymodules.HostExternalSettings, fc types.FileContract) types.Currency {
...
func (h *Host) managedAddCollateral(settings skymodules.HostExternalSettings, txnSet []types.Transaction) (builder skymodules.TransactionBuilder, newParents []types.Transaction, newInputs []types.SiacoinInput, newOutputs []types.SiacoinOutput, err error) {
...
func (h *Host) managedRPCFormContract(conn net.Conn) error {
	...
	conn.SetDeadline(time.Now().Add(skymodules.NegotiateFileContractTime))
	...
	if err != nil {
		...
		// why to the renter.
		skymodules.WriteNegotiationRejection(conn, err) // Error ignored to preserve type in extendErr
	...
	err = encoding.ReadObject(conn, &renterTxnSignatures, skymodules.NegotiateMaxTransactionSignaturesSize)
	if err != nil {
...
func (h *Host) managedVerifyNewContract(txnSet []types.Transaction, renterPK crypto.PublicKey, eSettings skymodules.HostExternalSettings) error {
