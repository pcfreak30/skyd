...
func createExplorerTester(name string) (*explorerTester, error) {
	...
	testdir := build.TempDir(skymodules.ExplorerDir, name)
	...
	m, err := miner.New(cs, tp, w, filepath.Join(testdir, skymodules.RenterDir))
	if err != nil {
		return nil, err
	}
	e, err := New(cs, filepath.Join(testdir, skymodules.ExplorerDir))
	if err != nil {
		return nil, err
	}
	et := &explorerTester{
...
func TestExplorerGenesisHeight(t *testing.T) {
	...
	testdir := build.TempDir(skymodules.HostDir, t.Name())
	g, err := gateway.New("localhost:0", false, filepath.Join(testdir, skymodules.GatewayDir))
	if err != nil {
		t.Fatal(err)
	}
