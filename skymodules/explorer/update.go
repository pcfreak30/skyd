...
func (e *Explorer) ProcessConsensusChange(cc skymodules.ConsensusChange) {
	...
	err := e.db.Update(func(tx *bolt.Tx) (err error) {
		...
		for _, scod := range cc.SiacoinOutputDiffs {
			if scod.Direction == skymodules.DiffApply {
		...
		for _, sfod := range cc.SiafundOutputDiffs {
			if sfod.Direction == skymodules.DiffApply {
...
func dbCalculateBlockFacts(tx *bolt.Tx, cs skymodules.ConsensusSet, block types.Block) blockFacts {
...
func dbAddGenesisBlock(tx *bolt.Tx) {
	...
	dbAddBlockFacts(tx, blockFacts{
		BlockFacts: skymodules.BlockFacts{
			BlockID:            id,
			Height:             0,
