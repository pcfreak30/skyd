...
var (
	...
	logFile = skymodules.FeeManagerDir + ".log"
...
func (fm *FeeManager) callInitPersist() error {
	...
	fh, err := os.OpenFile(filename, os.O_RDWR, skymodules.DefaultFilePerm)
	if err != nil {
...
func (ps *persistSubsystem) callPersistFeeCancelation(feeUID skymodules.FeeUID) error {
...
func (ps *persistSubsystem) callPersistFeeUpdate(feeUID skymodules.FeeUID, payoutHeight types.BlockHeight) error {
	entry := createUpdateFeeEntry(feeUID, payoutHeight)
	return ps.managedAppendEntry(entry)
}

// callPersistNewFee will persist a new fee to disk.
func (ps *persistSubsystem) callPersistNewFee(fee skymodules.AppFee) error {
...
// persist file.
func (ps *persistSubsystem) callPersistTxnConfirmed(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
...
// file.
func (ps *persistSubsystem) callPersistTxnCreated(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
...
// file.
func (ps *persistSubsystem) callPersistTxnDropped(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
...
func (fm *FeeManager) newPersist(filename string) error {
	// Open Persist file
	fh, err := os.OpenFile(filename, os.O_RDWR|os.O_CREATE, skymodules.DefaultFilePerm)
	if err != nil {
