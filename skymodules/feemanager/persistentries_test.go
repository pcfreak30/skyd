...
func checkApplyAddEntry(fm *FeeManager, fee skymodules.AppFee) error {
...
func checkApplyCancelEntry(fm *FeeManager, feeUID skymodules.FeeUID) error {
...
// to the FeeManager and checking the result.
func checkApplyUpdateEntry(fm *FeeManager, feeUID skymodules.FeeUID) error {
...
func TestApplyEntryWatchdog(t *testing.T) {
	...
	feeUIDs := []skymodules.FeeUID{fee.FeeUID}
...
func checkApplyTxnCreatedEntry(fm *FeeManager, feeUID skymodules.FeeUID, txnID types.TransactionID) error {
	// createTxnCreatedEntry with 1 feeUID should only create 1 persist entry
	pes, _ := createTxnCreatedEntrys([]skymodules.FeeUID{feeUID}, txnID)
	pe := pes[0]
...
func checkApplyTxnConfirmedEntry(fm *FeeManager, feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
	w := fm.staticCommon.staticWatchdog
...
func checkApplyTxnDroppedEntry(fm *FeeManager, feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
	w := fm.staticCommon.staticWatchdog
