...
var (
	...
	// Enforce that FeeManager satisfies the skymodules.FeeManager interface.
	_ skymodules.FeeManager = (*FeeManager)(nil)
)
...
type (
	...
	FeeManager struct {
		...
		fees map[skymodules.FeeUID]*skymodules.AppFee
	...
	feeManagerCommon struct {
		// Dependencies
		staticCS     skymodules.ConsensusSet
...
func New(cs skymodules.ConsensusSet, tp skymodules.TransactionPool, w skymodules.Wallet, persistDir string) (*FeeManager, error) {
	return NewCustomFeeManager(cs, tp, w, persistDir, skymodules.ProdDependencies)
}

// NewCustomFeeManager creates a new FeeManager using custom dependencies.
func NewCustomFeeManager(cs skymodules.ConsensusSet, tp skymodules.TransactionPool, w skymodules.Wallet, persistDir string, deps skymodules.Dependencies) (*FeeManager, error) {
	...
	err := os.MkdirAll(persistDir, skymodules.DefaultDirPerm)
	if err != nil {
	...
	// Create FeeManager
	fm := &FeeManager{
		fees: make(map[skymodules.FeeUID]*skymodules.AppFee),
	...
	// Create the watchdog
	wd := &watchdog{
		feeUIDToTxnID: make(map[skymodules.FeeUID]types.TransactionID),
...
func feeNotFoundError(feeUID skymodules.FeeUID) error {
	return errors.Compose(ErrFeeNotFound, fmt.Errorf("feeUID %v", feeUID))
}

// uniqueID creates a random unique FeeUID.
func uniqueID() skymodules.FeeUID {
	return skymodules.FeeUID(persist.UID())
}

// AddFee adds a fee to the fee manager.
func (fm *FeeManager) AddFee(address types.UnlockHash, amount types.Currency, appUID skymodules.AppUID, recurring bool) (skymodules.FeeUID, error) {
...
func (fm *FeeManager) CancelFee(feeUID skymodules.FeeUID) error {
	// Add thread group
	if err := fm.staticCommon.staticTG.Add(); err != nil {
		return err
	}
...
func (fm *FeeManager) PaidFees() ([]skymodules.AppFee, error) {
	// Add thread group
	if err := fm.staticCommon.staticTG.Add(); err != nil {
		return nil, err
	}
	...
	var paidFees []skymodules.AppFee
	fm.mu.Lock()
	...
	fm.mu.Unlock()
	// Sort by timestamp.
	sort.Sort(skymodules.AppFeeByTimestamp(paidFees))
...
// FeeManager
func (fm *FeeManager) PendingFees() ([]skymodules.AppFee, error) {
	// Add thread group
	if err := fm.staticCommon.staticTG.Add(); err != nil {
		return nil, err
	}
	...
	var pendingFees []skymodules.AppFee
	fm.mu.Lock()
	for _, fee := range fm.fees {
		if !fee.PaymentCompleted {
	...
	fm.mu.Unlock()
	// Sort by timestamp.
	sort.Sort(skymodules.AppFeeByTimestamp(pendingFees))
