...
func createAddFeeEntry(fee skymodules.AppFee) (ret [persistEntrySize]byte) {
...
// cancel fee request.
func createCancelFeeEntry(feeUID skymodules.FeeUID) (ret [persistEntrySize]byte) {
...
func createTxnConfirmedEntrys(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) ([][persistEntrySize]byte, error) {
...
func createTxnCreatedEntrys(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) ([][persistEntrySize]byte, error) {
...
func createTxnDroppedEntrys(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) ([][persistEntrySize]byte, error) {
...
// created, confirmed, or dropped.
func createTransactionFeeUIDEntrys(feeUIDs []skymodules.FeeUID, txnID types.TransactionID, entryType types.Specifier) (rets [][persistEntrySize]byte, err error) {
...
func createUpdateFeeEntry(feeUID skymodules.FeeUID, payoutHeight types.BlockHeight) (ret [persistEntrySize]byte) {
...
func (fm *FeeManager) applyTxnAndFeeUIDs(etf entryTransactionAndFeeUIDs, entryType types.Specifier) ([]skymodules.FeeUID, error) {
	index := 0
	feeUIDs := make([]skymodules.FeeUID, etf.NumFeeUIDs)
	for i := 0; i < etf.NumFeeUIDs; i++ {
		var feeUID skymodules.FeeUID
