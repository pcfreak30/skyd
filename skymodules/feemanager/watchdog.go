...
type (
	...
	watchdog struct {
		...
		feeUIDToTxnID map[skymodules.FeeUID]types.TransactionID
...
func (w *watchdog) addFeesToTransaction(feeUIDs []skymodules.FeeUID, tt trackedTransaction) error {
	txnID := tt.txn.ID()
...
// updates the watchdog
func (w *watchdog) callAddFeesToTransaction(feeUIDs []skymodules.FeeUID, txnID types.TransactionID) error {
	w.mu.Lock()
	defer w.mu.Unlock()
	tt, ok := w.txns[txnID]
	if !ok {
...
func (w *watchdog) callFeeTracked(feeUID skymodules.FeeUID) (types.TransactionID, bool) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
// associated fees.
func (w *watchdog) callMonitorTransaction(feeUIDs []skymodules.FeeUID, txn types.Transaction) error {
	w.mu.Lock()
	defer w.mu.Unlock()
