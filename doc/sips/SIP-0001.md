...
The program data is a single field in the program. Having a single data field
allows multiple instructions to re-use the same input, which can be helpful for
compression. Having a single program data field also allows possible future
extensions to the MDM that would modify the program data, substantially boosting
the power of the MDM. No such instructions exist within the SIP-0001
specification.
...
allows the program output to begin sending to the caller even before the entire
program has uploaded or finished executing.
...
Instruction inputs and outputs will be encoded using the Sia encoding format.
...
simultaneously, allowing a single host to saturate both the upload bandwidth and
the download bandwidth of a renter on a single connection. This greatly improves
the responsiveness of the renter for streaming downloads when a user is
currently uploading large amounts of data.
...
contract that is being modified. The exclusive lock will not block reads,
however it will block parallel attempts to PayByContract or RenewContract on the
same contract.

The contract lock will prevent all other processes from updating the contract.
This is because other processes that can update the contract may change the
filesize or Merkle root, which would invalidate any updates and proofs made by
the ReadWrite MDM.

The lock around the contract is held until the program is completed, the updated
contract is sent to the caller for verification, and then the caller
subsequently signs the update, rejects the update, or times out.

#### Interruption

A program being executed may be interrupted. If an interrupt signal is received,
the host will stop executing instructions, and will stop sending any output that
has been queued to be delivered to the caller for that program. Because of
latency between the caller and the host, the interrupt may arrive late. The host
will nonetheless send a message acknowledging the interrupt.

// TODO: Figure out the data format for the interrupt.

```go
type Interrupt struct {
	Signal types.Specifier
}

type InterruptAcknowledge {
	Signal types.Specifier
}
```

For ReadOnly programs, an interrupt signals that execution should stop and any
instruction output that has not yet been sent should be discarded rather than
sent.

For ReadWrite programs, all the same things are implied, and also implied is
that the program should be aborted, no changes should be committed. If the
update was pre-signed, the changes may have already been committed and it may be
too late to accept an interrupt. The host will still acknowledge the interrupt,
however the host will also send the finalized contract update.
...
the interrupt. It may be the case that all instructions have already been
executed, and all instruction output has already been sent over the network. If
this happens, the renter will implicitly interpret the interrupt as having been
accepted late upon receiving the final instruction output.

For ReadOnly execution, an interrupt merely stops the execution of instructions,
and if there is any output that has yet to be sent to the renter, that output is
blocked from being sent. Instead, the host will send an 'int
