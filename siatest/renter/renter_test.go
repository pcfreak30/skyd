...
func TestRenterBubbleForcedUpdateTime(t *testing.T) {
	...
	// Create a renter with the ForceBubbleDependency
	testDir := renterTestDir(t.Name())
	renterParams := node.Renter(testDir)
	renterParams.RenterDeps = &dependencies.DependencyForceBubbleOnly{}
	r, err := siatest.NewCleanNode(renterParams)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Grab the root directory and confirm that the dependency is preventing the
	// aggregatelasthealthcheck time from updating
	rf, err := r.RenterDirRootGet(skymodules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	initialLHCT := rf.Directories[0].AggregateLastHealthCheckTime
	time.Sleep(renter.HealthCheckInterval + time.Second)
	rf, err = r.RenterDirRootGet(skymodules.RootSiaPath())
	if err != nil {
		t.Fatal(err)
	}
	if !initialLHCT.Equal(rf.Directories[0].AggregateLastHealthCheckTime) {
		t.Fatal("Health loop is still running")
	}

	// Set the forcedUpdateTime to be in the future
	fut := time.Now().Add(time.Minute)
	err = r.RenterBubbleForcedUpdateTimePost(fut)
	if err != nil {
		t.Fatal(err)
	}

	// Check the root directory. The AggregateLastHealthCheckTime should have
	// been updated
	err = build.Retry(100, 100*time.Millisecond, func() error {
		rf, err := r.RenterDirRootGet(skymodules.RootSiaPath())
		if err != nil {
		...
		if initialLHCT.Equal(rf.Directories[0].AggregateLastHealthCheckTime) {
			return errors.New("AggregateLastHealthCheckTime not updated")
		}
		return nil
	})
	if err != nil {
