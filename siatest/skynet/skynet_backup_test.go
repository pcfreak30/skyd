...
func testSingleFileMultiPart(t *testing.T, tg *siatest.TestGroup) {
	...
	multiFileTest := func(filename, skykeyName string, files []siatest.TestFile) {
		...
		skylink, sup, _, err := portal1.UploadNewMultipartSkyfileEncryptedBlocking(filename, files, "", false, skymodules.DirResModeStandard, "", 404, false, nil, skykeyName, skykey.SkykeyID{})
		if err != nil {
			t.Fatalf("Test %v failed to upload: %v", filename, err)
		}
...
func testDirectoryBasic(t *testing.T, tg *siatest.TestGroup) {
	...
	directoryTest := func(filename, skykeyName, defaultPath string, files []siatest.TestFile, disableDefaultPath, force bool) {
		...
		skylink, sup, _, err := portal1.UploadNewMultipartSkyfileEncryptedBlocking(filename, files, defaultPath, disableDefaultPath, skymodules.DirResModeStandard, "", 404, force, nil, skykeyName, skykey.SkykeyID{})
		if err != nil {
			t.Fatalf("Test %v failed to upload: %v", filename, err)
		}
